...
struct Slab_cache::Slab
{
     ...
     */
    [[nodiscard]] static auto from_buffer (void *p)
    {
        // Masking off the page offset rounds any buffer pointer down to the Slab header it belongs to
        return std::launder (reinterpret_cast<Slab *>(reinterpret_cast<uintptr_t>(p) & ~OFFS_MASK (0)));
    }
 ...
 * !head &&  curr => illegal
 */
Slab_cache::Slab_cache (size_t s, size_t a) : bsz (static_cast<uint16_t>(aligned_up (max (a, alignof (Slab::Buffer)), max (s, sizeof (Slab::Buffer))))),
 ...
 *
 * @param p Pointer to the element (or nullptr)
 */
