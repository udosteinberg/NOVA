...
void const *Acpi_table::map (uint64_t p, unsigned w)
{
    ...
    auto const s { std::start_lifetime_as<Acpi_header const> (h)->length };
...
bool Acpi_table::validate (uint64_t phys, size_t avl) const
{
    // Table size must be sane and fit
    if (header.length < sizeof (Acpi_table) || header.length > avl) [[unlikely]]
        return false;

    // Checksum must be correct
    auto const valid { Checksum::additive<uint8_t> (this, header.length) == 0 };
