...
void Buddy::init()
{
    ...
    auto const size { Multiboot::ea - Kmem::ptr_to_phys (&KMEM_HVAS) };

    // Count of pages + block metadata fitting into the memory pool
    auto const cnt { size / (PAGE_SIZE (0) + sizeof (Block)) };

    min_idx  = page_to_index (virt);
    max_idx  = min_idx + cnt;
    blk_base = std::start_lifetime_as_array<Block> (virt + size - cnt * sizeof (Block), cnt);

    // Ensure the block metadata does not overlap the pages it describes
    assert (index_to_page (max_idx) <= reinterpret_cast<uintptr_t>(blk_base));

    // Free all pages in the pool
    for (auto i { virt }; i < index_to_page (max_idx); i += PAGE_SIZE (0))
...
void *Buddy::alloc (order_t ord, Fill fill)
{
    ...
    for (auto o { ord }; o <= max_order; o++) {
        ...
        // Ensure virtual and physical address are order-aligned
        assert (((BITN (PAGE_BITS + ord) - 1) & reinterpret_cast<uintptr_t>(ptr)) == 0);
        assert (((BITN (PAGE_BITS + ord) - 1) & Kmem::ptr_to_phys (ptr)) == 0);
...
void Buddy::coalesce (Block *block)
{
    ...
    for (auto o { block->ord }; o < max_order; block->ord = ++o) {
