 ...
 */
Space_hst::Space_hst() : Space_mem { Kobject::Subtype::HST }, vmid { 0 }
{
    Vmid::allocator.reserve (vmid);

    Space_obj::nova.insert (Space_obj::Selector::NOVA_HST, Capability { this, std::to_underlying (Capability::Perm_sp::TAKE) });

    // Mappable bounds: constrained by physical addressing width (TBD) and user boundary
    constexpr auto max_addr { user_boundary() };

    // Compute image addresses within mappable bounds
...
Space_hst *Space_hst::create (Status &s, Pd *pd)
{
    ...
    if (!ref_pd) [[unlikely]]
        ...
        // Allocate VMID
        if (auto const r { Vmid::allocator.alloc() }) [[likely]] {

            auto const vmid { r.val() };

            // Create new HST object
            auto const obj { new (ref_pd->hst_cache) Space_hst { ref_pd, vmid } };
