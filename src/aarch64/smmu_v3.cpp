...
Smmu_v3::Smmu_v3 (uint64_t base, unsigned const (&i)[4]) : Smmu { base, 0x20000 }, intid { i[0], i[1], i[2], i[3] },
...
bool Smmu_v3::init()
{
    ...
    for (unsigned i { 0 }; i < sizeof (intid) / sizeof (*intid); i++)
        if (Intid::type (intid[i]) == Intid::Type::SPI) [[likely]]
            Interrupt::conf_spi (Intid::to_spi (intid[i]), false, false, Cpu::id);
...
Status Smmu_v3::assign_dev (Space_dma *dma, uintptr_t dad)
{
    auto const sid { static_cast<uint32_t>(dad) };
    ...
    auto const ste { strt.entry (coherent(), sid) };

    // Check if stream table entry has been allocated
    if (!ste) [[unlikely]]
        return Status::MEM_OBJ;

    // Invalidation lambdas
    auto cfgi = [&]() { return cfgi_ste (sid); };
    auto tlbi = [&]() { return tlbi_vmalls12e1 (ste->vmid()); };

    Status s;

    // Detach STE
    if ((s = ste->detach (coherent(), cfgi, tlbi)) != Status::SUCCESS) [[unlikely]]
        return s;
    ...
    // Attach STE
    if ((s = ste->attach (coherent(), cfgi, ptab, vmid, pas, isz, lev)) != Status::SUCCESS) [[unlikely]]
        return s;
    ...
    while (!evtq.is_empty (hwi)) {
        ...
        switch (evt.type()) {
            ...
            case Evt::Type::C_BAD_STREAMID:
                trace (TRACE_SMMU, "SMMU: %#lx C_BAD_STREAMID SID:%#x", phys, evt.sid());
                break;

            case Evt::Type::C_BAD_STE:
                trace (TRACE_SMMU, "SMMU: %#lx C_BAD_STE SID:%#x", phys, evt.sid());
                break;

            default:
                trace (TRACE_SMMU, "SMMU: %#lx Event Type:%#x SID:%#x", phys, std::to_underlying (evt.type()), evt.sid());
                break;
        }
    }
}
...
void Smmu_v3::handle_glb (uint32_t const err)
{
    ...
    if (err & BIT (0)) [[unlikely]] {
        ...
        trace (TRACE_SMMU, "SMMU: %#lx CMD Queue Error CMD %#x ERR %#x", phys, cmd.type(), c >> 24 & BIT_RANGE (6, 0));
    }

    if (err & BIT_RANGE (31, 1)) [[unlikely]]
        trace (TRACE_SMMU, "SMMU: %#lx Global Error %#x", phys, err);
}
