...
void Ec_arch::handle_exc_kern (Exc_regs *r)
{
    ...
    switch (r->ep()) {
        ...
        default:
            trace (0, "KERN exception %#llx (ISS:%#lx) at IP:%#llx", r->ep(), iss, r->el2.elr);
            break;
    }
...
void Ec_arch::handle_exc_user (Exc_regs *r)
{
    ...
    if (current->subtype == Kobject::Subtype::EC_VCPU) {
        trace (TRACE_EXCEPTION, "EC:%p VMX %#llx at M:%#x IP:%#llx", static_cast<void *>(current), r->ep(), r->emode(), r->el2.elr);
        current->regs.vmcb->save_gst();
        resolved ? ret_user_vmexit() : send_msg<ret_user_vmexit>();
    } else {
        trace (TRACE_EXCEPTION, "EC:%p EXC %#llx at M:%#x IP:%#llx", static_cast<void *>(current), r->ep(), r->emode(), r->el2.elr);
