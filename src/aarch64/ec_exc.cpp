...
void Ec_arch::handle_exc_kern (Exc_regs *r)
{
    ...
    switch (r->ep()) {
        ...
        case 0x25:      // Data Abort
            Console::panic ("KERN %s abort (ISS:%#lx) at IP:%#lx FAR:%#lx", r->ep() == 0x21 ? "INST" : "DATA", iss, r->el2.elr, r->el2.far);

        default:
            Console::panic ("KERN exception %#lx (ISS:%#lx) at IP:%#lx", r->ep(), iss, r->el2.elr);
    }
}
