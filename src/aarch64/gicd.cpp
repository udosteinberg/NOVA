...
bool Gicd::mmap_mmio()
{
    ...
    for (size_t size { PAGE_SIZE (0) }; size <= PAGE_SIZE (0) << 4; size <<= 4) {
        ...
        if (pidr) {
            ...
            Interrupt::num_spi = min (Intid::NUM_SPI, (typer & BIT_RANGE (4, 0)) << 5);
            ...
            if (arch >= 3) [[likely]] {
                ...
                if (typer & BIT (8)) [[likely]]
                    Interrupt::num_espi = min (Intid::NUM_ESPI, (e + 1) << 5);

                // Determine number of LPIs
                if (typer & BIT (17)) [[likely]]
                    Interrupt::num_lpi = min (Intid::NUM_LPI, l ? BIT (l + 1) : Intid::to_lpi (BIT (ord_iid)));
            }
...
bool Gicd::init_mmio()
{
    ...
    if (arch < 3) [[unlikely]] {
        ...
        // Init SGIs and PPIs
        init_intid (Arr32::ICENABLER, Arr32::IGROUPR, Arr32::IPRIORITYR, Intid::BASE_SGI, Intid::NUM_SGI + Intid::NUM_PPI);
    ...
    // Init SPIs and ESPIs
    if (Cpu::bsp) [[unlikely]] {
        init_intid (Arr32::ICENABLER,   Arr32::IGROUPR,   Arr32::IPRIORITYR,   Intid::BASE_SPI,         Interrupt::num_spi);
        init_intid (Arr32::ICENABLER_E, Arr32::IGROUPR_E, Arr32::IPRIORITYR_E, Intid::BASE_ESPI - 4096, Interrupt::num_espi);
    }
 ...
 */
void Gicd::init_intid (Arr32 clr, Arr32 grp, Arr32 pri, uint32_t iid, unsigned n)
{
...
bool Gicd::act_get (Intid iid)
{
    // INTID can be SGI/PPI for GICv2 and must be a valid SPI otherwise
    assert ((arch < 3 || iid >= Intid::BASE_SPI) && iid < Intid::from_spi (Interrupt::num_spi));

    // Determine active state
    return read (Arr32::ISACTIVER, iid / 32) & BIT (iid % 32);
}

void Gicd::act_set (Intid iid, bool act)
{
    // INTID can be SGI/PPI for GICv2 and must be a valid SPI otherwise
    assert ((arch < 3 || iid >= Intid::BASE_SPI) && iid < Intid::from_spi (Interrupt::num_spi));
    ...
    // Device-nGnRnE + DSB ensures completion at GICD register level
    Barrier::fsb (Barrier::Domain::NSH);
}

Status Gicd::conf_std (Intid iid, cpu_t cpu, bool lvl, bool msk)
{
    // INTID can be SGI/PPI for GICv2 and must be a valid SPI otherwise
    assert ((arch < 3 || iid >= Intid::BASE_SPI) && iid < Intid::from_spi (Interrupt::num_spi));
...
Status Gicd::conf_ext (Intid iid, cpu_t cpu, bool lvl, bool msk)
{
    // INTID must be a valid ESPI
    assert (arch >= 3 && iid >= Intid::BASE_ESPI && iid < Intid::from_espi (Interrupt::num_espi));

    // CPU must be valid
    assert (cpu < Cpu::count);

    // Adjust INTID for ESPI register block
    auto const reg { iid - 4096 };

    Lock_guard <Spinlock> guard { lock };

    // Disable during reconfiguration
    write (Arr32::ICENABLER_E, reg / 32, BIT (reg % 32));
    ...
    auto const v { read (Arr32::ICFGR_E, reg / 16) };
    write (Arr32::ICFGR_E, reg / 16, lvl ? v & ~b : v | b);

    // Configure target CPU
    write (Arr64::IROUTER_E, reg, Cpu::affinity_bits (Cpu::remote_mpidr (cpu)));

    // Enable as needed
    if (!msk) [[likely]]
        write (Arr32::ISENABLER_E, reg / 32, BIT (reg % 32));
...
{
    assert (arch < 3 && sgi < Intid::NUM_SGI && cpu < 8);
...
{
    assert (arch < 3 && sgi < Intid::NUM_SGI);
