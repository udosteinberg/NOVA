...
Ec::Ec (Pd *p, Fpu *f, Utcb *u, unsigned c, mword e, mword a, mword s, void (*x)()) : Kobject (Kobject::Type::EC, x ? Kobject::Subtype::EC_GLOBAL : Kobject::Subtype::EC_LOCAL), cpu (c), evt (e), pd (p), fpu (f), utcb (u), cont (x)
{
    ...
    pd->Space_mem::update (a, Buddy::ptr_to_phys (utcb), 0, Paging::Permissions (Paging::K | Paging::U | Paging::W | Paging::R), Memattr::Cacheability::MEM_WB, Memattr::Shareability::INNER);
}
...
uintptr_t Ec::root_addr()
{
    return *reinterpret_cast<uintptr_t *>(Buddy::sym_to_virt (&__boot_p1));
}
...
void Ec::handle_hazard (unsigned hzd, void (*func)())
{
    ...
    if (hzd & HZD_BOOT_HST) {
        ...
        trace (TRACE_PERF, "TIME: First HEC: %llums", Timer::ticks_to_ms (Timer::time() - *reinterpret_cast<uintptr_t *>(Buddy::sym_to_virt (&__boot_ts))));
    }

    if (hzd & HZD_BOOT_GST) {
        Cpu::hazard &= ~HZD_BOOT_GST;
        trace (TRACE_PERF, "TIME: First GEC: %llums", Timer::ticks_to_ms (Timer::time() - *reinterpret_cast<uintptr_t *>(Buddy::sym_to_virt (&__boot_ts))));
    }
}
