...
bool Utcb_arch::save (Mtd_arch const m, Cpu_regs *r) const
{
    ...
    if (m & Mtd_arch::Item::EL2_ELR_SPSR) {
        r->el2.elr  = el2.elr;
        r->el2.spsr = el2.spsr & SPSR_ALL_nRW ? Cpu::constrain_spsr32 (el2.spsr) : Cpu::constrain_spsr64 (el2.spsr);
    }
    ...
    if (m & Mtd_arch::Item::EL1_ELR_SPSR) {
        v->el1.elr  = el1.elr;
        v->el1.spsr = el1.spsr & SPSR_ALL_nRW ? Cpu::constrain_spsr32 (el1.spsr) : Cpu::constrain_spsr64 (el1.spsr);
    }
    ...
    if (m & Mtd_arch::Item::EL1_TCR)
        v->el1.tcr = v->el2.hcr & HCR_RW ? Cpu::constrain_tcr64 (el1.tcr) : Cpu::constrain_tcr32 (el1.tcr);
    ...
    if (m & Mtd_arch::Item::EL1_SCTLR)
        v->el1.sctlr = v->el2.hcr & HCR_RW ? Cpu::constrain_sctlr64 (el1.sctlr) : Cpu::constrain_sctlr32 (el1.sctlr);
