...
void Utcb_arch::load (Mtd_arch const m, Cpu_regs const *r)
{
    ...
    if (m & Mtd_arch::Item::EL0_IDR) {
        el0.tpidr   = r->el0.tpidr;
        el0.tpidrro = r->el0.tpidrro;
    }
    ...
    if (m & Mtd_arch::Item::EL2_ESR_FAR) {
        el2.esr = r->el2.esr;
        el2.far = (BIT64_RANGE (0x35, 0x34) | BIT64_RANGE (0x25, 0x24) | BIT64_RANGE (0x22, 0x20)) & BIT64 (r->ep()) ? r->el2.far : 0;
    }

    Vmcb const * const v = r->vmcb;
    ...
    if (m & Mtd_arch::Item::EL2_HPFAR)
        el2.hpfar = (BIT64_RANGE (0x25, 0x24) | BIT64_RANGE (0x21, 0x20)) & BIT64 (r->ep()) ? v->el2.hpfar : 0;
...
bool Utcb_arch::save (Mtd_arch const m, Cpu_regs *r) const
{
    if (m & Mtd_arch::Item::POISON)
        return false;

    if (m & Mtd_arch::Item::ICI)
        Cache::icache_invalidate();
    ...
    if (m & Mtd_arch::Item::EL0_IDR) {
        r->el0.tpidr   = el0.tpidr;
        r->el0.tpidrro = el0.tpidrro;
    }

    if (m & Mtd_arch::Item::EL2_ELR_SPSR) {
        r->el2.elr  = el2.elr;
        r->el2.spsr = el2.spsr & ~(el2.spsr & SPSR_ALL_nRW ? Cpu::spsr32_res0 : Cpu::spsr64_res0);
    }
    ...
    // Handle EL2_HCR early because it controls 64bit/32bit format of some EL1 registers
    if (m & Mtd_arch::Item::EL2_HCR)
        v->el2.hcr = ((el2.hcr | Vmcb::hcr_hyp1) & ~Vmcb::hcr_hyp0) & ~Cpu::hcr_res0;
    ...
    if (m & Mtd_arch::Item::EL1_ELR_SPSR) {
        v->el1.elr  = el1.elr;
        v->el1.spsr = el1.spsr & ~(el1.spsr & SPSR_ALL_nRW ? Cpu::spsr32_res0 : Cpu::spsr64_res0);
    }
    ...
    if (m & Mtd_arch::Item::EL1_TCR)
        v->el1.tcr = el1.tcr & ~(v->el2.hcr & HCR_RW ? Cpu::tcr64_res0 : Cpu::tcr32_res0);
    ...
    if (m & Mtd_arch::Item::EL1_SCTLR)
        v->el1.sctlr = (el1.sctlr |  (v->el2.hcr & HCR_RW ? Cpu::sctlr64_res1 : Cpu::sctlr32_res1))
                                  & ~(v->el2.hcr & HCR_RW ? Cpu::sctlr64_res0 : Cpu::sctlr32_res0);
    ...
    if (m & (Mtd_arch::Item::EL2_HCR | Mtd_arch::Item::EL2_ELR_SPSR))
        return (v->el2.hcr & HCR_RW ? BIT (16) | BIT_RANGE (5, 4) | BIT (0) : BIT (31) | BIT (27) | BIT (23) | BIT_RANGE (19, 16)) & BIT (r->emode());
