...
void Utcb_arch::load (Mtd_arch mtd, Cpu_regs const *r)
{
    mword const m = mtd.val();
...
bool Utcb_arch::save (Mtd_arch mtd, Cpu_regs *r) const
{
    mword const m = mtd.val();
    ...
    if (m & Mtd_arch::Item::EL2_ELR_SPSR) {
        r->el2.elr  = el2.elr;
        r->el2.spsr = el2.spsr & ~(el2.spsr & PSTATE_ALL_nRW ? Cpu::spsr32_res0 : Cpu::spsr64_res0);
    }
    ...
    if (m & Mtd_arch::Item::EL1_ELR_SPSR) {
        v->el1.elr  = el1.elr;
        v->el1.spsr = el1.spsr & ~(el1.spsr & PSTATE_ALL_nRW ? Cpu::spsr32_res0 : Cpu::spsr64_res0);
    }
    ...
    if (m & Mtd_arch::Item::EL1_TCR)
        v->el1.tcr = el1.tcr & ~(v->el1.spsr & PSTATE_ALL_nRW ? Cpu::tcr32_res0 : Cpu::tcr64_res0);
    ...
    if (m & Mtd_arch::Item::EL1_SCTLR)
        v->el1.sctlr = (el1.sctlr |  (v->el1.spsr & PSTATE_ALL_nRW ? Cpu::sctlr32_res1 : Cpu::sctlr64_res1))
