...
bool Smmu::configure (Pd *pd, Space::Index si, uintptr_t dev)
{
    auto sid = static_cast<uint16>(dev);
    auto smg = static_cast<uint8> (dev >> 16);
    auto ctx = static_cast<uint8> (dev >> 24);
    ...
    if (!config || sid >= BIT (sidx_bits) || smg >= num_smg || ctx >= num_ctx || !Space::is_dma (si))
        return false;

    trace (TRACE_SMMU, "SMMU: SID:%#06x SMG:%#04x CTX:%#04x assigned to PD:%p", sid, smg, ctx, static_cast<void *>(pd));
