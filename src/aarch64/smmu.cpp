...
Smmu *      Smmu::list      { nullptr };
uintptr_t   Smmu::mmap      { DEV_GLOBL_SMMU };
unsigned    Smmu::num_smg   { 0 };
unsigned    Smmu::num_ctx   { 0 };
bool        Smmu::nc        { false };

Smmu::Smmu (Hpt::OAddr phys, unsigned spi, bool lvl) : List (list), phys_base (phys), fault_spi (spi), fault_spi_lvl (lvl)
{
    Hptp::master.update (mmap, phys, 0,
    ...
    Hptp::master.update (mmap, phys, static_cast<unsigned>(bit_scan_reverse (smmu_size)) - PAGE_BITS,
                         Paging::Permissions (Paging::G | Paging::W | Paging::R),
                         Memattr::Cacheability::DEV, Memattr::Shareability::NONE);
    ...
    trace (TRACE_IOMMU, "SMMU: %#010llx %#lx r%up%u S1:%u S2:%u SMG:%u CTX:%u SID:%u-bit Mode:%u",
    ...
    // Reserve MMIO region
    Pd_kern::remove_user_mem (phys, smmu_size);
...
void Smmu::init()
{
    ...
    Interrupt::conf_spi (fault_spi, Cpu::id, false, fault_spi_lvl);
...
bool Smmu::configure (Pd *pd, Space::Index si, mword dev)
{
    ...
    trace (TRACE_IOMMU, "SMMU: %s: SID:%#06x SMG:%02u CTX:%02u assigned to PD:%p", __func__, sid, smg, ctx, static_cast<void *>(pd));

    {   Lock_guard <Spinlock> guard (lock);

        // First configure the CTX
        write (ctx, Ctx_Array64::TTBR0, ttbr);
        write (ctx, Ctx_Array32::TCR,   phys_bits << 16 | TCR_A64_TG0_4K | TCR_ALL_SH0_IS | TCR_ALL_ORGN0_WB_WA | TCR_ALL_IRGN0_WB_WA | VTCR_SL0_L1 | VTCR_T0SZ (IPA_BITS));
