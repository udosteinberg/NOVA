...
Smmu::Smmu (Board::Smmu const &brd) : List (list), board (brd)
{
    ...
    auto const idr0 { read (GR0_Register32::IDR0) };
    auto const idr1 { read (GR0_Register32::IDR1) };
    auto const idr2 { read (GR0_Register32::IDR2) };
    auto const idr7 { read (GR0_Register32::IDR7) };
    ...
    trace (TRACE_SMMU, "SMMU: %#010llx %#x r%up%u S1:%u S2:%u N:%u C:%u SMG:%u CTX:%u SID:%u-bit Mode:%u",
...
void Smmu::init()
{
    ...
    write (GR0_Register32::CR0, BIT (21) | BIT_RANGE (12, 11) | BIT (10) | BIT_RANGE (5, 4) | BIT_RANGE (2, 1));
}
...
bool Smmu::conf_smg (uint8 smg)
{
    ...
    write (ctx, GR1_Array32::CBAR,  sdid & BIT_RANGE (7, 0));
    ...
    write (ctx, Ctx_Array32::TCR,   oas << 16 | TCR_A64_TG0_4K | TCR_ALL_SH0_INNER | TCR_ALL_ORGN0_WB_RW | TCR_ALL_IRGN0_WB_RW | (lev - 2) << 6 | (64 - isz));
    write (ctx, Ctx_Array64::TTBR0, Kmem::ptr_to_phys (dma->get_ptab (lev - 1)));
    write (ctx, Ctx_Array32::SCTLR, BIT_RANGE (6, 5) | BIT (0));

    // Disable SMG during configuration
    write (smg, GR0_Array32::SMR, 0);

    // Configure and enable SMG
    write (smg, GR0_Array32::S2CR, BIT (27) | ctx);
...
bool Smmu::configure (Space_dma *dma, uintptr_t dad)
{
    ...
    auto const msk { static_cast<uint16>(dad >> 16) };
    auto       smg { static_cast<uint8> (dad >> 32) };
    auto const ctx { static_cast<uint8> (dad >> 40) };
    ...
    trace (TRACE_SMMU, "SMMU: SID:%#06x MSK:%#06x SMG:%#04x CTX:%#04x assigned to DMA:%p", sid, msk, smg, ctx, static_cast<void *>(dma));
...
void Smmu::fault()
{
    auto const gfsr { read (GR0_Register32::GFSR) };

    if (gfsr & BIT_RANGE (8, 0)) {

        auto const syn { read (GR0_Register32::GFSYNR0) };
    ...
    for (unsigned ctx { 0 }; ctx < num_ctx; ctx++) {
        ...
        auto const fsr { read (ctx, Ctx_Array32::FSR) };

        if (fsr & BIT_RANGE (8, 1)) {

            auto const syn { read (ctx, Ctx_Array32::FSYNR0) };
 ...
 */
void Smmu::tlb_invalidate (unsigned ctx, uint64 ipa)
{
    // Post TLB maintenance operation
    write (ctx, Ctx_Array64::TLBIIPAS2, ipa >> 12);
...
{
    Lock_guard <Spinlock> guard { tlb_lock };

    write (ctx, Ctx_Array32::TLBSYNC, 0);

    while (read (ctx, Ctx_Array32::TLBSTATUS) & BIT (0))
        pause();
}
