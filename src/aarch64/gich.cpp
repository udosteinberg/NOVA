...
void Gich::init_mmio()
{
    ...
    // Restrict to the minimum of what NOVA and HW implement
    num_apr = min (max_apr, BIT ((vtr >> 26 & BIT_RANGE (2, 0)) + 1) >> 5);
    num_lr  = min (max_lr, (vtr & BIT_RANGE (5, 0)) + 1);
...
void Gich::init_regs()
{
    ...
    // Restrict to the minimum of what NOVA and HW implement
    num_apr = min (max_apr, BIT ((vtr >> 26 & BIT_RANGE (2, 0)) + 1) >> 5);
    num_lr  = min (max_lr, (vtr & BIT_RANGE (4, 0)) + 1);
