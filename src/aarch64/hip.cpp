...
void Hip::build (uint64 root_s, uint64 root_e)
{
    ...
    length          = sizeof (*this);
    nova_p_addr     = Buddy::sym_to_phys (&NOVA_HPAS);
    nova_e_addr     = Buddy::sym_to_phys (&NOVA_HPAE);
    mbuf_p_addr     = Console_mbuf::con.addr();
    mbuf_e_addr     = Console_mbuf::con.addr() + Console_mbuf::con.size();
    root_p_addr     = root_s;
    root_e_addr     = root_e;
    acpi_rsdp_addr  = Acpi::rsdp;
    uefi_mmap_addr  = Uefi::map ? reinterpret_cast<uintptr_t>(Uefi::map) : ~0ULL;
    uefi_mmap_size  = Uefi::map ? static_cast<decltype (uefi_mmap_size)>(Uefi::msz) : 0;
    uefi_desc_size  = Uefi::map ? static_cast<decltype (uefi_desc_size)>(Uefi::dsc) : 0;
    uefi_desc_vers  = Uefi::map ? static_cast<decltype (uefi_desc_vers)>(Uefi::ver) : 0;
    ...
    trace (TRACE_ROOT, "INFO: UEFI: %#llx", uefi_mmap_addr);
