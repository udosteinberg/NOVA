...
uint32 const *Fdt::fdtb;
uint32 const *Fdt::fdte;
char   const *Fdt::fdts;
...
void Fdt::parse() const
{
    ...
    fdts = reinterpret_cast<char   const *>(reinterpret_cast<mword>(this) + offs_strings());
            ...
            case FDT_BEGIN_NODE:
                {
                    ...
                    while (*p++) ;
                    w = reinterpret_cast<uint32 const *>((reinterpret_cast<mword>(p) + 3) & ~3);
...
void Fdt::init()
{
    Fdt *fdt = reinterpret_cast<Fdt *>(Hpt::map (FDTB_ADDR));
