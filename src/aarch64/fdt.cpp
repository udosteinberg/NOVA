...
bool Fdt::Header::parse (uint64_t phys) const
{
    ...
    fdtb = std::start_lifetime_as<Unaligned_be<uint32_t> const> (reinterpret_cast<uintptr_t>(this) + offs_structs);
    fdte = std::start_lifetime_as<Unaligned_be<uint32_t> const> (reinterpret_cast<uintptr_t>(fdtb) + size_structs);
            ...
            case FDT_BEGIN_NODE:
                {
                    ...
                    while (*p++) ;
                    w = std::start_lifetime_as<Unaligned_be<uint32_t> const> ((reinterpret_cast<uintptr_t>(p) + 3) & ~3);
            ...
            case FDT_PROP:
                {
                    ...
                    w = std::start_lifetime_as<Unaligned_be<uint32_t> const> ((reinterpret_cast<uintptr_t>(w + 3) + len + 3) & ~3);
                    continue;
                }
...
bool Fdt::init()
{
    ...
    for (unsigned i { 0 }; i < sizeof (Board::smmu_v2) / sizeof (*Board::smmu_v2); i++)
        if (Board::smmu_v2[i].mmio) [[likely]]
            if (!Smmu_v2::create (Board::smmu_v2[i])) [[unlikely]]
                panic ("SMMUv2 setup failed");

    // Setup SMMUv3
    for (unsigned i { 0 }; i < sizeof (Board::smmu_v3) / sizeof (*Board::smmu_v3); i++)
        if (Board::smmu_v3[i].mmio) [[likely]]
            if (!Smmu_v3::create (Board::smmu_v3[i].mmio, { Board::smmu_v3[i].evt, Board::smmu_v3[i].pri, Board::smmu_v3[i].glb, Board::smmu_v3[i].cmd })) [[unlikely]]
    ...
    return std::start_lifetime_as<Fdt::Header const> (Hptp::map (MMAP_GLB_MAP0, p))->parse (p);
}
