...
bool Fdt::parse (uint64 phys) const
{
    if (!valid())
        return false;
...
bool Fdt::init()
{
    ...
    if (Board::spin_addr) {
        ...
        for (unsigned i = 0; i < cnt; i++)
            if (Psci::boot_cpu (i, Board::cpu[i].id))
                Cpu::count++;
    }

    auto p = reinterpret_cast<Uefi *>(Kmem::sym_to_virt (&Uefi::uefi))->fdtb;

    // No FDT or FDT not properly aligned
    if (!p || p & 7)
        return false;

    return static_cast<Fdt *>(Hptp::map (p))->parse (p);
}
