...
void Gicr::init()
{
    ...
    for (unsigned offs = 0; offs < Board::gic[1].size; offs += 0x20000) {

        Hptp::current().update (DEV_LOCAL_GICR, Board::gic[1].mmio + offs, bit_scan_reverse (0x20000) - PAGE_BITS,
        ...
        if (pidr) {
            ...
            if (typer >> 32 == Cpu::affinity) {
                trace (TRACE_INTR, "GICR: %#010llx Impl:%#x Prod:%#x r%up%u (v%u) PPI:%llu",
                       Board::gic[1].mmio + offs,
