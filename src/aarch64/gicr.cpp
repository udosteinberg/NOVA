...
Status Gicr::conf_lpi (arm_intid_t iid, cpu_t cpu, pci_t src, arm_evtid_t eid, bool msk, uintptr_t &msi_addr, uintptr_t &msi_data)
{
    ...
    // Current encoding is ITS[31:24] DEV[23:0]
    auto const num { src >> 24 };
    auto const did { src & BIT_RANGE (23, 0) };

    // Lookup GITS
    auto const gits { Gits::lookup (num) };

    // Configure LPI in GITS
    return gits ? gits->conf_lpi (iid, cpu, did, eid, msi_addr, msi_data) : Status::BAD_DEV;
}
