...
bool Gicr::mmap_mmio()
{
    for (uint64 addr = phys, size;; addr += size) {
        ...
        Hptp::current().update (DEV_LOCAL_GICR, addr, bit_scan_reverse (0x20000) - PAGE_BITS,
                                Paging::Permissions (Paging::G | Paging::W | Paging::R),
                                Memattr::Cacheability::DEV, Memattr::Shareability::NONE);

        Hptp::flush_cpu();      // XXX: Should flush VA
    ...
    Hptp::current().update (DEV_LOCAL_GICR, 0, bit_scan_reverse (0x20000) - PAGE_BITS,
