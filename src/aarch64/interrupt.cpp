...
{
    auto sgi = Intid::to_sgi (val & BIT_RANGE (9, 0));
...
{
    auto spi = Intid::to_spi (val & BIT_RANGE (9, 0));
...
{
    uint32 val = Gicc::ack(), i = val & BIT_RANGE (9, 0);
...
void Interrupt::configure (unsigned spi, Config cfg, uint32 &msi_addr, uint16 &msi_data)
{
...
void Interrupt::init()
{
    assert (count() <= sizeof (int_table) / sizeof (*int_table));

    for (unsigned spi = 0; spi < count(); spi++) {
