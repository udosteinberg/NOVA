...
void Acpi_table_madt::Controller_gicd::parse() const
{
    ...
    Gicd::phys = static_cast<uint64_t>(phys_gicd_hi) << 32 | phys_gicd_lo;

    trace (TRACE_FIRM | TRACE_PARSE, "MADT: GICD:%#010lx", Gicd::phys);
}

void Acpi_table_madt::Controller_gicr::parse() const
{
    // Use split loads to avoid unaligned accesses
    Gicr::phys = static_cast<uint64_t>(phys_gicr_hi) << 32 | phys_gicr_lo;

    trace (TRACE_FIRM | TRACE_PARSE, "MADT: GICR:%#010lx", Gicr::phys);
}
...
void Acpi_table_madt::Controller_gicc::parse() const
{
    ...
    trace (TRACE_FIRM | TRACE_PARSE, "MADT: CPU:%u GICC:%#010lx GICH:%#010lx", cpu, Gicc::phys, Gich::phys);
...
void Acpi_table_madt::parse() const
{
    ...
    for (auto a { addr + sizeof (*this) }; a < addr + table.header.length; ) {
        ...
        auto const c { reinterpret_cast<Controller const *>(a) };
