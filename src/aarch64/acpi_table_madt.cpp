...
void Acpi_table_madt::Controller_gicc::parse() const
{
    ...
    // Use split loads to avoid unaligned accesses
    auto const gicc { static_cast<uint64>(phys_gicc_hi) << 32 | phys_gicc_lo };
    auto const gich { static_cast<uint64>(phys_gich_hi) << 32 | phys_gich_lo };
    auto const gicr { static_cast<uint64>(phys_gicr_hi) << 32 | phys_gicr_lo };
    ...
    auto const mpidr { static_cast<uint64>(val_mpidr_hi) << 32 | val_mpidr_lo };
