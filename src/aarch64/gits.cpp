...
Gits::Gits (uint64_t p, uint32_t i) : List { list }, Mmio { p, 0x10000, Memattr::dev() }, id { i }, typer { read (Reg64::TYPER) }
{
    ...
    // Treat as noncoherent if cacheability/shareability bits are not programmable
    ...
    trace (TRACE_INTR, "GITS: %#010lx %03x:%03x r%up%u v%u DEV:%u EVT:%u ITT:%u HCC:%u VLPI:%u PLPI:%u NC:%u (#%u)",
           phys, BIT_RANGE (11, 0) & iidr, iidr >> 24, BIT_RANGE (3, 0) & iidr >> 16, BIT_RANGE (3, 0) & iidr >> 12, arch,
           ord_dev(), ord_evt(), ord_itt(), num_hcc(), feat_vlpi(), feat_plpi(), noncoherent, id);
}
...
bool Gits::init_baser (unsigned n)
{
    ...
    for (unsigned p { 0 }; p < 3; p++) {
        ...
        auto const ptr { new (noncoherent, ord_c + p * 2) Table };
        ...
        *baser = BIT64 (63) | BIT64 (62) * !!ord_1 | (noncoherent ? attr_nsnc : attr_isic) | uint64_t { n } << 56 | (val & BIT64_RANGE (52, 48)) | Kmem::ptr_to_phys (ptr) | size;
...
bool Gits::init_table (uint64_t baser, unsigned idx)
{
    ...
    return Kmem::phys_to_ptr<Table> (baser & BIT64_RANGE (47, 12))[s].allocate (noncoherent, o);
}
...
bool Gits::command (Cmd const &c)
{
    ...
    if (err) [[unlikely]] {
        ...
        cmdq.replace (noncoherent, e, hwi);
...
bool Gits::init()
{
    ...
    write (Reg64::CBASER, (noncoherent ? attr_nsnc: attr_isic) | BIT64 (63) | cmdq.get_ptr() | (BIT (cmdq.pao) - 1));
...
Status Gits::conf_lpi (Intid iid, arm_colid_t cid, arm_devid_t did, arm_evtid_t eid, uintptr_t &msi_addr, uintptr_t &msi_data)
{
    // Fail if DID or EID is too large
    if (did >= BIT (ord_dev()) || eid >= BIT (ord_itt())) [[unlikely]]
        return Status::BAD_PAR;

    trace (TRACE_INTR | TRACE_PARSE, "GITS: %#010lx MAPTI DID:%#x EID:%u => IID:%u CID:%u", phys, did, eid, uint32_t { iid }, cid);
