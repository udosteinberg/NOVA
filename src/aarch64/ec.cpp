...
void Ec::create_root()
{
    ...
    auto sc  = Sc::create (Cpu::id, ec, Sc::max_prio(), 1000);
...
void Ec::root_invoke (Ec *const self)
{
    ...
    for (unsigned i = 0; i < c; i++, p++) {
        ...
        if (p->type == 1) {
            ...
            for (unsigned o; size; size -= BIT64 (o), phys += BIT64 (o), virt += BIT64 (o))
                self->pd->update_space_mem (&Pd_kern::nova(), phys >> PAGE_BITS, virt >> PAGE_BITS, (o = static_cast<unsigned>(min (max_order (phys, size), max_order (virt, size)))) - PAGE_BITS, perm, Memattr::Cacheability::MEM_WB, Memattr::Shareability::INNER, Space::Index::CPU_HST);
        }
    }
    ...
    self->pd->Space_obj::insert (Space_obj::num - 1, Capability (&Pd_kern::nova(), BIT (0)));
    self->pd->Space_obj::insert (Space_obj::num - 2, Capability (self->pd, BIT_RANGE (4, 0)));
    self->pd->Space_obj::insert (Space_obj::num - 3, Capability (Ec::current, BIT_RANGE (4, 0)));
    self->pd->Space_obj::insert (Space_obj::num - 4, Capability (Sc::current, BIT_RANGE (4, 0)));
