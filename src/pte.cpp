...
PTT *Pte<PTT,L,B,I,O>::walk (IAddr v, unsigned n, bool a, bool nc)
{
    ...
    for (PTT *e = static_cast<decltype (e)>(this), pte;; e = static_cast<decltype (e)>(Buddy::phys_to_ptr (pte.addr())) + (v >> (--l * B + PAGE_BITS) & (BIT (B) - 1))) {
        ...
        for (;;) {
            ...
            pte.val = Atomic::load (&e->val, __ATOMIC_RELAXED);
            ...
            if (!pte.val || pte.large (l)) {
                ...
                OAddr tmp = Buddy::ptr_to_phys (p) | (l != L) * PTT::ptab_attr();

                if (!Atomic::compare_exchange (&e->val, &pte.val, tmp)) {
                    delete p;
                    continue;
                }

                if (nc)
                    Cache::clean_dcache (e);
...
Paging::Permissions Pte<PTT,L,B,I,O>::lookup (IAddr v, OAddr &p, unsigned &o, Memattr::Cacheability &ca, Memattr::Shareability &sh) const
{
    ...
    for (PTT const *e = static_cast<decltype (e)>(this);; e = static_cast<decltype (e)>(Buddy::phys_to_ptr (pte.addr())) + (v >> (--l * B + PAGE_BITS) & (BIT (B) - 1))) {
...
void Pte<PTT,L,B,I,O>::update (IAddr v, OAddr p, unsigned ord, Paging::Permissions pm, Memattr::Cacheability ca, Memattr::Shareability sh, bool nc)
{
    ...
    for (unsigned i = 0; i < BIT (ord - o); i++, v += BIT (o + PAGE_BITS), p += BIT (o + PAGE_BITS)) {
        ...
        for (unsigned j = 0; j < n; j++, x += s) {
            ...
            pte.val = Atomic::exchange (&e[j].val, x);
        ...
        if (nc)
            Cache::clean_dcache (e, n * sizeof (*e));
    }
