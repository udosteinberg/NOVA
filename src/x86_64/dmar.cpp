...
Dmar::Dmar (Paddr p) : List<Dmar> (list), reg_base ((hwdev_addr -= PAGE_SIZE) | (p & PAGE_MASK)), invq (static_cast<Dmar_qi *>(Buddy::alloc (ord, Buddy::Fill::BITS0))), invq_idx (0)
{
#if 0   // FIXME
    ...
    Dpt::set_lps (static_cast<unsigned>(bit_scan_reverse (cap >> 34 & 0xf) + 2));
...
void Dmar::assign (unsigned long rid, Pd *p)
{
    ...
    c->set (lev | p->vpid() << 8, p->dpt.init_root (true, lev + 1) | 1);
}
