...
bool Acpi_table_srat::parse() const
{
    ...
    while (ptr + sizeof (list_t) <= end) {
        ...
        switch (s->type()) {
            case Affinity::Type::LAPIC:  static_cast<Affinity_lapic  const *>(s)->parse(); break;
            case Affinity::Type::MEMORY: static_cast<Affinity_memory const *>(s)->parse(); break;
            case Affinity::Type::X2APIC: static_cast<Affinity_x2apic const *>(s)->parse(); break;
            default: break;
        }
    }
