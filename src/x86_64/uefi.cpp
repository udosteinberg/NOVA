...
bool Uefi::Graphics_output_protocol::select_mode (Info &info)
{
    ...
    for (unsigned n { 0 }; n < mode->max; n++) {
        ...
        if (sizeof (uint32_t) * i->pitch * i->res_y > Console_fbuf::max_size) [[unlikely]]
            continue;

        // Prefer mode with more rows
        if (y < i->res_y) [[unlikely]] {
            y = i->res_y;
            m = n;
        }
    }
...
bool Uefi::Bsv_table::exit (handle img, Info &info) const
{
    ...
    for (unsigned i { 3 }; i--; ) {
        ...
        // Retry with the required size plus 3 additional descriptors (d is still unknown at this point)
        if (allocate_pool (Memory_type::LDR_DATA, m += 3 * sizeof (Memory_desc), &p) != Status::SUCCESS) [[unlikely]]
            return false;
    }
