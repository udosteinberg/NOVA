...
bool Uefi::Graphics_output_protocol::select_mode (Info &info) const
{
    uint32_t y { 0 }, m { 0 };
    ...
    info.gfx.pitch = mode_info->pitch;
    info.gfx.res_x = mode_info->res_x;
    info.gfx.res_y = mode_info->res_y;
...
bool Uefi::Bsv_table::handle_gfx (handle img, Info &info) const
{
    ...
    for (unsigned i { 0 }; i < cnt; i++)
        if (open_protocol (hdl[i], &uuid, &dev, img, nullptr, Open_attr::GET_PROTOCOL) == Status::SUCCESS && dev) [[likely]]
            if (std::start_lifetime_as<Graphics_output_protocol const> (dev)->select_mode (info)) [[likely]]
                break;
...
bool Uefi::Bsv_table::exit (handle img, Info &info) const
{
    ...
    for (uintptr_t i { 3 }, m { 0 };;) {
        ...
        if (!--i || s != Status::BUFFER_TOO_SMALL || allocate_pool (Memory_type::LDR_DATA, m += 3 * sizeof (Memory_desc), &p) != Status::SUCCESS) [[unlikely]]
            break;
    }
 ...
 */
void Uefi::init (handle img, uintptr_t sys_table, Info &info)
{
