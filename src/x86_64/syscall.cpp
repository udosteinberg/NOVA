...
void Ec::sys_create_sc (Ec *const self)
{
    ...
    trace (TRACE_SYSCALL, "EC:%p %s SC:%#lx EC:%#lx P:%u B:%u C:%u", static_cast<void *>(self), __func__, r.sel(), r.ec(), r.prio(), r.budget(), r.cos());
    ...
    auto sc = Sc::create (ec->cpu, ec, r.budget(), r.prio(), r.cos());
...
void Ec::sys_ctrl_hw (Ec *const self)
{
    ...
    switch (s) {
        ...
        case Status::BAD_PAR:
            trace (TRACE_ERROR, "%s: Invalid operation or parameter", __func__);
            sys_finish<Status::BAD_PAR> (self);

        case Status::BAD_FTR:
            trace (TRACE_ERROR, "%s: Feature not supported", __func__);
            sys_finish<Status::BAD_FTR> (self);
    }
}
...
void Ec::sys_assign_int (Ec *const self)
{
    ...
    Interrupt::configure (id, Interrupt::Config (r.cpu(), r.dev(), r.flg()), msi_addr, msi_data);
...
void Ec::sys_assign_dev (Ec *const self)
{
    ...
    trace (TRACE_SYSCALL, "EC:%p %s PD:%#lx SMMU:%#lx SI:%u DAD:%#lx", static_cast<void *>(self), __func__, r.pd(), r.smmu(), static_cast<unsigned>(r.si()), r.dad());
