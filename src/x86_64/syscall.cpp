...
void Ec::sys_create_sc (Ec *const self)
{
    ...
    trace (TRACE_SYSCALL, "EC:%p SYS_CREATE SC:%#lx EC:%#lx P:%#x Q:%#x", self, r->sel(), r->ec(), r->qpd().prio(), r->qpd().quantum());
...
void Ec::sys_create_pt (Ec *const self)
{
    ...
    trace (TRACE_SYSCALL, "EC:%p SYS_CREATE PT:%#lx EC:%#lx EIP:%#lx", self, r->sel(), r->ec(), r->eip());
...
void Ec::sys_create_sm (Ec *const self)
{
    ...
    trace (TRACE_SYSCALL, "EC:%p SYS_CREATE SM:%#lx CNT:%lu", self, r->sel(), r->cnt());
