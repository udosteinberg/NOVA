...
void Ec::sys_create_ec (Ec *const self)
{
    ...
    if (r.hva() >= Space_hst::user_boundary() || r.sp() >= Space_hst::user_boundary()) [[unlikely]]
...
void Ec::sys_create_pt (Ec *const self)
{
    ...
    if (r.ip() >= Space_hst::user_boundary()) [[unlikely]]
        self->sys_finish_status (Status::BAD_PAR);
...
void Ec::sys_create_sm (Ec *const self)
{
    ...
    if (i && !Interrupt::valid (Intid { r.iid() })) [[unlikely]]
        self->sys_finish_status (Status::BAD_PAR);

    Status s;
    pd->create_sm (s, obj, r.sel(), i ? Kobject::Subtype::SM_INT : Kobject::Subtype::SM_REG, r.val());
...
void Ec::sys_assign_dev (Ec *const self)
{
    ...
    trace (TRACE_SYSCALL, "EC:%p %s DC:%#lx DMA:%#lx->%#lx", static_cast<void *>(self), __func__, r.dc(), r.dma_old(), r.dma_new());
    ...
    auto const cdo { obj->lookup (r.dma_old()) };
    if (cdo.obj() && !cdo.validate (Capability::Perm_sp::ASSIGN, Kobject::Subtype::DMA)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);

    // Check DMA-new capability
    auto const cdn { obj->lookup (r.dma_new()) };
    if (cdn.obj() && !cdn.validate (Capability::Perm_sp::ASSIGN, Kobject::Subtype::DMA)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);

    auto const sdo { static_cast<Space_dma *>(cdo.obj()) };
    auto const sdn { static_cast<Space_dma *>(cdn.obj()) };

    // Error if both are null capabilities
    if (!sdo && !sdn) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);

    self->sys_finish_status (dc->smmu->assign_dev (dc, sdo, sdn, r.sbw()));
}
...
void Ec::sys_assign_int (Ec *const self)
{
    ...
    trace (TRACE_SYSCALL, "EC:%p %s OP:%u SM:%#lx DC:%#lx IDX:%#x CPU:%u VEC:%#x CFG:%#x", static_cast<void *>(self), __func__, r.op(), r.sm(), r.dc(), r.idx(), r.cpu(), r.vec(), r.cfg());
    ...
    // Check DC capability
    auto const cdc { obj->lookup (r.dc()) };
    if (cdc.obj() && !cdc.validate (Capability::Perm_dc::ASSIGN_INT)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);

    // DC can be nullptr for pin-based interrupts
    auto const dc { static_cast<Dc *>(cdc.obj()) };

    self->sys_finish_status (Interrupt::assign (r.op(), sm, dc, r.idx(), r.cpu(), r.vec(), r.cfg(), r.msi_addr(), r.msi_data()));
}
