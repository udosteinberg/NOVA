...
void Ec::sys_create_ec (Ec *const self)
{
    ...
    Status s;
    Pd::create_ec (s, self->get_obj(), r.sel(), static_cast<Pd *>(cpd.obj()), r.cpu(), r.evt(), r.sp(), r.hva(), r.flg());
...
void Ec::sys_create_pt (Ec *const self)
{
    ...
    if (self->get_obj()->insert (r.sel(), Capability (pt, static_cast<unsigned>(Capability::Perm_pt::DEFINED))) != Status::SUCCESS) {
        trace (TRACE_ERROR, "%s: Non-NULL CAP (%#lx)", __func__, r.sel());
        delete pt;
        sys_finish<Status::BAD_CAP> (self);
    }
...
void Ec::sys_create_sm (Ec *const self)
{
    ...
    auto cap = self->get_obj()->lookup (r.pd());
    ...
    if (self->get_obj()->insert (r.sel(), Capability (sm, static_cast<unsigned>(Capability::Perm_sm::DEFINED))) != Status::SUCCESS) {
        trace (TRACE_ERROR, "%s: Non-NULL CAP (%#lx)", __func__, r.sel());
        delete sm;
        sys_finish<Status::BAD_CAP> (self);
    }
...
void Ec::sys_ctrl_pt (Ec *const self)
{
    ...
    auto cap = self->get_obj()->lookup (r.pt());
...
void Ec::sys_ctrl_sm (Ec *const self)
{
    ...
    auto cap = self->get_obj()->lookup (r.sm());
