...
Ec_arch::Ec_arch (Pd *p, Fpu *f, Vmcs *v, unsigned c, unsigned long e, bool t) : Ec (p, f, v, c, e, send_msg<ret_user_vmexit_vmx>, t)
{
    ...
    auto cr3 = pd->loc[c].init_root (false) | (Cpu::feature (Cpu::FEAT_PCID) ? pd->pcid() : 0);
    v->init (reinterpret_cast<mword>(sys_regs() + 1), 0, // FIXME: pd->Space_pio::walk()
                                     cr3, pd->ept.init_root (false), pd->vpid());
...
{
    auto hzd = (Cpu::hazard ^ self->hazard) & (HZD_ILLEGAL | HZD_RECALL | HZD_BOOT_HST | HZD_RCU | HZD_FPU | HZD_DS_ES | HZD_SCHED);
    if (EXPECT_FALSE (hzd))
...
void Ec_arch::ret_user_exception (Ec *const self)
{
    ...
    auto hzd = (Cpu::hazard ^ self->hazard) & (HZD_ILLEGAL | HZD_RECALL | HZD_BOOT_HST | HZD_RCU | HZD_FPU | HZD_SCHED);
    if (EXPECT_FALSE (hzd))
...
{
    auto hzd = (Cpu::hazard ^ self->hazard) & (HZD_ILLEGAL | HZD_RECALL | HZD_TSC | HZD_BOOT_GST | HZD_RCU | HZD_SCHED);
    if (EXPECT_FALSE (hzd))
    ...
    asm volatile ("lea %0, %%rsp;"
                  ...
                  "vmlaunch;"
                  "mov %1, %%rsp;"
                  : : "m" (*self->exc_regs()), "i" (CPU_LOCAL_STCK + PAGE_SIZE) : "memory");
...
void Ec_arch::ret_user_vmexit_svm (Ec *const self)
{
    ...
    asm volatile ("lea %0, %%rsp;"
                  ...
                  "stgi;"
                  "jmp svm_handler;"
                  : : "m" (*self->exc_regs()), "m" (Vmcb::root), "i" (CPU_LOCAL_STCK + PAGE_SIZE) : "memory");
