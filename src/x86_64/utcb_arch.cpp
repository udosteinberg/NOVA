...
bool Utcb_arch::save_exc (Mtd_arch const m, Cpu_regs *r) const
{
    ...
    if (m & Mtd_arch::Item::RFLAGS)
        r->rfl = (rfl & ~(EFL_VIP | EFL_VIF | EFL_VM | EFL_RF | EFL_NT | EFL_IOPL)) | EFL_AC | EFL_IF;
...
void Utcb_arch::load_vmx (Mtd_arch const m, Cpu_regs const *r)
{
    ...
    if (m & Mtd_arch::Item::RFLAGS)
        rfl = Vmcs::read<mword> (Vmcs::Encoding::GUEST_RFLAGS);

    if (m & Mtd_arch::Item::RIP) {
        rip = Vmcs::read<mword>  (Vmcs::Encoding::GUEST_RIP);
        len = Vmcs::read<uint32> (Vmcs::Encoding::EXI_INST_LEN);
    }
    ...
    if (m & Mtd_arch::Item::INJ) {
        if (r->ep() == 33 || r->ep() == Event::gst_arch + Event::Selector::RECALL) {
            ...
            intr_info  = Vmcs::read<uint32> (Vmcs::Encoding::IDT_VECT_INFO);
            intr_error = Vmcs::read<uint32> (Vmcs::Encoding::IDT_VECT_ERROR);
        }
    }
    ...
    if (m & Mtd_arch::Item::SYSENTER) {
        ...
        sysenter_esp = Vmcs::read<mword>  (Vmcs::Encoding::GUEST_SYSENTER_ESP);
        sysenter_eip = Vmcs::read<mword>  (Vmcs::Encoding::GUEST_SYSENTER_EIP);
    }
...
bool Utcb_arch::save_vmx (Mtd_arch const m, Cpu_regs *r) const
{
    ...
    if (m & Mtd_arch::Item::RIP) {
        ...
        Vmcs::write (Vmcs::Encoding::ENT_INST_LEN, len);
    }

    if (m & Mtd_arch::Item::CTRL) {
        r->set_cpu_ctrl0<Vmcs> (ctrl[0]);
        r->set_cpu_ctrl1<Vmcs> (ctrl[1]);
    }
    ...
    if (m & Mtd_arch::Item::INJ) {
        ...
        Vmcs::write (Vmcs::Encoding::ENT_INTR_ERROR, intr_error);
    }
...
bool Utcb_arch::save_svm (Mtd_arch const m, Cpu_regs *r) const
{
    ...
    if (m & Mtd_arch::Item::CTRL) {
        r->set_cpu_ctrl0<Vmcb> (ctrl[0]);
        r->set_cpu_ctrl1<Vmcb> (ctrl[1]);
    }
