...
void Utcb_arch::load_vmx (Mtd_arch const m, Cpu_regs const &c)
{
    ...
    if (m & Mtd_arch::Item::GPR_0_7) {
        ...
        rsp = Vmcs::read<Vmcs::Encoding::GUEST_RSP>();
    }
    ...
    if (m & Mtd_arch::Item::RFLAGS)
        rfl = Vmcs::read<Vmcs::Encoding::GUEST_RFLAGS>();

    if (m & Mtd_arch::Item::RIP) {
        rip       = Vmcs::read<Vmcs::Encoding::GUEST_RIP>();
        inst_len  = Vmcs::read<Vmcs::Encoding::EXI_INST_LEN>();
        inst_info = Vmcs::read<Vmcs::Encoding::EXI_INST_INFO>();
    }

    if (m & Mtd_arch::Item::STA) {
        intr_state = Vmcs::read<Vmcs::Encoding::GUEST_INTR_STATE>();
        actv_state = Vmcs::read<Vmcs::Encoding::GUEST_ACTV_STATE>();
    }

    if (m & Mtd_arch::Item::QUAL) {
        qual[0] = Vmcs::read<Vmcs::Encoding::EXI_QUALIFICATION>();
        qual[1] = Vmcs::read<Vmcs::Encoding::GUEST_LINEAR_ADDRESS>();
        qual[2] = Vmcs::read<Vmcs::Encoding::GUEST_PHYSICAL_ADDRESS>();
    }
    ...
    if (m & Mtd_arch::Item::INJ) {
        if (c.exc.ep() == Vmcs::Reason::VMX_FAIL_STATE || c.exc.ep() == Event::gst_arch + Event::Selector::RECALL) {
            intr_info = Vmcs::read<Vmcs::Encoding::INJ_EVENT_IDENT>();
            intr_errc = Vmcs::read<Vmcs::Encoding::INJ_EVENT_ERROR>();
        } else {
            intr_info = Vmcs::read<Vmcs::Encoding::EXI_EVENT_IDENT>();
            intr_errc = Vmcs::read<Vmcs::Encoding::EXI_EVENT_ERROR>();
            vect_info = Vmcs::read<Vmcs::Encoding::ORG_EVENT_IDENT>();
            vect_errc = Vmcs::read<Vmcs::Encoding::ORG_EVENT_ERROR>();
        }
    }

    if (m & Mtd_arch::Item::CS_SS) {
        cs.set_vmx (Vmcs::read<Vmcs::Encoding::GUEST_SEL_CS>(), Vmcs::read<Vmcs::Encoding::GUEST_BASE_CS>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_CS>(), Vmcs::read<Vmcs::Encoding::GUEST_AR_CS>());
        ss.set_vmx (Vmcs::read<Vmcs::Encoding::GUEST_SEL_SS>(), Vmcs::read<Vmcs::Encoding::GUEST_BASE_SS>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_SS>(), Vmcs::read<Vmcs::Encoding::GUEST_AR_SS>());
    }

    if (m & Mtd_arch::Item::DS_ES) {
        ds.set_vmx (Vmcs::read<Vmcs::Encoding::GUEST_SEL_DS>(), Vmcs::read<Vmcs::Encoding::GUEST_BASE_DS>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_DS>(), Vmcs::read<Vmcs::Encoding::GUEST_AR_DS>());
        es.set_vmx (Vmcs::read<Vmcs::Encoding::GUEST_SEL_ES>(), Vmcs::read<Vmcs::Encoding::GUEST_BASE_ES>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_ES>(), Vmcs::read<Vmcs::Encoding::GUEST_AR_ES>());
    }

    if (m & Mtd_arch::Item::FS_GS) {
        fs.set_vmx (Vmcs::read<Vmcs::Encoding::GUEST_SEL_FS>(), Vmcs::read<Vmcs::Encoding::GUEST_BASE_FS>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_FS>(), Vmcs::read<Vmcs::Encoding::GUEST_AR_FS>());
        gs.set_vmx (Vmcs::read<Vmcs::Encoding::GUEST_SEL_GS>(), Vmcs::read<Vmcs::Encoding::GUEST_BASE_GS>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_GS>(), Vmcs::read<Vmcs::Encoding::GUEST_AR_GS>());
    }

    if (m & Mtd_arch::Item::TR)
        tr.set_vmx (Vmcs::read<Vmcs::Encoding::GUEST_SEL_TR>(), Vmcs::read<Vmcs::Encoding::GUEST_BASE_TR>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_TR>(), Vmcs::read<Vmcs::Encoding::GUEST_AR_TR>());

    if (m & Mtd_arch::Item::LDTR)
        ld.set_vmx (Vmcs::read<Vmcs::Encoding::GUEST_SEL_LDTR>(), Vmcs::read<Vmcs::Encoding::GUEST_BASE_LDTR>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_LDTR>(), Vmcs::read<Vmcs::Encoding::GUEST_AR_LDTR>());

    if (m & Mtd_arch::Item::GDTR)
        gd.set_vmx (0, Vmcs::read<Vmcs::Encoding::GUEST_BASE_GDTR>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_GDTR>(), 0);

    if (m & Mtd_arch::Item::IDTR)
        id.set_vmx (0, Vmcs::read<Vmcs::Encoding::GUEST_BASE_IDTR>(), Vmcs::read<Vmcs::Encoding::GUEST_LIMIT_IDTR>(), 0);

    if (m & Mtd_arch::Item::PDPTE) {
        pdpte[0] = Vmcs::read<Vmcs::Encoding::GUEST_PDPTE0>();
        pdpte[1] = Vmcs::read<Vmcs::Encoding::GUEST_PDPTE1>();
        pdpte[2] = Vmcs::read<Vmcs::Encoding::GUEST_PDPTE2>();
        pdpte[3] = Vmcs::read<Vmcs::Encoding::GUEST_PDPTE3>();
    }
    ...
    if (m & Mtd_arch::Item::CR) {
        ...
        cr2 = c.cr2;
        cr3 = Vmcs::read<Vmcs::Encoding::GUEST_CR3>();
    }

    if (m & Mtd_arch::Item::DR)
        dr7 = Vmcs::read<Vmcs::Encoding::GUEST_DR7>();

    if (m & Mtd_arch::Item::SYSENTER) {
        sysenter_cs  = Vmcs::read<Vmcs::Encoding::GUEST_SYSENTER_CS>();
        sysenter_esp = Vmcs::read<Vmcs::Encoding::GUEST_SYSENTER_ESP>();
        sysenter_eip = Vmcs::read<Vmcs::Encoding::GUEST_SYSENTER_EIP>();
    }

    if (m & Mtd_arch::Item::PAT)
        pat = Vmcs::read<Vmcs::Encoding::GUEST_PAT>();
...
bool Utcb_arch::save_vmx (Mtd_arch const m, Cpu_regs &c, Space_obj const *obj) const
{
    ...
    if (m & Mtd_arch::Item::INJ) {
        ...
        auto val { Vmcs::read<Vmcs::Encoding::CPU_CONTROLS_PRI>() };
    ...
    if (m & Mtd_arch::Item::SYSENTER) {
        Vmcs::write<Vmcs::Encoding::GUEST_SYSENTER_CS>(static_cast<uint32_t>(sysenter_cs));
    ...
    if (m & Mtd_arch::Item::EFER) {
        ...
        auto ent { Vmcs::read<Vmcs::Encoding::ENT_CONTROLS>() };
