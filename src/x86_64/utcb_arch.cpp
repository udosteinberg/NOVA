...
void Utcb_arch::load_vmx (Mtd_arch const m, Cpu_regs const *r)
{
    ...
    // CTRL state is write-only

    if (m & Mtd_arch::Item::QUAL) {
        qual[0] = Vmcs::read<uintptr_t> (Vmcs::Encoding::EXI_QUALIFICATION);
        qual[1] = Vmcs::read<uint64>    (Vmcs::Encoding::INFO_PHYS_ADDR);
    }
    ...
    if (m & Mtd_arch::Item::INJ) {
        if (r->dst_portal == 33 || r->dst_portal == NUM_VMI - 1) {
            ...
            intr_info = Vmcs::read<uint32> (Vmcs::Encoding::EXI_INTR_INFO);
            intr_errc = Vmcs::read<uint32> (Vmcs::Encoding::EXI_INTR_ERROR);
            vect_info = Vmcs::read<uint32> (Vmcs::Encoding::IDT_VECT_INFO);
            vect_errc = Vmcs::read<uint32> (Vmcs::Encoding::IDT_VECT_ERROR);
        }
    }
...
//      r->set_exc<Vmcs> (exc_bitmap);
    ...
    if (m & Mtd_arch::Item::INJ) {
        ...
        uint32 val = Vmcs::read<uint32> (Vmcs::Encoding::CPU_CONTROLS0);
        ...
        Vmcs::write (Vmcs::Encoding::ENT_INTR_INFO,  intr_info & ~0x3000);
        Vmcs::write (Vmcs::Encoding::ENT_INTR_ERROR, intr_errc);
    }
