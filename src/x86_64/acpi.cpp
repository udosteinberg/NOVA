...
bool Acpi::init()
{
    if (!resume) {
        ...
        if (static_cast<Acpi_table_rsdp *>(Hptp::map (MMAP_GLB_MAP0, rsdp))->parse (rsdt, size) == false)
            return false;

        if (rsdt)
            static_cast<Acpi_table_rsdt *>(Hptp::map (MMAP_GLB_MAP0, rsdt))->parse (rsdt, size);
        if (fadt)
