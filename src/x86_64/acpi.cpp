...
bool Acpi::init()
{
    if (!resume) {
        ...
        if ((ptr = Acpi_table::map (xsdt, 0))) [[likely]]
            ret &= static_cast<Acpi_table_xsdt const *>(ptr)->parse();
        ...
        if ((ptr = Acpi_table::map (dbg2, 0))) [[likely]]
            ret &= static_cast<Acpi_table_dbg2 const *>(ptr)->parse();
        if ((ptr = Acpi_table::map (madt, 0))) [[likely]]
            ret &= static_cast<Acpi_table_madt const *>(ptr)->parse();
        if ((ptr = Acpi_table::map (mcfg, 0))) [[likely]]
            ret &= static_cast<Acpi_table_mcfg const *>(ptr)->parse();
        if ((ptr = Acpi_table::map (srat, 0))) [[likely]]
            ret &= static_cast<Acpi_table_srat const *>(ptr)->parse();
        if ((ptr = Acpi_table::map (hest, 0))) [[likely]]
            ret &= static_cast<Acpi_table_hest const *>(ptr)->parse();
