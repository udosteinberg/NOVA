...
void Acpi::setup()
{
    ...
    if (xsdt)
        static_cast<Acpi_table_rsdt *>(Hptp::map (Hptp::Remap::MAP0, xsdt))->parse (xsdt, sizeof (uint64));
    else if (rsdt)
        static_cast<Acpi_table_rsdt *>(Hptp::map (Hptp::Remap::MAP0, rsdt))->parse (rsdt, sizeof (uint32));

    if (fadt)
        static_cast<Acpi_table_fadt *>(Hptp::map (Hptp::Remap::MAP0, fadt))->parse();
    if (hpet)
        static_cast<Acpi_table_hpet *>(Hptp::map (Hptp::Remap::MAP0, hpet))->parse();
    if (madt)
        static_cast<Acpi_table_madt *>(Hptp::map (Hptp::Remap::MAP0, madt))->parse();
    if (mcfg)
