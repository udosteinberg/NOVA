...
void Ec::create_root()
{
    ...
    obj->delegate (&Space_obj::nova, Space_obj::Selector::NOVA_OBJ, Space_obj::num - 1, 0, Capability::pmask);
    obj->delegate (&Space_obj::nova, Space_obj::Selector::ROOT_OBJ, Space_obj::num - 2, 0, Capability::pmask);
    obj->insert (Space_obj::num - 3, Capability (Pd::root, std::to_underlying (Capability::Perm_pd::DEFINED)));

    constexpr auto info_addr { (Space_hst::num - 1) << PAGE_BITS };
    constexpr auto utcb_addr { (Space_hst::num - 2) << PAGE_BITS };

    auto const ec { Pd::create_ec (s, obj, Space_obj::num - 4, Pd::root, Cpu::id, utcb_addr, 0, 0, BIT (2) | BIT (0)) };
    auto const sc { Pd::create_sc (s, obj, Space_obj::num - 5, ec, Cpu::id, 1000, Scheduler::priorities - 1, 0) };
    ...
    for (auto p { static_cast<Ph const *>(Hptp::map (ra + e->ph_offset)) }; c--; p++) {
        ...
        for (unsigned o; size; size -= BIT64 (o), phys += BIT64 (o), virt += BIT64 (o))
            hst->delegate (&Space_hst::nova, phys >> PAGE_BITS, virt >> PAGE_BITS, (o = static_cast<unsigned>(min (max_order (phys, size), max_order (virt, size)))) - PAGE_BITS, perm, Memattr::Cacheability::MEM_WB, Memattr::Shareability::INNER);
    }

    Hip::hip->build (root_s, root_e);

    hst->update (info_addr, Kmem::ptr_to_phys (Hip::hip), 0, Paging::Permissions (Paging::K | Paging::U | Paging::R), Memattr::Cacheability::MEM_WB, Memattr::Shareability::INNER);
