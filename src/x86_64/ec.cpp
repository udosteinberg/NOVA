...
// Constructors
Ec::Ec (Pd *own, void (*f)(), cpu_t c) : Kobject (Kobject::Type::EC, Kobject::Subtype::EC_GLOBAL), cont (f), regs (own, own, own), utcb (nullptr), pd (own), cpu (c), glb (true), evt (0)
{
    trace (TRACE_SYSCALL, "EC:%p created (PD:%p Kernel)", this, own);
}

Ec::Ec (Pd *, mword, Pd *p, void (*f)(), cpu_t c, uintptr_t e, mword u, mword s) : Kobject (Kobject::Type::EC, u ? (f ? Kobject::Subtype::EC_GLOBAL : Kobject::Subtype::EC_LOCAL) : Kobject::Subtype::EC_VCPU_REAL), cont (f), regs (p, p, p), pd (p), cpu (c), glb (!!f), evt (e)
{
...
void Ec::root_invoke()
{
    auto const e { static_cast<Eh const *>(Hptp::map (MMAP_GLB_MAP0, Multiboot::rs)) };
    if (!Multiboot::rs || !e->valid (ELF_MACHINE))
        die ("No ELF");
