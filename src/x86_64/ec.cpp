...
// Constructors
Ec::Ec (Pd *own, void (*f)(), cpu_t c) : Kobject (Kobject::Type::EC, Kobject::Subtype::EC_GLOBAL), cont (f), utcb (nullptr), pd (own), cpu (c), glb (true), evt (0)
{
...
//          regs.nst_ctrl<Vmcs>();
            ...
            sys_regs().rax = Kmem::ptr_to_phys (regs.vmcb = new Vmcb (pd->Space_pio::walk(), Kmem::ptr_to_phys (pd->npt.root_init (false))));
