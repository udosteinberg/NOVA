...
/*
 * Ptab consists of a tree of tables. Each table has size PAGE_SIZE, is indexed by
    ...
    for (auto ptr { &entry };; ptr = &pte->entry + T::lev_idx (--l, v)) {
        ...
        for (pte = static_cast<T>(*ptr);;) {
            ...
            if (type != Entry::Type::PTAB) {
                ...
                auto  const n { l - 1 };
                OAddr const p { (type == Entry::Type::LEAF) * (pte.addr (l) | T::page_attr (n, pte.page_pm(), pte.page_ca (l), pte.page_sh())) };
 ...
 */
template <typename T, typename I, typename O>
Paging::Permissions Ptab<T,I,O>::lookup (IAddr v, OAddr &p, unsigned &o, Memattr::Cacheability &ca, Memattr::Shareability &sh) const
{
    auto l { T::lev() }; T pte;
 ...
 */
template <typename T, typename I, typename O>
Status Ptab<T,I,O>::update (IAddr v, OAddr p, unsigned ord, Paging::Permissions pm, Memattr::Cacheability ca, Memattr::Shareability sh)
{
    ...
    auto const l { min (T::lev() - 1, o / T::bpl) };
