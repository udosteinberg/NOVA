...
{
    mword virt = idx_to_virt (idx); Hpt::OAddr phys; unsigned o;

    if (!space_mem()->lookup (virt, phys, o) || (phys & ~OFFS_MASK (0)) == Kmem::ptr_to_phys (&PAGE_0)) {
...
{
    *Kmem::phys_to_ptr<Capability> (walk (idx)) = cap;
}

size_t Space_obj::lookup (mword idx, Capability &cap)
{
    Hpt::OAddr phys; unsigned o;
    if (!space_mem()->lookup (idx_to_virt (idx), phys, o) || (phys & ~OFFS_MASK (0)) == Kmem::ptr_to_phys (&PAGE_0))
        return 0;

    cap = *Kmem::phys_to_ptr<Capability> (phys);
