 ...
 * Level > 0: Non-leaf Captables store pointers to the next level:
 *   - either Captable* (next level exists)
 *   - or nullptr (next level does not yet exist)
 * Level = 0: Leaf Captables store Capabilities:
 ...
 */
Atomic<Capability> *Space_obj::walk (unsigned long sel, bool a)
{
    ...
    for (auto ptr { &root };; ptr = &cte->slot[(sel >> --l * bpl) % Captable::entries]) {
        ...
        if (!(cte = *ptr)) [[unlikely]] {
            ...
            if (ptr->compare_exchange (cte, tbl)) [[likely]]
                cte = tbl;
            else
                delete tbl;
        }
...
Status Space_obj::update (unsigned long sel, Capability cap)
{
    ...
    // Try to acquire a reference on the capability object
    if (cap.acquire()) [[likely]]
        ptr->exchange (old, cap);   // success: replace with capability
    else
        ptr->exchange (old, old);   // failure: replace with null capability
...
Status Space_obj::insert (unsigned long sel, Capability cap)
{
    ...
    Capability old;
    if (ptr->compare_exchange (old, cap)) [[likely]]
        return Status::SUCCESS;
