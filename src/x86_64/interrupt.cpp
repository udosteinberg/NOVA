...
Status Interrupt::assign (bool attach, Sm * const sm, pci_t sbdf, uint16_t idx, uint16_t cpu, uint8_t vec, uint8_t cfg, uintptr_t &msi_addr, uintptr_t &msi_data)
{
    ...
    if (attach) [[likely]] {
        ...
        if (!Smmu::noir) [[likely]]
            if (auto const s { smmu->irte_set (irte, sm->ise, ioapic, iid, src, dst, vec, trg, Smmu::ise_msi (src, idx)) }; s != Status::SUCCESS) [[unlikely]]
                return s;
        ...
        // Update IOAPIC RTE (if PIN)
        if (Smmu::noir && ioapic)
            ioapic->rte_clr_compat (sm->ise, gsi, static_cast<uint8_t>(dst), vec);
