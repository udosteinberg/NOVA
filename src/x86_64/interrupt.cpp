...
{
    if (int_table[gsi].trg()) {

        auto ioapic = int_table[gsi].ioapic;

        if (ioapic)
            ioapic->set_rte_lo (gsi, msk << 16 | int_table[gsi].trg() << 15 | int_table[gsi].pol() << 13 | (VEC_GSI + gsi));
    }
}
...
void Interrupt::configure (unsigned gsi, unsigned flags, uint16 cpu, uint16 rid, uint32 &msi_addr, uint16 &msi_data)
{
    int_table[gsi].config = flags << 16 | cpu;
