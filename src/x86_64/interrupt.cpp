...
void Interrupt::deactivate (Sm *)
{
    ...
    auto const irt { Smmu_itl::lookup_irt (gsi) };
...
Status Interrupt::assign (bool attach, Sm * const sm, cpu_t cpu, uint16_t vec, pci_t src, uint8_t cfg, uintptr_t &msi_addr, uintptr_t &msi_data)
{
    ...
    auto const irt { Smmu_itl::lookup_irt (0) };

    return Smmu_itl::assign_int (irt, 0, cpu, static_cast<uint8_t>(VEC_GSI + vec), src, cfg, msi_addr, msi_data);
}
