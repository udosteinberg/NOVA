...
{
    auto const acm { std::start_lifetime_as<Acm::Header const> (Hptp::map (MMAP_GLB_MAP0, sinit_base)) };
    ...
    auto const info { std::start_lifetime_as<Acm::Info const> (reinterpret_cast<uintptr_t>(acm) + 4 * (acm->header_size + acm->scratch_size)) };
    ...
    if (pch && !std::start_lifetime_as<Acm::Id_pch const> (pch->list())->match (pch->count, read (Space::PUBLIC, Reg64::DIDVID))) [[unlikely]]
        return false;

    // ACM must support CPU
    auto const cpu { acm->cpu (info) };
    if (cpu && !std::start_lifetime_as<Acm::Id_cpu const> (cpu->list())->match (cpu->count, fms, Msr::read (Msr::Reg64::IA32_PLATFORM_ID))) [[unlikely]]
        return false;
...
{
    // EFI to PRE
    auto const efi_pre { std::start_lifetime_as<Data_efi_pre const> (MMAP_GLB_TXTH + (heap_base & Hpt::offs_mask (Hpt::bpl))) };
...
void Txt::restore()
{
    ...
    if (launched) [[likely]] {
        ...
        auto const efi_pre { std::start_lifetime_as<Data_efi_pre const> (MMAP_GLB_TXTH + (read (Space::PUBLIC, Reg32::HEAP_BASE) & Hpt::offs_mask (Hpt::bpl))) };
        auto const pre_mle { std::start_lifetime_as<Data_pre_mle const> (efi_pre->data.next()) };
        auto mtrr { pre_mle->mtrr() };
    ...
    for (; static_cast<void const *>(e) < l; e = e->get_next()) {
        ...
        switch (e->get_type()) {
            ...
            case Element::Type::ACPI_CEDT:
                std::start_lifetime_as<Acpi_table const> (e->get_data())->validate (e->get_data() - o);
                break;
...
void Txt::init()
{
    ...
    // TXT registers and heap have already been mapped by launch()
    auto const efi_pre { std::start_lifetime_as<Data_efi_pre const> (heap_base + heap_offs) };
    auto const pre_mle { std::start_lifetime_as<Data_pre_mle const> (efi_pre->data.next()) };
    auto const pre_acm { std::start_lifetime_as<Data_pre_acm const> (pre_mle->data.next()) };
    auto const acm_mle { std::start_lifetime_as<Data_acm_mle const> (pre_acm->data.next()) };
    ...
    std::start_lifetime_as<Acpi_table const> (dmar)->validate (dmar - heap_offs);
