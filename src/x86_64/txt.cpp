...
void Txt::restore()
{
    ...
    if (EXPECT_TRUE (launched)) {
        ...
        if (EXPECT_FALSE (!Mtrr::validate (vcnt, ~mask))) {
            ...
            // Disable MTRRs
            Msr::write (Msr::Register::IA32_MTRR_DEF_TYPE, Msr::read (Msr::Register::IA32_MTRR_DEF_TYPE) & ~BIT (11));
...
void Txt::init()
{
    ...
    auto const didvid { read (Space::PUBLIC, Register64::DIDVID) };
    auto const verqpi { read (Space::PUBLIC, Register32::VER_QPIIF) };
    ...
    if (!command (Register8::SECRETS_SET, Register64::E2STS, E2STS::SECRETS) || !command (Register8::LOCALITY1_OPEN, Register64::STS, STS::OPENED_LOCALITY1))
...
void Txt::fini()
{
    ...
    if (command (Register8::SECRETS_CLR, Register64::E2STS, E2STS::SECRETS) || command (Register8::LOCALITY1_CLOSE, Register64::STS, STS::OPENED_LOCALITY1))
        trace (TRACE_ERROR, "%s: TXT command failed", __func__);
