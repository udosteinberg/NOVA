...
bool Txt::check_acm (uint32_t sinit_base, uint32_t sinit_size, uint32_t fms, uint32_t &acm_size, uint32_t &acm_caps, uint32_t &tpm_caps)
{
    ...
    if (EXPECT_TRUE (pch) && EXPECT_FALSE (!reinterpret_cast<Acm::Id_pch const *>(pch->list())->match (pch->count, read (Space::PUBLIC, Register64::DIDVID))))
        return false;

    // ACM must support CPU
    auto const cpu { acm->cpu (info) };
    if (EXPECT_TRUE (cpu) && EXPECT_FALSE (!reinterpret_cast<Acm::Id_cpu const *>(cpu->list())->match (cpu->count, fms, Msr::read (Msr::Register::IA32_PLATFORM_ID))))
        return false;
...
bool Txt::init_heap (uint32_t heap_base, uint32_t /*heap_size*/, uint32_t acm_caps, uint32_t tpm_caps, unsigned vcnt)
{
    ...
    os_mle->msr_ia32_mtrr_def_type  = Msr::read (Msr::Register::IA32_MTRR_DEF_TYPE);
    os_mle->msr_ia32_misc_enable    = Msr::read (Msr::Register::IA32_MISC_ENABLE);
    os_mle->msr_ia32_debugctl       = Msr::read (Msr::Register::IA32_DEBUGCTL);
...
bool Txt::init_mtrr (uint64_t phys, uint64_t size, unsigned vcnt, unsigned bits)
{
    ...
    {   Cache_guard guard;
        ...
        Msr::write (Msr::Register::IA32_MTRR_DEF_TYPE, CA_TYPE_MEM_UC);
        ...
        Msr::write (Msr::Register::IA32_MTRR_DEF_TYPE, CA_TYPE_MEM_UC | BIT (11));
    }
...
void Txt::launch()
{
    ...
    auto const heap_base { read (Space::PUBLIC, Register32::HEAP_BASE) };
    auto const heap_size { read (Space::PUBLIC, Register32::HEAP_SIZE) };
    ...
    auto const sinit_base { read (Space::PUBLIC, Register32::SINIT_BASE) };
    auto const sinit_size { read (Space::PUBLIC, Register32::SINIT_SIZE) };
    ...
    auto const error { read (Space::PUBLIC, Register32::ERRORCODE) };
    if (EXPECT_FALSE (error & BIT (31)))
        if (!(error & BIT (30)) || error >> 4 & BIT_RANGE (5, 0))
            return;

    // Abort if any MCE banks report errors
    auto const mce { static_cast<uint8_t>(Msr::read (Msr::Register::IA32_MCG_CAP)) };
    for (unsigned i { 0 }; i < mce; i++)
        if (EXPECT_FALSE (Msr::read (Msr::Array::IA32_MC_STATUS, 4, i) & BIT64 (63)))
            return;

    // Abort if MCE is in progress
    if (EXPECT_FALSE (Msr::read (Msr::Register::IA32_MCG_STATUS) & BIT (2)))
        return;
...
void Txt::restore()
{
    ...
    // Fix up MTRR masks to work around firmware bugs (key bits must not be set)
    ...
    auto const bios { reinterpret_cast<Data_bios const *>(MMAP_GLB_TXTH + (read (Space::PUBLIC, Register32::HEAP_BASE) & Hpt::offs_mask (Hpt::bpl))) };
    auto const os_mle { reinterpret_cast<Data_os_mle const *>(bios->data.next()) };
    auto mtrr { os_mle->mtrr() };

    {   Cache_guard guard;

        // Restore variable MTRRs
        ...
        // Restore MSRs
        Msr::write (Msr::Register::IA32_MTRR_DEF_TYPE,  os_mle->msr_ia32_mtrr_def_type);
        Msr::write (Msr::Register::IA32_MISC_ENABLE,    os_mle->msr_ia32_misc_enable);
        Msr::write (Msr::Register::IA32_DEBUGCTL,       os_mle->msr_ia32_debugctl);
    }
...
void Txt::init()
{
    ...
    auto const dpr { read (Space::PUBLIC, Register32::DPR) };
    ...
    trace (TRACE_DRTM, "DRTM: TXT %#010x-%#010x", txt_base, txt_base + txt_size);
    trace (TRACE_DRTM, "DRTM: DPR %#010x-%#010x", dma_base, dma_base + dma_size);

    auto const heap_base { read (Space::PUBLIC, Register32::HEAP_BASE) };
    auto const heap_size { read (Space::PUBLIC, Register32::HEAP_SIZE) };
    auto const heap_offs { MMAP_GLB_TXTH - (heap_base & ~Hpt::offs_mask (Hpt::bpl)) };

    trace (TRACE_DRTM, "DRTM: %#x/%#x SINIT %#x/%#x HEAP", read (Space::PUBLIC, Register32::SINIT_BASE), read (Space::PUBLIC, Register32::SINIT_SIZE), heap_base, heap_size);
    ...
    write (Space::PRIVATE, Register32::ERRORCODE, 0);

    if (!command (Register8::LOCALITY1_OPEN, Register64::STS,   STS::OPENED_LOCALITY1)  ||
        !command (Register8::BASE_LOCK,      Register64::STS,   STS::LOCKED_BASE)       ||
        !command (Register8::SECRETS_SET,    Register64::E2STS, E2STS::SECRETS))
        trace (TRACE_ERROR, "%s: TXT command failed", __func__);

    // Wake RLPs
    write (Space::PUBLIC, Register32::MLE_JOIN, static_cast<uint32_t>(Kmem::ptr_to_phys (&Smx::mle_join)));
...
void Txt::fini()
{
    ...
    if (command (Register8::SECRETS_CLR,   Register64::E2STS, E2STS::SECRETS)       ||
        command (Register8::MEMCFG_UNLOCK, Register64::STS,   STS::LOCKED_MEMCFG)   ||
        command (Register8::PRIVATE_CLOSE, Register64::STS,   STS::OPENED_PRIVATE))
        trace (TRACE_ERROR, "%s: TXT command failed", __func__);
