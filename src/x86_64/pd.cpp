...
bool Pd::update_space_obj (Pd *pd, mword src, mword dst, unsigned ord, unsigned pmm)
{
    ...
    for (mword s_sel = src, d_sel = dst; s_sel < s_end; s_sel++, d_sel++) {
        ...
        Capability cap = pd->Space_obj::lookup (s_sel), old (0);
...
bool Pd::update_space_pio (Pd *pd, mword src, mword dst, unsigned ord, unsigned pmm)
{
    mword const end = src + (1UL << ord);

    if (EXPECT_FALSE (end > Space_pio::num || src != dst))
        return false;

    for (mword sel = src; sel < end; sel++) {

        auto acc = pd->Space_pio::lookup (sel);

        acc &= pmm & BIT (0);

        Space_pio::update (sel, acc);
    }
