...
bool Ec_arch::handle_exc_pf (Exc_regs *r)
{
    ...
    if (r->err & BIT (2))       // User-mode access
        return pfa < Space_hst::user_boundary() && hst->loc[Cpu::id].share_from (hst->hptp, pfa, Space_hst::user_boundary());
...
void Ec_arch::handle_exc (Exc_regs *r)
{
    ...
    panic ("Fatal exception %#lx (%#lx) at CS:%#lx IP:%#lx", r->vec, r->err, r->cs, r->rip);
}
