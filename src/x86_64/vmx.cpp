...
void Vmcs::init (uintptr_t rsp, uintptr_t bmp, uintptr_t cr3, uint64 eptp, uint16 vpid)
{
    make_current();
    ...
    write (Encoding::EPTP, eptp | (Ept::lev - 1) << 3 | 6);

    write (Encoding::BITMAP_IO_A, bmp);
    write (Encoding::BITMAP_IO_B, bmp + PAGE_SIZE);
    ...
    write (Encoding::HOST_SYSENTER_CS,  SEL_KERN_CODE);
    write (Encoding::HOST_SYSENTER_ESP, reinterpret_cast<uintptr_t>(&Tss::run.sp0));
    write (Encoding::HOST_SYSENTER_EIP, reinterpret_cast<uintptr_t>(&entry_sysenter));
...
void Vmcs::init()
{
    ...
    if (has_ept())
        Ept::set_lps (static_cast<unsigned>(bit_scan_reverse (ept_vpid.super) + 2));
    if (has_mbec())
        Ept::mbec = true;
    if (has_urg())
