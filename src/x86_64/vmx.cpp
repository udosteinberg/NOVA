...
Vmcs::Vmcs (mword esp, mword bmp, mword cr3, uint64 eptp, uint16 vpid) : rev (basic.revision)
{
    ...
    write (VPID, vpid);
    write (EPTP, eptp | (Eptp::lev - 1) << 3 | 6);
    ...
    write (HOST_EFER, Msr::read (Msr::Register::IA32_EFER));
...
void Vmcs::init()
{
    ...
    fix_cr0_set = static_cast<mword>( Msr::read (Msr::Register::IA32_VMX_CR0_FIXED0));
    fix_cr0_clr = static_cast<mword>(~Msr::read (Msr::Register::IA32_VMX_CR0_FIXED1));
    fix_cr4_set = static_cast<mword>( Msr::read (Msr::Register::IA32_VMX_CR4_FIXED0));
    fix_cr4_clr = static_cast<mword>(~Msr::read (Msr::Register::IA32_VMX_CR4_FIXED1));

    fix_cr0_clr |= CR0_CD | CR0_NW;

    basic.val       = Msr::read (Msr::Register::IA32_VMX_BASIC);
    ctrl_exi.val    = Msr::read (basic.ctrl ? Msr::Register::IA32_VMX_TRUE_EXI : Msr::Register::IA32_VMX_CTRL_EXI_PRI);
    ctrl_ent.val    = Msr::read (basic.ctrl ? Msr::Register::IA32_VMX_TRUE_ENT : Msr::Register::IA32_VMX_CTRL_ENT);
    ctrl_pin.val    = Msr::read (basic.ctrl ? Msr::Register::IA32_VMX_TRUE_PIN : Msr::Register::IA32_VMX_CTRL_PIN);
    ctrl_cpu[0].val = Msr::read (basic.ctrl ? Msr::Register::IA32_VMX_TRUE_CPU : Msr::Register::IA32_VMX_CTRL_CPU_PRI);

    if (has_secondary())
        ctrl_cpu[1].val = Msr::read (Msr::Register::IA32_VMX_CTRL_CPU_SEC);
    if (has_ept() || has_vpid())
        ept_vpid.val = Msr::read (Msr::Register::IA32_VMX_EPT_VPID);
    if (has_ept())
