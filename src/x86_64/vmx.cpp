...
Vmcs::Vmcs (mword esp, mword bmp, mword cr3, uint64 eptp, uint16 vpid) : rev (basic.revision)
{
    ...
    write (HOST_SEL_CS, static_cast<uint16>(SEL_KERN_CODE));
    write (HOST_SEL_SS, static_cast<uint16>(SEL_KERN_DATA));
    write (HOST_SEL_DS, static_cast<uint16>(SEL_KERN_DATA));
