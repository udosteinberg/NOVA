...
void Acpi_table_dmar::Remapping_rmrr::parse() const
{
    for (uint64 hpa = base & ~OFFS_MASK; hpa < limit; hpa += PAGE_SIZE)
        Pd_kern::nova().Space_mem::update (hpa, hpa, 0, Paging::Permissions (Paging::W | Paging::R), Memattr::Cacheability::MEM_WB, Memattr::Shareability::NONE, Space::Index::DMA_HST);
