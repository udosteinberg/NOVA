...
{
    Space_dma::user_access (align_dn (base, PAGE_SIZE), align_up (limit, PAGE_SIZE) - align_dn (base, PAGE_SIZE), true);
    ...
    for (auto a = addr + sizeof (*this); a < addr + length; ) {
        ...
        if (smmu)
            smmu->configure (&Space_dma::nova, s->dev(), false);
...
void Acpi_table_dmar::parse() const
{
    ...
    Smmu::ir = flags & Flags::INTR_REMAPPING;
