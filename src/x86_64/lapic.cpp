...
void Lapic::init()
{
    ...
    // Reserve MMIO region
    Pd_kern::remove_user_mem (apic_base & ~PAGE_MASK, PAGE_SIZE);
    ...
    if ((Cpu::bsp = apic_base & BIT (8))) {
        ...
        memcpy (Hpt::map (0x1000, true), reinterpret_cast<void *>(Kmem::sym_to_virt (&__init_aps)), &__desc_gdt__ - &__init_aps);
        ...
        auto t1 = static_cast<uint32>(__builtin_ia32_rdtsc());
        Acpi_fixed::delay (10);
        auto v2 = read (Register32::TMR_CCR);
        auto t2 = static_cast<uint32>(__builtin_ia32_rdtsc());
