...
Space_hst::Space_hst() : Space_mem { Kobject::Subtype::HST }, pcid { 0 }
{
    ...
    (void) Hptp::master.walk (MMAP_GLB_CPUS, 1, m, inv);

    // Mappable bounds: constrained by physical addressing width and user boundary
    auto const max_addr { min (BITN (Memattr::obits), user_boundary()) };
