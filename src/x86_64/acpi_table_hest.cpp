...
bool Acpi_table_hest::parse() const
{
    ...
    while (ptr + sizeof (list_t) <= end) {
        ...
        switch (s->type()) {
            case Source::Type::MCE:     ptr += std::start_lifetime_as<Source_mce    const> (s)->parse(); break;
            case Source::Type::CMC:     ptr += std::start_lifetime_as<Source_cmc    const> (s)->parse(); break;
            ...
            case Source::Type::DMC:     ptr += std::start_lifetime_as<Source_dmc    const> (s)->parse(); break;
