...
cpu_t Cpu::id;
unsigned Cpu::hazard;
uint64_t Cpu::phys_local, Cpu::csthint;
uint32_t Cpu::topology, Cpu::features[13], Cpu::upatch, Cpu::socket, Cpu::cstates;
...
void Cpu::enumerate_topology (uint32_t leaf, uint32_t &t, uint32_t &s, uint32_t (&lvl)[4])
{
    uint32_t c { 0 }, n, eax, ebx, ecx, edx;

    // Bounded against a processor reporting more domains than architecturally defined
    for (unsigned l { 0 }; l < 8; l++, c = n) {

        cpuid (leaf, l, eax, ebx, ecx, edx);

        // Enumeration ends at the first invalid domain
        if (!(ebx & BIT_RANGE (15, 0))) [[unlikely]]
            break;

        // x2APIC ID
        t = edx;

        // Next-level shift count
        n = eax & BIT_RANGE (4, 0);

        // Extract bits [n-1:c] = all bits between next and current bit position
        if (l < sizeof (lvl) / sizeof (*lvl))
            lvl[l] = (t & (BIT (n) - 1)) >> c;
    }

    // Extract bits [max:c] = all remaining bits as package/socket
    s = t >> c;
}
...
void Cpu::enumerate_features (uint32_t &clk, uint32_t &rat, uint32_t (&lvl)[4], uint32_t (&name)[12])
{
    ...
    switch (static_cast<uint8_t>(eax)) {
        default:
            enumerate_topology (0x1f, topology, socket, lvl);
            [[fallthrough]];
        case 0x1a ... 0x1e:
        ...
        case 0xb ... 0xc:
            if (topology == invalid_topology)
                enumerate_topology (0xb, topology, socket, lvl);
            [[fallthrough]];
        case 0x7 ... 0xa:
        ...
        case 0x1 ... 0x3:
            ...
            stepping =  eax & 0xf;
            Cache::reduce (ebx >> 5 & BIT_RANGE (10, 3));   // Accounts for the 8x multiplier
            if (topology == invalid_topology) {
                topology = ebx >> 24;
    ...
    // Intel: PATCH_SIGN_ID=[63:32], AMD: PatchLevel=[31:0]
    upatch = static_cast<uint32_t>(Msr::read (Msr::Reg64::IA32_BIOS_SIGN_ID) >> (vendor == Vendor::INTEL ? 32 : 0));
...
void Cpu::setup_cst()
{
    ...
    trace (TRACE_PWR, "CCST:%s%s%s%s%s%s%s (%#lx:%#lx)",
...
void Cpu::init()
{
    ...
    if (!Acpi::resume) {
        ...
        Space_hst::nova.loc[id].lookup (MMAP_CPU_DATA, phys_local, o, ma);
    ...
    trace (TRACE_CPU, "CORE: %02u:%03u.%u %s%x:%x:%x:%x [%#x] %.48s", lvl[2], lvl[1], lvl[0], t, family, model, stepping, platform, upatch, reinterpret_cast<char const *>(name));
