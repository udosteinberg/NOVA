...
uint32_t Cpu::features[8], Cpu::topology;
...
void Cpu::enumerate_features (uint32_t &clk, uint32_t &rat, uint32_t (&lvl)[4], uint32_t (&name)[12])
{
    ...
    switch (static_cast<uint8_t>(eax)) {
        ...
        case 0xd ... 0xf:
            ...
            Fpu::hst_xsv.xcr = Fpu::desired & (static_cast<uint64_t>(edx) << 32 | eax);
            cpuid (0xd, 1, eax, ebx, ecx, edx);
            Fpu::hst_xsv.xss = Fpu::desired & (static_cast<uint64_t>(edx) << 32 | ecx);
            [[fallthrough]];
        case 0xb ... 0xc:
...
void Cpu::setup_pstate()
{
    ...
    if (feature (Feature::HWP)) {
        ...
        trace (TRACE_CPU, "HWPS: Level:%u (%u:%u:%u)", des, min, gtd, max);
        ...
        auto const max { feature (Feature::TURBO_BOOST) ? static_cast<uint8_t>(Msr::read (Msr::Register::TURBO_RATIO_LIMIT)) : gtd };
        auto const des { Cmdline::noturbo ? gtd : max };
