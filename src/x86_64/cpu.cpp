...
uint32_t Cpu::topology, Cpu::features[13], Cpu::upatch, Cpu::cstates;
...
void Cpu::enumerate_topology (uint32_t leaf, uint32_t &topology, uint32_t (&lvl)[4])
{
    ...
    for (unsigned l { 0 }, c { 0 }, n; l < sizeof (lvl) / sizeof (*lvl); l++, c = n) {

        cpuid (leaf, l, eax, ebx, ecx, edx);

        // Extract bits [n-1:c] = all bits between next and current bit position
        if (ebx & BIT_RANGE (15, 0)) [[likely]] {
            lvl[l] = ((topology = edx) & (BIT (n = eax & BIT_RANGE (4, 0)) - 1)) >> c;
            continue;
        }

        // Extract bits [max:c] = all bits remaining (if valid leaf)
        if (l) [[likely]]
            lvl[l] = edx >> c;
