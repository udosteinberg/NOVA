...
bool Cpu::bsp;
unsigned Cpu::id, Cpu::hazard, Cpu::platform, Cpu::family, Cpu::model, Cpu::stepping, Cpu::patch;
uint32 Cpu::features[8], Cpu::topology;
...
void Cpu::enumerate_features (uint32 &clk, uint32 &rat, uint32 (&name)[12], unsigned &package, unsigned &core, unsigned &thread)
{
    ...
    switch (static_cast<uint8>(eax)) {
        ...
        case 0xd ... 0xf:
            ...
            Fpu::hst_xsv.xcr = Fpu::desired & (static_cast<uint64>(edx) << 32 | eax);
            cpuid (0xd, 1, eax, ebx, ecx, edx);
            Fpu::hst_xsv.xss = Fpu::desired & (static_cast<uint64>(edx) << 32 | ecx);
            [[fallthrough]];
        case 0x7 ... 0xc:
...
void Cpu::setup_msr()
{
    ...
    if (EXPECT_TRUE (feature (Feature::RDPID) || feature (Feature::RDTSCP)))
        Msr::write (Msr::Register::IA32_TSC_AUX, hst_tsc.tsc_aux = Cpu::id);

    if (EXPECT_TRUE (feature (Feature::TME)) && bsp) {

        trace (TRACE_CPU, "TMEE: Split:%u/%u Keys:%u", Memattr::kbits, Memattr::obits, Memattr::kimax);

        // FIXME: Check for valid PCONFIG targets
        if (EXPECT_TRUE (feature (Feature::PCONFIG)))
            for (uint16 i { 0 }; i < Memattr::kimax; i++)
                (Cmdline::nomktme ? Pconfig::key_clr : Pconfig::key_rnd)(i + 1);
    }
...
void Cpu::setup_pstate()
{
    ...
    if (feature (Feature::HWP)) {
        ...
        auto const msr { Msr::read (Msr::Register::IA32_HWP_CAPABILITIES) };
        auto const min { static_cast<uint8>(msr >> 24) };
        auto const nom { static_cast<uint8>(msr >>  8) };
        auto const max { static_cast<uint8>(msr) };
        auto const des { Cmdline::noturbo ? nom : max };
        ...
        auto const msr { Msr::read (Msr::Register::PLATFORM_INFO) };
        auto const min { static_cast<uint8>(msr >> 48) };
        auto const nom { static_cast<uint8>(msr >>  8) };
        auto const max { feature (Feature::TURBO_BOOST) ? static_cast<uint8>(Msr::read (Msr::Register::TURBO_RATIO_LIMIT)) : nom };
        auto const des { Cmdline::noturbo ? nom : max };
