...
bool Tpm::invoke (Locality l, Cmd &&cmd, Hmac_session *session)
{
    ...
    *static_cast<Tpm_st *>(&cmd) = Tpm_st { session ? Tpm_st::Type::SESSIONS : Tpm_st::Type::NO_SESSIONS };
    ...
    if (execute (l, cmd)) [[likely]] {
        ...
        auto const res { std::start_lifetime_as<Res const> (buffer) };
...
bool Tpm::start_auth_session (Locality l, Hmac_session &s)
{
    ...
    // Handle + Nonce
    auto const h { std::start_lifetime_as<Tpm_handle const> (buffer + sizeof (Res)) };
    auto const n { std::start_lifetime_as<Tpm2b_digest<0> const> (h + 1) };
...
bool Tpm::cap_pcrs (Locality l)
{
    ...
    auto const tpms { std::start_lifetime_as<Tpms_capability_data const> (buffer + sizeof (Res) + sizeof (bool)) };
