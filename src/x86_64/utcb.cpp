...
bool Utcb::load_exc (Cpu_regs const &c)
{
    ...
    Barrier::sw();
    mtd = m;
    items = sizeof (Utcb_data) / sizeof (uintptr_t);
...
bool Utcb::load_vmx (Cpu_regs const &c)
{
    ...
    if (m & Mtd_arch::RFLAGS)
        rflags = Vmcs::read<Vmcs::Encoding::GUEST_RFLAGS>();
    ...
    if (m & Mtd_arch::SYSENTER) {
        ...
        sysenter_rsp = Vmcs::read<Vmcs::Encoding::GUEST_SYSENTER_ESP>();
        sysenter_rip = Vmcs::read<Vmcs::Encoding::GUEST_SYSENTER_EIP>();
    }

    if (m & Mtd_arch::QUAL) {
        qual[0] = Vmcs::read<Vmcs::Encoding::EXI_QUALIFICATION>();
        qual[1] = Vmcs::read<Vmcs::Encoding::GUEST_PHYSICAL_ADDRESS>();
    }

    if (m & Mtd_arch::INJ) {
        if (c.exc.ep() == 33 || c.exc.ep() == NUM_VMI - 1) {
            intr_info  = Vmcs::read<Vmcs::Encoding::INJ_EVENT_IDENT>();
            intr_error = Vmcs::read<Vmcs::Encoding::INJ_EVENT_ERROR>();
        } else {
            intr_info  = Vmcs::read<Vmcs::Encoding::ORG_EVENT_IDENT>();
            intr_error = Vmcs::read<Vmcs::Encoding::ORG_EVENT_ERROR>();
        }
    }
