...
bool Acpi_table_madt::parse() const
{
    ...
    while (ptr + sizeof (list_t) <= end) {
        ...
        switch (s->type()) {
            case Controller::Type::LAPIC:  static_cast<Controller_lapic  const *>(s)->parse(); break;
            case Controller::Type::IOAPIC: static_cast<Controller_ioapic const *>(s)->parse(); break;
            case Controller::Type::X2APIC: static_cast<Controller_x2apic const *>(s)->parse(); break;
            default: break;
        }
    }
