...
bool Acpi_table_fadt::parse() const
{
    Acpi::facs = facs64 ? facs64 : facs32;
    Acpi::fflg = fflg;

    trace (TRACE_FIRM, "ACPI: Version %u.%u Profile %u Features %#x Boot %#x", uint8_t { table.revision }, minor_errata & BIT_RANGE (3, 0), uint8_t { pm_profile }, uint32_t { fflg }, uint16_t { bflg_x86 });
    ...
    if (fflg & BIT (20)) {
        ...
        // Verify that the sleep registers are within the FADT bounds
        if (table.header.length < __builtin_offsetof (Acpi_table_fadt, sleep_sts) + sizeof (sleep_sts)) [[unlikely]]
            return false;
