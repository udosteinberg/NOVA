...
void Acpi_table_fadt::parse() const
{
    ...
    for (unsigned i = 0; i < sizeof (regs) / sizeof (*regs); i++) {
        ...
        if (regs[i].res)
            switch (regs[i].reg.asid) {
             // case Acpi_gas::Asid::MMIO:  Space_hst::user_access (regs[i].reg.addr, regs[i].reg.bits / 8, false); break;
                case Acpi_gas::Asid::PIO:   Space_pio::user_access (regs[i].reg.addr, regs[i].reg.bits / 8, false); break;
                default: break;
            }
    }
    ...
    if (smi_cmd) {
        ...
        if (!Cmdline::insecure)
            Space_pio::user_access (smi_cmd, 1, false);
    }
}
