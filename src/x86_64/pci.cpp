...
void Pci::Function::enumerate_pcap()
{
    ...
    // Because capabilities are 32-bit aligned, bits[1:0] of the 8-bit pointer must be masked
...
void Pci::Function::enumerate_ecap()
{
    ...
    // Because capabilities are 32-bit aligned, bits[1:0] of the 12-bit pointer must be masked
...
uint8_t Pci::init_bus (uint16_t const seg, uint8_t const bus, uint8_t const ebn, uint8_t const lev)
{
    ...
    for (unsigned i { 0 }; i < 256; i++) {
        ...
        auto const didvid { *reinterpret_cast<uint32_t volatile *>(ecam_addr (sbdf)) };
