...
bool Hptp::share_from (Hptp src, IAddr v, IAddr o)
{
    ...
    auto const s { src.walk (v, l, m = false, i = false) };
    if (!s) [[unlikely]]
        return false;

    // Walk the dst page table to the respective slot at that level (allocating)
    auto const d { walk (v, l, m = true, i = false) };
    if (!d) [[unlikely]]
        return false;

    // Atomically read both PTEs
    auto const spte { s->load() };
    auto const dpte { d->load() };
...
void *Hptp::map (uintptr_t v, OAddr p, Paging::Permissions pm, Memattr ma, unsigned n)
{
    ...
    bool m { false }, i { false };

    // No PTAB allocation because called from pre-launch code
    auto pte { current().walk (v, 1, m, i) };

    auto const r { v | (p & o) };

    for (p = (p & ~o) | Hpt::attr_leaf (1, pm, ma); n--; invalidate (v), pte++, v += s, p += s)
        pte->store (p);
