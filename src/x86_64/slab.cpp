...
struct Slab_cache::Slab
{
    ...
    struct Metadata
    {
         ...
         */
        ALWAYS_INLINE
        Metadata (Slab_cache *c) : cache (c) {}
    };

    Metadata    meta;
    uint8_t     data[PAGE_SIZE - sizeof (meta)];
    ...
    {
        return reinterpret_cast<Slab *>(reinterpret_cast<uintptr_t>(p) & ~OFFS_MASK);
    }
