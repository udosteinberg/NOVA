...
Smmu::Smmu (uint64_t p) : List (list), phys_base (p), mmio_base (mmap), invq (static_cast<Inv_dsc *>(Buddy::alloc (ord, Buddy::Fill::BITS0)))
{
    ...
    Lapic::x2apic &= has_eim();

    auto const ver { read (Register32::VER) };

    trace (TRACE_SMMU, "SMMU: %#010lx %u.%u CAP:%#018lx ECAP:%#018lx C:%u I:%u", phys_base, ver >> 4 & BIT_RANGE (3, 0), ver & BIT_RANGE (3, 0), cap, ecap, has_co(), has_ir());
}

void Smmu::init()
{
    write (Register32::FEADDR, 0xfee00000 | Lapic::id[0] << 12);
...
bool Smmu::configure (Space_dma *dma, uintptr_t dad, bool inv)
{
    ...
    trace (TRACE_SMMU, "SMMU: %#010lx Device %02x:%02x.%x assigned to Domain %u", phys_base, src >> 8, src >> 3 & BIT_RANGE (4, 0), src & BIT_RANGE (2, 0), static_cast<unsigned>(sdid));
