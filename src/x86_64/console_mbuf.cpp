...
Console_mbuf::Console_mbuf() : regs { new Console_mbuf_mmio }
{
    ...
    Status s;
    if (!(sm = Pd::nova.create_sm (s, &Space_obj::nova, Space_obj::Selector::NOVA_CON, Kobject::Subtype::SM_REG, 0))) [[unlikely]]
        return;
...
bool Console_mbuf::outc (char c)
{
    ...
    // Publish new data
    regs->buffer[wc] = c;                   // (1) ST.RELAXED

    // Publish new write index
    regs->w_idx = wn;                       // (2) ST.RELEASE

    // Signal reader at line end
    if (c == '\n') [[unlikely]]
        sm->up();                           // (3) ST.RELEASE via sm->lock->unlock()
