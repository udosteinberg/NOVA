...
uint8  Cos::config { 0 }, Cos::supcfg { 0 }, Cos::cos_l3 { 0 }, Cos::cos_l2 { 0 }, Cos::cos_mb { 0 }, Cos::hcb_l3 { 0 }, Cos::hcb_l2 { 0 };
uint16 Cos::del_mb { 0 }, Cos::current { 0 };
...
void Cos::init()
{
    ...
    for (uint8 i = 0; i < cos_l3; i++)
        set_l3_mask (i, BIT_RANGE (hcb_l3, 0));

    for (uint8 i = 0; i < cos_l2; i++)
        set_l2_mask (i, BIT_RANGE (hcb_l2, 0));

    for (uint8 i = 0; i < cos_mb; i++)
        set_mb_thrt (i, 0);
