...
bool Acpi::init()
{
    if (!resume) {
        ...
        if (static_cast<Acpi_table_rsdp *>(Hptp::map (Hptp::Remap::MAP0, rsdp))->parse (rsdt, size) == false)
            return false;

        if (rsdt)
            static_cast<Acpi_table_rsdt *>(Hptp::map (Hptp::Remap::MAP0, rsdt))->parse (rsdt, size);
        if (fadt)
            static_cast<Acpi_table_fadt *>(Hptp::map (Hptp::Remap::MAP0, fadt))->parse();
        if (facs)
            static_cast<Acpi_table_facs *>(Hptp::map (Hptp::Remap::MAP0, facs))->parse();
        if (dbg2)
            static_cast<Acpi_table_dbg2 *>(Hptp::map (Hptp::Remap::MAP0, dbg2))->parse();
        if (spcr)
            static_cast<Acpi_table_spcr *>(Hptp::map (Hptp::Remap::MAP0, spcr))->parse();
        if (madt)
