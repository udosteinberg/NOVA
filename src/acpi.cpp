...
bool Acpi::init()
{
    if (!resume) {
        ...
        if (!ptr || !std::start_lifetime_as<Acpi_table_rsdp const> (ptr)->parse())
            return false;

        bool ret { true };

        if ((ptr = Acpi_table::map (xsdt, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_xsdt const> (ptr)->parse();
        if ((ptr = Acpi_table::map (fadt, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_fadt const> (ptr)->parse();
        if ((ptr = Acpi_table::map (facs, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_facs const> (ptr)->parse();
        if ((ptr = Acpi_table::map (spcr, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_spcr const> (ptr)->parse();
        if ((ptr = Acpi_table::map (dbg2, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_dbg2 const> (ptr)->parse();
        if ((ptr = Acpi_table::map (madt, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_madt const> (ptr)->parse();
        if ((ptr = Acpi_table::map (mcfg, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_mcfg const> (ptr)->parse();
        if ((ptr = Acpi_table::map (srat, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_srat const> (ptr)->parse();
        if ((ptr = Acpi_table::map (hest, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_hest const> (ptr)->parse();
        if ((ptr = Acpi_table::map (tpm2, 0))) [[likely]]
            ret &= std::start_lifetime_as<Acpi_table_tpm2 const> (ptr)->parse();

        // Parse architecture-specific tables
        parse_tables (ret);

        if (!ret) [[unlikely]]
            trace (TRACE_ERROR, "ACPI: Errors during table parsing!");
