...
class Kmem final
{
    ...
    public:
        NONNULL static auto sym_to_virt (void const *p) { return reinterpret_cast<uintptr_t>(p) + OFFSET; }
        NONNULL static auto sym_to_phys (void const *p) { return virt_to_phys (sym_to_virt (p)); }
        NONNULL static auto ptr_to_phys (void const *p) { return virt_to_phys (reinterpret_cast<uintptr_t>(p)); }
        ...
        template <typename T>
        NONNULL static auto loc_to_glob (unsigned cpu, T *p)
        {
