...
class Gich final
{
    ...
    private:
        ...
        static inline auto read  (Register32 r)               { return *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICH + std::to_underlying (r)); }
        static inline auto read  (Array32 r, unsigned n)      { return *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICH + std::to_underlying (r) + n * sizeof (uint32_t)); }

        static inline void write (Register32 r, uint32_t v)          { *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICH + std::to_underlying (r)) = v; }
        static inline void write (Array32 r, unsigned n, uint32_t v) { *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICH + std::to_underlying (r) + n * sizeof (uint32_t)) = v; }
        ...
        static void load (uint64_t const (&lr)[16], uint32_t const (&ap0r)[4], uint32_t const (&ap1r)[4], uint32_t const &hcr, uint32_t const &vmcr)
        {
            if (Gicc::mode == Gicc::Mode::REGS) {
                ...
                for (unsigned i { 0 }; i < num_lr; i++)
                    write (Array32::LR, i, static_cast<uint32_t>(lr[i]));
