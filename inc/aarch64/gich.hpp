...
class Gich final
{
    ...
    private:
        ...
        static auto read  (Reg32 r)                  { return *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_GICH + std::to_underlying (r)); }
        static auto read  (Arr32 r, unsigned n)      { return *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_GICH + std::to_underlying (r) + n * sizeof (uint32_t)); }

        static void write (Reg32 r, uint32_t v)             { *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_GICH + std::to_underlying (r)) = v; }
        static void write (Arr32 r, unsigned n, uint32_t v) { *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_GICH + std::to_underlying (r) + n * sizeof (uint32_t)) = v; }
    ...
    public:
        static constexpr unsigned max_apr { 4 };
        static constexpr unsigned max_lr { 16 };
        ...
        static void load (uint64_t const (&lr)[max_lr], uint32_t const (&ap0r)[max_apr], uint32_t const (&ap1r)[max_apr], uint32_t const &hcr, uint32_t const &vmcr)
        {
            if (Gicc::mode == Gicc::Mode::REGS) {
        ...
        static void save (uint64_t (&lr)[max_lr], uint32_t (&ap0r)[max_apr], uint32_t (&ap1r)[max_apr], uint32_t &hcr, uint32_t &vmcr, uint32_t &elrsr)
        {
            if (Gicc::mode == Gicc::Mode::REGS) {
