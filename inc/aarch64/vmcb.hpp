...
{
    public:
        static uint64 const hcr_hyp0 =  HCR_ATA         |   // Trap GCR, RGSR, TFSR*
        ...
        static uint64 const hcr_hyp1 =  HCR_TID5        |   // Trap GMID
        ...
        static uint64 const cptr_hyp0 = 0;

        static uint64 const cptr_hyp1 = CPTR_TAM        |   // Trap activity monitor: AMCFGR, AMCGCR, AMCNTEN*, AMCR, AMEVCNTR*, AMEVTYPER*, AMUSERENR
                                        CPTR_TTA        |   // Trap trace registers
                                        CPTR_TZ;            // Trap ZCR

        static uint64 const mdcr_hyp0 = MDCR_E2PB;          // Trap profiling buffer control: PMB*

        static uint64 const mdcr_hyp1 = MDCR_TTRF       |   // Trap trace filter: TRFCR
        ...
        ALWAYS_INLINE
        static inline void *operator new (size_t) { return Buddy::allocator.alloc (0); }
