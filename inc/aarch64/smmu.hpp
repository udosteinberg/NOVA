...
class Smmu : public List<Smmu>, protected Mmio
{
    private:
        ...
        virtual void tlb_invalidate (Sdid) = 0;
        virtual bool is_using_spi (unsigned) const = 0;
        virtual uint8_t num_smg() const = 0;
        virtual uint8_t num_ctx() const = 0;

    protected:
        explicit Smmu (uint64_t p, size_t s) : List { list }, Mmio { p, s } {}

    public:
        virtual Status assign_dev (Space_dma *, uintptr_t) = 0;
        ...
        static bool using_spi (unsigned spi)
        {
            for (auto smmu { list }; smmu; smmu = smmu->next)
                if (smmu->is_using_spi (spi))
                    return true;

            return false;
        }

        static void interrupt (unsigned spi)
        {
            for (auto smmu { list }; smmu; smmu = smmu->next)
                if (smmu->is_using_spi (spi))
                    smmu->fault();
