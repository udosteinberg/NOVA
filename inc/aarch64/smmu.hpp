...
class Smmu : public List<Smmu>
{
    private:
        ...
        Mode                mode;           // SMMU Mode
        Board::Smmu const & config;         // SMMU Configuration
        ...
        static inline uintptr_t mmap    { DEV_GLOBL_SMMU }; // SMMU Memory Map Pointer
        ...
        inline bool glb_spi (unsigned spi) const
        {
            for (unsigned i = 0; i < sizeof (config.glb) / sizeof (*config.glb); i++)
                if (config.glb[i].flg && config.glb[i].spi == spi)
                    return true;
        ...
        inline bool ctx_spi (unsigned spi) const
        {
            for (unsigned i = 0; i < sizeof (config.ctx) / sizeof (*config.ctx); i++)
                if (config.ctx[i].flg && config.ctx[i].spi == spi)
                    return true;
    ...
    public:
        static inline unsigned num_smg  { 0 };      // Number of Stream Mapping Groups
        static inline unsigned num_ctx  { 0 };      // Number of Translation Context Banks
