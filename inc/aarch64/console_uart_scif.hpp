...
class Console_uart_scif final : private Console_uart
{
    private:
        ...
        auto read  (Register8  r) const      { return *reinterpret_cast<uint8_t  volatile *>(mmap + std::to_underlying (r)); }
        auto read  (Register16 r) const      { return *reinterpret_cast<uint16_t volatile *>(mmap + std::to_underlying (r)); }
        void write (Register8  r, uint8_t  v) const { *reinterpret_cast<uint8_t  volatile *>(mmap + std::to_underlying (r)) = v; }
        void write (Register16 r, uint16_t v) const { *reinterpret_cast<uint16_t volatile *>(mmap + std::to_underlying (r)) = v; }

        bool tx_busy() const override final { return !(read (Register16::SCFSR) & BIT (6)); }
        bool tx_full() const override final { return !(read (Register16::SCFSR) & BIT (5)); }

        inline void tx (uint8_t c) const override final
        {
            write (Register8::SCFTDR, c);
            write (Register16::SCFSR, uint16_t (~BIT_RANGE (6, 5)) & read (Register16::SCFSR));
        }
