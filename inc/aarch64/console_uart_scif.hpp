...
class Console_uart_scif final : private Console_uart
{
    private:
        ...
        auto read  (Reg8  r) const      { return *std::start_lifetime_as<uint8_t  volatile> (mmap + std::to_underlying (r)); }
        auto read  (Reg16 r) const      { return *std::start_lifetime_as<uint16_t volatile> (mmap + std::to_underlying (r)); }
        void write (Reg8  r, uint8_t  v) const { *std::start_lifetime_as<uint8_t  volatile> (mmap + std::to_underlying (r)) = v; }
        void write (Reg16 r, uint16_t v) const { *std::start_lifetime_as<uint16_t volatile> (mmap + std::to_underlying (r)) = v; }
