    ...
    protected:
        ...
        static void parse_tables (bool &ret)
        {
            ...
            if ((ptr = Acpi_table::map (gtdt, 0)))
                ret &= std::start_lifetime_as<Acpi_table_gtdt const> (ptr)->parse();
            if ((ptr = Acpi_table::map (iort, 0)))
                ret &= std::start_lifetime_as<Acpi_table_iort const> (ptr)->parse();
        }

        static void wake_prepare() {}

        static inline constinit uint64_t dbg2 {}, facs {}, fadt {}, gtdt {}, hest {}, iort {}, madt {}, mcfg {}, spcr {}, srat {}, tpm2 {}, xsdt {};
