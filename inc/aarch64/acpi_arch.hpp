...
class Acpi_arch
{
    protected:
        ...
        {
            if (gtdt)
                static_cast<Acpi_table_gtdt *>(Hptp::map (Hptp::Remap::MAP0, gtdt))->parse();
            if (iort)
                static_cast<Acpi_table_iort *>(Hptp::map (Hptp::Remap::MAP0, iort))->parse();
        }

        static void wake_prepare() {}

        static inline uint64_t dbg2 { 0 }, facs { 0 }, fadt { 0 }, gtdt { 0 }, iort { 0 }, madt { 0 }, spcr { 0 };
