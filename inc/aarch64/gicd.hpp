...
class Gicd final : private Coresight, private Intid
{
    ...
    private:
        ...
        static inline auto read  (Reg32 r)                  { return *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICD + std::to_underlying (r)); }
        static inline auto read  (Arr32 r, unsigned n)      { return *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICD + std::to_underlying (r) + n * sizeof (uint32_t)); }
        static inline auto read  (Arr64 r, unsigned n)      { return *reinterpret_cast<uint64_t volatile *>(MMAP_GLB_GICD + std::to_underlying (r) + n * sizeof (uint64_t)); }

        static inline void write (Reg32 r,             uint32_t v) { *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICD + std::to_underlying (r)) = v; }
        static inline void write (Arr32 r, unsigned n, uint32_t v) { *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_GICD + std::to_underlying (r) + n * sizeof (uint32_t)) = v; }
        static inline void write (Arr64 r, unsigned n, uint64_t v) { *reinterpret_cast<uint64_t volatile *>(MMAP_GLB_GICD + std::to_underlying (r) + n * sizeof (uint64_t)) = v; }
