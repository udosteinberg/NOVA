...
class Gicd : private Coresight, private Intid
{
    ...
    private:
        ...
        static inline uint64    phys    { Board::gic[0].mmio };
        static inline uint8     ifid[8] { 0 };
        static inline Spinlock  lock;

        static inline auto read  (Register32 r)             { return *reinterpret_cast<uint32 volatile *>(MMAP_GLB_GICD + static_cast<unsigned>(r)); }
        static inline auto read  (Array32 r, unsigned n)    { return *reinterpret_cast<uint32 volatile *>(MMAP_GLB_GICD + static_cast<unsigned>(r) + n * sizeof (uint32)); }
        static inline auto read  (Array64 r, unsigned n)    { return *reinterpret_cast<uint64 volatile *>(MMAP_GLB_GICD + static_cast<unsigned>(r) + n * sizeof (uint64)); }

        static inline void write (Register32 r,          uint32 v) { *reinterpret_cast<uint32 volatile *>(MMAP_GLB_GICD + static_cast<unsigned>(r)) = v; }
        static inline void write (Array32 r, unsigned n, uint32 v) { *reinterpret_cast<uint32 volatile *>(MMAP_GLB_GICD + static_cast<unsigned>(r) + n * sizeof (uint32)) = v; }
        static inline void write (Array64 r, unsigned n, uint64 v) { *reinterpret_cast<uint64 volatile *>(MMAP_GLB_GICD + static_cast<unsigned>(r) + n * sizeof (uint64)) = v; }

        static bool mmap_mmio();
        static void init_mmio();
        static void wait_rwp();

        static inline void send_sgi (uint32 v)
        {
    ...
    public:
        ...
        static inline unsigned ints     { 0 };
        static inline Group group       { GROUP0 };

        static void init();

        static void conf (unsigned, bool, unsigned = 0);
