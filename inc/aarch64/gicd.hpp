...
class Gicd final : private Coresight, private Intid
{
    ...
    private:
        ...
        static inline auto read  (Register32 r)             { return *reinterpret_cast<uint32 volatile *>(MMAP_GLB_GICD + std::to_underlying (r)); }
        static inline auto read  (Array32 r, unsigned n)    { return *reinterpret_cast<uint32 volatile *>(MMAP_GLB_GICD + std::to_underlying (r) + n * sizeof (uint32)); }
        static inline auto read  (Array64 r, unsigned n)    { return *reinterpret_cast<uint64 volatile *>(MMAP_GLB_GICD + std::to_underlying (r) + n * sizeof (uint64)); }

        static inline void write (Register32 r,          uint32 v) { *reinterpret_cast<uint32 volatile *>(MMAP_GLB_GICD + std::to_underlying (r)) = v; }
        static inline void write (Array32 r, unsigned n, uint32 v) { *reinterpret_cast<uint32 volatile *>(MMAP_GLB_GICD + std::to_underlying (r) + n * sizeof (uint32)) = v; }
        static inline void write (Array64 r, unsigned n, uint64 v) { *reinterpret_cast<uint64 volatile *>(MMAP_GLB_GICD + std::to_underlying (r) + n * sizeof (uint64)) = v; }
