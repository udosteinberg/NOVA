...
class Console_uart_pl011 final : private Console_uart
{
    private:
        ...
        bool tx_busy() const override final { return read (Register16::FR) & BIT (3); }
        bool tx_full() const override final { return read (Register16::FR) & BIT (5); }
        void tx (uint8 c) const override final { write (Register16::DR, c); }
        ...
        void init() const override final
        {
            ...
            auto const i { static_cast<uint16>(clock / b) };
            auto const f { static_cast<uint16>(((clock << 6) + (b >> 1)) / b - (i << 6)) };
