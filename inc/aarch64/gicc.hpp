...
class Gicc final
{
    ...
    private:
        ...
        static auto read  (Reg32 r)      { return *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_GICC + std::to_underlying (r)); }
        static void write (Reg32 r, uint32_t v) { *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_GICC + std::to_underlying (r)) = v; }
