...
class Console_uart_mini final : private Console_uart
{
    private:
        ...
        bool tx_busy() const override final { return !(read (Register32::MU_STAT) & BIT (9)); }
        bool tx_full() const override final { return  (read (Register32::MU_STAT) & BIT (5)); }
        void tx (uint8_t c) const override final { write (Register32::MU_IO, c); }
    ...
    protected:
        explicit Console_uart_mini (Hpt::OAddr p, unsigned c) : Console_uart (c) { setup_regs (Acpi_gas::Asid::MMIO, p); }
};
