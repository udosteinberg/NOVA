...
class Smmu_v3 final : private Smmu
{
    private:
        ...
        template<typename T> class Queue
        {
            ...
            public:
                ...
                explicit Queue (bool c, unsigned hwo) : ptr { static_cast<T *>(Buddy::alloc (0, Buddy::Fill::BITS0)) }, ord { min (swo, hwo) }, coh { c }, swi { 0 } {}
        };
        ...
        arm_intid_t const   intid[4];                                   // EVT, PRI, GLB, CMD
        uint32_t const      idr0;
        uint32_t const      idr1;
        uint32_t const      idr5;
        ...
        explicit Smmu_v3 (uint64_t, arm_intid_t const (&)[4]);
        ...
        void fault (arm_intid_t iid) override final
        {
            if (intid[0] == iid)
                notify_evt();
        ...
        bool is_using_iid (arm_intid_t iid) const override final
        {
    ...
    public:
        [[nodiscard]] static auto setup (uint64_t base, arm_intid_t const (&intid)[4]) { return new Smmu_v3 { base, intid }; }
};
