...
class Smmu_v3 final : private Smmu
{
    private:
        ...
        {
            // A DSB is sufficient if SMMU is coherent
            if (coherent) [[likely]]
        ...
        class Cmd
        {
            ...
            public:
                ...
                auto type() const { return static_cast<uint8_t>(val[0]); }
        ...
        {
            explicit constexpr Cmd_cfgi_ste (uint32_t sid) : Cmd { Type::CFGI_STE, uint64_t { sid } << 32, BIT (0) } {}
        };
        ...
        class Evt final
        {
            ...
            public:
                ...
                auto type() const { return Type { static_cast<uint8_t>(val[0]) }; }
                auto sid() const { return static_cast<uint32_t>(val[0] >> 32); }
        };
        ...
        template<typename T> class Queue
        {
            ...
            public:
                ...
                explicit constexpr Queue (bool c, unsigned hwo) : ptr { static_cast<T *>(Buddy::alloc (0, Buddy::Fill::BITS0)) }, ord { min (swo, hwo) }, coh { c }, swi { 0 } {}
        };
        ...
        class Streamtable
        {
            private:
                ...
                class Ste final
                {
                    private:
                        /*
                         * An STE is always in one of the following states according to val[0]:
                         *
                         ...
                         */
                        Atomic<uint64_t> val[8];

                    public:
                        auto vmid() const { return static_cast<uint16_t>(val[2]); }

                        [[nodiscard]] Status detach (bool coherent, auto const &cfgi, auto const &tlbi)
                        {
                            // Determine if STE needs to be cleared
                            if (val[0]) [[unlikely]] {

                                // Attempt Valid -> Transient (abort on contention)
                                if (!val[0].test_and_clr (BIT (0))) [[unlikely]]
                                    return Status::ABORTED;

                                // STE must be transient now, ensuring exclusive access
                                assert (val[0] == BIT_RANGE (3, 2));

                                // Make invalid STE observable by SMMU
                                publish (&val[0], sizeof (*val), coherent);

                                // CFG invalidation (stale val[0]) + TLB invalidation
                                if (!cfgi() || !tlbi()) [[unlikely]]
                        ...
                        [[nodiscard]] Status attach (bool coherent, auto const &cfgi, void *ptab, uint16_t vmid, unsigned pas, unsigned isz, unsigned lev)
                        {
                            // Attempt Clear -> Transient (abort on contention)
                            if (val[0].test_and_set (BIT_RANGE (3, 2))) [[unlikely]]
                                return Status::ABORTED;

                            // STE must be transient now, ensuring exclusive access
                            assert (val[0] == BIT_RANGE (3, 2));

                            // Because we never set these fields, we expect them to remain zero
                            assert (!val[1] && !val[4] && !val[5] && !val[6] && !val[7]);

                            //       S2R        | S2AA64     | S2PS                   | S2SH0                | S2OR0      | S2IR0      | S2SL0                      | S2T0SZ                      | S2VMID
                            val[2] = BIT64 (58) | BIT64 (51) | uint64_t { pas } << 48 | BIT64_RANGE (45, 44) | BIT64 (42) | BIT64 (40) | uint64_t { lev - 2 } << 38 | uint64_t { 64 - isz } << 32 | vmid;

                            //       S2TTB
                            val[3] = Kmem::ptr_to_phys (ptab);

                            // Make updates observable by SMMU
                            publish (&val[2], sizeof (*val) * 2, coherent);

                            // CFG invalidation (stale val[2,3])
                            if (!cfgi()) [[unlikely]]
                                return Status::TIMEOUT;
                            ...
                            // Make valid STE observable by SMMU
                            publish (&val[0], sizeof (*val), coherent);

                            // CFG invalidation (stale val[0])
                            if (!cfgi()) [[unlikely]]
                                return Status::TIMEOUT;
                ...
                class Std final
                {
                    ...
                    public:
                        [[nodiscard]] Ste *table (bool coherent)
                        {
                            for (uint64_t v { val };;) {

                                // Return existing stream table
                                if (v) [[likely]]
                                    return static_cast<Ste *>(Kmem::phys_to_ptr (v & ~BIT_RANGE (5, 0)));
                                ...
                                // Make updated STD observable by SMMU
                                publish (this, sizeof (*this), coherent);
            ...
            public:
                ...
                [[nodiscard]] Ste *entry (bool coherent, uint32_t sid)
                {
                    auto p { ptr };
                    ...
                    switch (lev) {
                        ...
                        case 1:
                            ...
                            p = (static_cast<Std *>(p) + (sid >> ord_ste) % BIT (ord - ord_ste))->table (coherent);
                            [[fallthrough]];
                ...
                explicit constexpr Streamtable (bool c, unsigned hwo) : ord { min (swo, hwo) }, lev { ord > ord_ste }, ptr { lev ? static_cast<void *>(new (c, ord) Std) : static_cast<void *>(new (c) Ste) } {}
        };

        unsigned const      intid[4];                                   // EVT, PRI, GLB, CMD
        uint32_t const      idr0;
        uint32_t const      idr1;
        uint32_t const      idr5;
        ...
        bool feat_s2p() const { return BIT  (0) & idr0; }               // Supports stage-2 translation
        bool feat_s1p() const { return BIT  (1) & idr0; }               // Supports stage-1 translation
        bool feat_t64() const { return BIT  (3) & idr0; }               // Supports VMSAv8-64
        bool coherent() const { return BIT  (4) & idr0; }               // Supports coherent access to translations, structures and queues
        bool feat_btm() const { return BIT  (5) & idr0; }               // Supports broadcast TLB maintenance
        bool feat_hyp() const { return BIT  (9) & idr0; }               // Supports hypervisor stage-1 contexts
        bool feat_msi() const { return BIT (13) & idr0; }               // Supports message-signalled interrupts
        bool feat_pri() const { return BIT (16) & idr0; }               // Supports page request interface

        auto oas()      const { return BIT_RANGE (2, 0) & idr5; }       // Output address size
        bool feat_g04() const { return BIT (4) & idr5; }                // Supports translation granule 4K
        ...
        explicit Smmu_v3 (uint64_t, unsigned const (&)[4]);
        ...
        [[nodiscard]] bool cfgi_ste (uint32_t sid)
        {
            return command (Cmd_cfgi_ste { sid }, true);
        }
        ...
        bool tlb_invalidate (Sdid vmid) override final
        {
        ...
        void fault (unsigned spi) override final
        {
            auto const i { Intid::from_spi (spi) };
        ...
        bool is_using_spi (unsigned spi) const override final
        {
            auto const i { Intid::from_spi (spi) };

            return i == intid[0] || i == intid[1] || i == intid[2] || i == intid[3];
        }

        bool init() override final;

        Status assign_dev (Space_dma *, uintptr_t) override final;
    ...
    public:
        [[nodiscard]] static auto setup (uint64_t base, unsigned const (&intid)[4]) { return new Smmu_v3 { base, intid }; }
};
