...
class Smmu_v3 final : private Smmu
{
    private:
            ...
            private:
                ...
                class Ste final
                {
                    ...
                    public:
                        ...
                        [[nodiscard]] Status detach (bool coherent, auto const &stei, auto const &tlbi)
                        {
                            // Determine if STE needs to be cleared
                            if (val[0]) [[unlikely]] {

                                // Attempt Valid -> Transient (abort if another CPU beat us to it)
                                ...
                                // Make STE update observable by SMMU
                                Coherency::observe (&val[0], sizeof (*val), coherent);

                                // STE invalidation (after update) + TLB invalidation
                                if (!stei() || !tlbi()) [[unlikely]]
                        ...
                        [[nodiscard]] Status attach (bool coherent, auto const &stei, void *ptab, uint16_t vmid, unsigned pas, unsigned isz, unsigned lev)
                        {
                            // Attempt Clear -> Transient (abort if another CPU beat us to it)
                            ...
                            // Make STE update observable by SMMU
                            Coherency::observe (&val[2], sizeof (*val) * 2, coherent);

                            // STE invalidation (after update)
                            if (!stei()) [[unlikely]]
                                return Status::TIMEOUT;
                        ...
                        [[nodiscard]] static void *operator new (size_t, bool c) noexcept
                        {
                            ...
                            if (ptr) [[likely]]
                                Coherency::observe (ptr, PAGE_SIZE (0), c);
                    ...
                    public:
                        [[nodiscard]] Ste *table (bool coherent, auto const &stdi)
                        {
                            for (uint64_t v { val };;) {
                                ...
                                if (v) [[likely]]
                                    return static_cast<Ste *>(Kmem::phys_to_ptr (v & BIT64_RANGE (55, 6)));
                                ...
                                Coherency::observe (this, sizeof (*this), coherent);
        ...
        void fault (unsigned iid) override final
        {
            if (intid[0] == iid)
                notify_evt();
