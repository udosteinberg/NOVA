...
class Console_uart_mmio_scif final : private Console_uart_mmio
{
    private:
        ...
        auto read  (Register8  r)    { return *reinterpret_cast<uint8  volatile *>(mmio_base + std::to_underlying (r)); }
        auto read  (Register16 r)    { return *reinterpret_cast<uint16 volatile *>(mmio_base + std::to_underlying (r)); }
        void write (Register8  r, uint8  v) { *reinterpret_cast<uint8  volatile *>(mmio_base + std::to_underlying (r)) = v; }
        void write (Register16 r, uint16 v) { *reinterpret_cast<uint16 volatile *>(mmio_base + std::to_underlying (r)) = v; }

        bool tx_busy() override final { return !(read (Register16::SCFSR) & BIT (6)); }
        bool tx_full() override final { return !(read (Register16::SCFSR) & BIT (5)); }
    ...
    protected:
        explicit Console_uart_mmio_scif (Hpt::OAddr p, unsigned c) : Console_uart_mmio (c) { mmap (p); }
