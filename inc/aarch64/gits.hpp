...
class Gits final : public List<Gits>, private Coresight, private Mmio
{
    ...
    private:
        ...
        auto read  (Arr64 r, unsigned n) const      { return *std::start_lifetime_as<uint64_t volatile> (mmio + std::to_underlying (r) + n * sizeof (uint64_t)); }
        ...
        void write (Arr64 r, unsigned n, uint64_t v) const { *std::start_lifetime_as<uint64_t volatile> (mmio + std::to_underlying (r) + n * sizeof (uint64_t)) = v; }
        ...
        {
            explicit constexpr Cmd_mapti (arm_devid_t did, arm_evtid_t eid, uint32_t iid, arm_colid_t cid) : Cmd { Type::MAPTI, uint64_t { did } << 32, uint64_t { iid } << 32 | eid, cid } {}
        };
        ...
        class Queue
        {
            ...
            public:
                ...
                static constexpr Buddy::order_t pao { 4 };

                // Order of queue entries
                static constexpr unsigned ord { pao + bit_scan_msb (PAGE_SIZE (0) / sizeof (Cmd)) };
                ...
                void produce (bool noncoherent, Cmd const &cmd)
                {
                    ...
                    *p = cmd;
                    ...
                    Coherence::producer (noncoherent, p, sizeof (*p));
                }

                void replace (bool noncoherent, Cmd &cmd, index_t hwi) const
                {
                ...
                explicit Queue() : ptr { new Cmd[BIT (ord)] }, swi { 0 } {}
        };
        ...
        struct Table
        {
            ...
            public:
                [[nodiscard]] bool allocate (bool noncoherent, unsigned o)
                {
                    ...
                    if (!v) [[unlikely]] {
                        ...
                        // Allocate leaf table
                        auto const ptr { new (noncoherent, o) Table };
                        ...
                        if (val.compare_exchange_n (v, BIT64 (63) | Kmem::ptr_to_phys (ptr))) [[likely]]
                            Coherence::producer (noncoherent, this, sizeof (*this));
                        else
                            delete ptr;
                    }
                ...
                [[nodiscard]] static void *operator new (size_t, bool noncoherent, unsigned o) noexcept
                {
                    ...
                    if (ptr) [[likely]]
                        Coherence::producer (noncoherent, ptr, PAGE_SIZE (0) << o);
    ...
    public:
        ...
        [[nodiscard]] Status conf_lpi (Intid, arm_colid_t, arm_devid_t, arm_evtid_t, uintptr_t &, uintptr_t &);
