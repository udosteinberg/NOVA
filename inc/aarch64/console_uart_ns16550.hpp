...
class Console_uart_ns16550 final : private Console_uart
{
    private:
        ...
        auto read  (Reg8 r) const     { return *std::start_lifetime_as<uint8_t volatile> (mmap + (std::to_underlying (r) << regs.shl)); }
        void write (Reg8 r, uint8_t v) const { *std::start_lifetime_as<uint8_t volatile> (mmap + (std::to_underlying (r) << regs.shl)) = v; }
