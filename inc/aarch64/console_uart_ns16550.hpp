...
class Console_uart_ns16550 final : private Console_uart
{
    private:
        ...
        auto read  (Reg8 r) const     { return *reinterpret_cast<uint8_t volatile *>(mmap + (std::to_underlying (r) << regs.shft)); }
        void write (Reg8 r, uint8_t v) const { *reinterpret_cast<uint8_t volatile *>(mmap + (std::to_underlying (r) << regs.shft)) = v; }
    ...
    protected:
        explicit Console_uart_ns16550 (Hpt::OAddr p, unsigned c) : Console_uart { c } { setup (Regs { .phys = p, .shft = 2 }); }
};
