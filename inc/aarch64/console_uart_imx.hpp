...
class Console_uart_imx final : private Console_uart
{
    private:
        ...
        auto read  (Reg32 r) const      { return *std::start_lifetime_as<uint32_t volatile> (mmap + std::to_underlying (r)); }
        void write (Reg32 r, uint32_t v) const { *std::start_lifetime_as<uint32_t volatile> (mmap + std::to_underlying (r)) = v; }
