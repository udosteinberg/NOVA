...
{
    private:
        static bool permitted (uint32_t fid)
        {
            unsigned const s { fid >> 24 & BIT_RANGE (5, 0) };

            switch (static_cast<Service>(s)) {
                case Service::SEC:
                    return (BIT (19) | BIT (5)) & BIT (static_cast<uint16_t>(fid) >> 4);
                default:
    ...
    protected:
        ...
        static constexpr uint32_t fid (Call c, Service s, uint16_t f)
        {
            return BIT (31) | std::to_underlying (c) << 30 | std::to_underlying (s) << 24 | f;
        }

        ALWAYS_INLINE
        static inline auto call (Call c, Service s, uint16_t f, uintptr_t &p1, uintptr_t &p2, uintptr_t &p3)
        {
    ...
    public:
        ...
        {
            if (!permitted (static_cast<uint32_t>(r[0]))) [[unlikely]]
                return false;
