...
class Cpu final
{
    private:
        ...
        static uint64   feat_cpu64[3]       CPULOCAL;           // ID_AA64PFRx
        static uint64   feat_dbg64[2]       CPULOCAL;           // ID_AA64DFRx
        static uint64   feat_isa64[3]       CPULOCAL;           // ID_AA64ISARx
        static uint64   feat_mem64[5]       CPULOCAL;           // ID_AA64MMFRx
    ...
    public:
        ...
        [[nodiscard]] static uint8 feature (Cpu_feature f) { return feat_cpu64[std::to_underlying (f) / 16] >> std::to_underlying (f) % 16 * 4 & BIT_RANGE (3, 0); }
        [[nodiscard]] static uint8 feature (Dbg_feature f) { return feat_dbg64[std::to_underlying (f) / 16] >> std::to_underlying (f) % 16 * 4 & BIT_RANGE (3, 0); }
        [[nodiscard]] static uint8 feature (Isa_feature f) { return feat_isa64[std::to_underlying (f) / 16] >> std::to_underlying (f) % 16 * 4 & BIT_RANGE (3, 0); }
        [[nodiscard]] static uint8 feature (Mem_feature f) { return feat_mem64[std::to_underlying (f) / 16] >> std::to_underlying (f) % 16 * 4 & BIT_RANGE (3, 0); }
