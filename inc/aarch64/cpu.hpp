...
class Cpu final
{
    ...
    public:
        enum class Cpu_feature : unsigned
        {
            ...
            DIT         = 12,   // FEAT_DIT (1)
            RME         = 13,   // FEAT_RME (1)
            CSV2        = 14,   // FEAT_CSV2 (1), FEAT_CSV2_2 (2)
            CSV3        = 15,   // FEAT_CSV3 (1)
            BT          = 16,   // FEAT_BTI (1)
        ...
        static inline unsigned feature (Cpu_feature f) { return feat_cpu64[std::to_underlying (f) / 16] >> std::to_underlying (f) % 16 * 4 & BIT_RANGE (3, 0); }
        static inline unsigned feature (Dbg_feature f) { return feat_dbg64[std::to_underlying (f) / 16] >> std::to_underlying (f) % 16 * 4 & BIT_RANGE (3, 0); }
        static inline unsigned feature (Isa_feature f) { return feat_isa64[std::to_underlying (f) / 16] >> std::to_underlying (f) % 16 * 4 & BIT_RANGE (3, 0); }
        static inline unsigned feature (Mem_feature f) { return feat_mem64[std::to_underlying (f) / 16] >> std::to_underlying (f) % 16 * 4 & BIT_RANGE (3, 0); }
