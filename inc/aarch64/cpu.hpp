...
class Cpu
{
    ...
    public:
        ...
        {
            return *reinterpret_cast<decltype (affinity) *>(reinterpret_cast<uintptr_t>(&affinity) - CPU_LOCAL_DATA + CPU_GLOBL_DATA + cpu * PAGE_SIZE);
        }
        ...
        {
            return feat_dbg64[static_cast<unsigned>(f) / 16] >> static_cast<unsigned>(f) % 16 * 4 & 0xf;
        }

        static inline unsigned feature (Isa_feature f)
        {
            return feat_isa64[static_cast<unsigned>(f) / 16] >> static_cast<unsigned>(f) % 16 * 4 & 0xf;
        }

        static inline unsigned feature (Mem_feature f)
        {
            return feat_mem64[static_cast<unsigned>(f) / 16] >> static_cast<unsigned>(f) % 16 * 4 & 0xf;
        }
