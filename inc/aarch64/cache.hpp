...
class Cache
{
    ...
    public:
        ...
        static inline void init()
        {
            ...
            icache_line_size = 4 << (ctr       & 0xf);
        }

        ALWAYS_INLINE
        static inline void dcache_clean (void const *ptr)
        {
            asm volatile ("dc cvac, %0; dsb sy" : : "r" (ptr) : "memory");
        }

        ALWAYS_INLINE
        static inline void dcache_clean (void const *ptr, size_t size)
        {
        ...
        static inline void icache_invalidate()
        {
            asm volatile ("     ic      iallu       ;"  // Invalidate entire I$ to PoU
                          ...
                          "     isb                 ;"  // Ensure instruction fetches observe new cache state
                          : : : "memory");
        }
};
