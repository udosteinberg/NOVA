...
class Gicr final : private Coresight
{
    ...
    private:
        ...
        class Cfg_table
        {
            ...
            public:
                void set (bool nc, bool msk)
                {
                    // Apply configuration
                    val = BIT (1) | !msk * BIT (0);

                    // Make update observable
                    Coherence::producer (nc, &val, sizeof (val));
                }

                [[nodiscard]] static void *operator new (size_t, bool nc, unsigned o) noexcept
                {
                    ...
                    if (ptr) [[likely]]
                        Coherence::producer (nc, ptr, PAGE_SIZE (0) << ord);
