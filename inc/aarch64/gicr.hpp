...
class Gicr final : private Coresight, private Intid
{
    ...
    private:
        ...
        static inline auto read  (Register32 r)             { return *reinterpret_cast<uint32 volatile *>(MMAP_CPU_GICR + std::to_underlying (r)); }
        static inline auto read  (Register64 r)             { return *reinterpret_cast<uint64 volatile *>(MMAP_CPU_GICR + std::to_underlying (r)); }
        static inline auto read  (Array32 r, unsigned n)    { return *reinterpret_cast<uint32 volatile *>(MMAP_CPU_GICR + std::to_underlying (r) + n * sizeof (uint32)); }

        static inline void write (Register32 r,          uint32 v) { *reinterpret_cast<uint32 volatile *>(MMAP_CPU_GICR + std::to_underlying (r)) = v; }
        static inline void write (Register64 r,          uint64 v) { *reinterpret_cast<uint64 volatile *>(MMAP_CPU_GICR + std::to_underlying (r)) = v; }
        static inline void write (Array32 r, unsigned n, uint32 v) { *reinterpret_cast<uint32 volatile *>(MMAP_CPU_GICR + std::to_underlying (r) + n * sizeof (uint32)) = v; }
