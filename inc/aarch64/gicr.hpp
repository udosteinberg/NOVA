...
class Gicr final : private Coresight, private Intid
{
    ...
    private:
        ...
        static inline auto read  (Register32 r)               { return *reinterpret_cast<uint32_t volatile *>(MMAP_CPU_GICR + std::to_underlying (r)); }
        static inline auto read  (Register64 r)               { return *reinterpret_cast<uint64_t volatile *>(MMAP_CPU_GICR + std::to_underlying (r)); }
        static inline auto read  (Array32 r, unsigned n)      { return *reinterpret_cast<uint32_t volatile *>(MMAP_CPU_GICR + std::to_underlying (r) + n * sizeof (uint32_t)); }

        static inline void write (Register32 r,          uint32_t v) { *reinterpret_cast<uint32_t volatile *>(MMAP_CPU_GICR + std::to_underlying (r)) = v; }
        static inline void write (Register64 r,          uint64_t v) { *reinterpret_cast<uint64_t volatile *>(MMAP_CPU_GICR + std::to_underlying (r)) = v; }
        static inline void write (Array32 r, unsigned n, uint32_t v) { *reinterpret_cast<uint32_t volatile *>(MMAP_CPU_GICR + std::to_underlying (r) + n * sizeof (uint32_t)) = v; }
