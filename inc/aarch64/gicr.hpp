...
class Gicr final : private Coresight
{
    ...
    private:
        ...
        static auto read  (Reg32 r)                  { return *std::start_lifetime_as<uint32_t volatile> (MMAP_CPU_GICR + std::to_underlying (r)); }
        static auto read  (Reg64 r)                  { return *std::start_lifetime_as<uint64_t volatile> (MMAP_CPU_GICR + std::to_underlying (r)); }
        static auto read  (Arr32 r, unsigned n)      { return *std::start_lifetime_as<uint32_t volatile> (MMAP_CPU_GICR + std::to_underlying (r) + n * sizeof (uint32_t)); }

        static void write (Reg32 r,             uint32_t v) { *std::start_lifetime_as<uint32_t volatile> (MMAP_CPU_GICR + std::to_underlying (r)) = v; }
        static void write (Reg64 r,             uint64_t v) { *std::start_lifetime_as<uint64_t volatile> (MMAP_CPU_GICR + std::to_underlying (r)) = v; }
        static void write (Arr32 r, unsigned n, uint32_t v) { *std::start_lifetime_as<uint32_t volatile> (MMAP_CPU_GICR + std::to_underlying (r) + n * sizeof (uint32_t)) = v; }
        ...
        class Cfg_table
        {
            ...
            public:
                void set (bool noncoherent, bool msk)
                {
                    // Apply configuration
                    val = BIT (1) | !msk * BIT (0);

                    // Make update observable
                    Coherence::producer (noncoherent, &val, sizeof (val));
                }

                [[nodiscard]] static void *operator new (size_t, bool noncoherent, unsigned o) noexcept
                {
                    ...
                    if (ptr) [[likely]]
                        Coherence::producer (noncoherent, ptr, PAGE_SIZE (0) << ord);
        ...
        static inline void init_intid (uint32_t, unsigned);
    ...
    public:
        ...
        static bool act_get (Intid);
        static void act_set (Intid, bool);

        static Status conf_ppi (Intid, bool = false, bool = false);

        static void conf_lpi (Intid, bool);
};
