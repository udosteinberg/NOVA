...
class Gicr : private Coresight, private Intid
{
    ...
    private:
        ...
        static inline uint64 phys   { Board::gic[1].mmio };
        static inline uint32 size   { Board::gic[1].size };

        static inline auto read  (Register32 r)             { return *reinterpret_cast<uint32 volatile *>(DEV_LOCAL_GICR + static_cast<unsigned>(r)); }
        static inline auto read  (Register64 r)             { return *reinterpret_cast<uint64 volatile *>(DEV_LOCAL_GICR + static_cast<unsigned>(r)); }
        static inline auto read  (Array32 r, unsigned n)    { return *reinterpret_cast<uint32 volatile *>(DEV_LOCAL_GICR + static_cast<unsigned>(r) + n * sizeof (uint32)); }

        static inline void write (Register32 r,          uint32 v) { *reinterpret_cast<uint32 volatile *>(DEV_LOCAL_GICR + static_cast<unsigned>(r)) = v; }
        static inline void write (Register64 r,          uint64 v) { *reinterpret_cast<uint64 volatile *>(DEV_LOCAL_GICR + static_cast<unsigned>(r)) = v; }
        static inline void write (Array32 r, unsigned n, uint32 v) { *reinterpret_cast<uint32 volatile *>(DEV_LOCAL_GICR + static_cast<unsigned>(r) + n * sizeof (uint32)) = v; }
