...
class Smmu_v2 final : private Smmu
{
    private:
        ...
        auto read  (GR0_Reg32 r)               const { return *reinterpret_cast<uint32_t volatile *>(mmio                                    + std::to_underlying (r)); }
        auto read  (GR0_Reg64 r)               const { return *reinterpret_cast<uint64_t volatile *>(mmio                                    + std::to_underlying (r)); }
        auto read  (unsigned smg, GR0_Arr32 r) const { return *reinterpret_cast<uint32_t volatile *>(mmio          + smg * sizeof (uint32_t) + std::to_underlying (r)); }
        auto read  (unsigned ctx, GR1_Arr32 r) const { return *reinterpret_cast<uint32_t volatile *>(mmio_base_gr1 + ctx * sizeof (uint32_t) + std::to_underlying (r)); }
        auto read  (unsigned ctx, Ctx_Arr32 r) const { return *reinterpret_cast<uint32_t volatile *>(mmio_base_ctx + ctx * page_size         + std::to_underlying (r)); }
        auto read  (unsigned ctx, Ctx_Arr64 r) const { return *reinterpret_cast<uint64_t volatile *>(mmio_base_ctx + ctx * page_size         + std::to_underlying (r)); }

        void write (GR0_Reg32 r,               uint32_t v) const { *reinterpret_cast<uint32_t volatile *>(mmio                                    + std::to_underlying (r)) = v; }
        void write (GR0_Reg64 r,               uint64_t v) const { *reinterpret_cast<uint64_t volatile *>(mmio                                    + std::to_underlying (r)) = v; }
        void write (unsigned smg, GR0_Arr32 r, uint32_t v) const { *reinterpret_cast<uint32_t volatile *>(mmio          + smg * sizeof (uint32_t) + std::to_underlying (r)) = v; }
        void write (unsigned ctx, GR1_Arr32 r, uint32_t v) const { *reinterpret_cast<uint32_t volatile *>(mmio_base_gr1 + ctx * sizeof (uint32_t) + std::to_underlying (r)) = v; }
        void write (unsigned ctx, Ctx_Arr32 r, uint32_t v) const { *reinterpret_cast<uint32_t volatile *>(mmio_base_ctx + ctx * page_size         + std::to_underlying (r)) = v; }
        void write (unsigned ctx, Ctx_Arr64 r, uint64_t v) const { *reinterpret_cast<uint64_t volatile *>(mmio_base_ctx + ctx * page_size         + std::to_underlying (r)) = v; }
        ...
        explicit Smmu_v2 (Board::Smmu_v2 const &);
        ...
        bool tlb_invalidate (Sdid) override final;
        Status assign_dev (Space_dma *, uintptr_t) override final;

        bool tlb_invalidate (unsigned, uint64_t);
        bool tlb_sync_ctx (unsigned);
        bool tlb_sync_glb();
    ...
    public:
        [[nodiscard]] static auto setup (Board::Smmu_v2 const &board) { return new Smmu_v2 { board }; }
};
