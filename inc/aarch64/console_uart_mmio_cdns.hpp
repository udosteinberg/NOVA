...
class Console_uart_mmio_cdns final : private Console_uart_mmio
{
    private:
        ...
        auto read  (Register32 r)    { return *reinterpret_cast<uint32 volatile *>(mmio_base + std::to_underlying (r)); }
        void write (Register32 r, uint32 v) { *reinterpret_cast<uint32 volatile *>(mmio_base + std::to_underlying (r)) = v; }

        bool tx_busy() override final { return (read (Register32::CSTS) & (BIT (11) | BIT (3))) != BIT (3); }
        bool tx_full() override final { return (read (Register32::CSTS) &  BIT (4)); }
        void tx (uint8 c) override final { write (Register32::FIFO, c); }
    ...
    protected:
        explicit Console_uart_mmio_cdns (Hpt::OAddr p, unsigned c) : Console_uart_mmio (c) { mmap (p); }
