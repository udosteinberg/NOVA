...
class Console_uart_mmio_cdns final : private Console_uart_mmio
{
    private:
        ...
        void init()
        {
            ...
            write (Register32::BGEN, refclock / baudrate / 5);
            write (Register32::BDIV, 4);
            write (Register32::CTRL, BIT_RANGE (4, 3) | BIT_RANGE (1, 0));
        }

    public:
        Console_uart_mmio_cdns (Hpt::OAddr p, unsigned c) : Console_uart_mmio (p), refclock (c)
        {
            if (!mmio_base)
                return;
