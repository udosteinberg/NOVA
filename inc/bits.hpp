 ...
 */
template<typename T> static constexpr unsigned bits() { return 8 * sizeof (T); }

/*
 * Determine the most significant bit number for type T
 */
template<typename T> static constexpr unsigned msbn() { return bits<T>() - 1; }
...
{
    return !v ? -1 : msbn<decltype (v)>() - __builtin_clzl (v);
}
...
{
    return min (bit_scan_msb (size), bit_scan_lsb (BITN (msbn<uintptr_t>()) | (addr | ...)));
}
...
// Sanity checks
static_assert (msbn<uint16_t>() == 15);
static_assert (msbn<uint32_t>() == 31);
static_assert (msbn<uint64_t>() == 63);
