...
class Pci final
{
    private:
        ...
        static constexpr size_t   cfg_size { 4096 };
        static constexpr unsigned seg_shft {   16 };
        static constexpr unsigned bus_shft {    8 };
        static constexpr unsigned dev_shft {    3 };
        ...
        // Bus numbers already enumerated in the segment being scanned
        using Bus_bitmap = Bitmap<BIT (seg_shft - bus_shft), false>;

        static bool init_bus (uint16_t, uint8_t, uint8_t, uint8_t, Bus_bitmap &);

    public:
        static constexpr pci_t pci (uint16_t s, uint8_t b, uint8_t d, uint8_t f) { return pci_t { s } << seg_shft | b << bus_shft | d << dev_shft | f; }
        static constexpr pci_t pci (uint16_t s, uint16_t bdf)                    { return pci_t { s } << seg_shft | bdf; }
