    ...
    public:
        ...
        class Function final : public List<Function>, private Cap_pmi, private Cap_pcie, private Cap_msi, private Cap_msix, private Cap_sdev, private Cap_sriov, private Cap_ats, private Cap_pri
        {
            ...
            public:
                ...
                auto read (Cfg::Reg8  r) const { return *reinterpret_cast<uint8_t  volatile *>(ecam_addr (sbdf, std::to_underlying (r))); }
                auto read (Cfg::Reg16 r) const { return *reinterpret_cast<uint16_t volatile *>(ecam_addr (sbdf, std::to_underlying (r))); }
                auto read (Cfg::Reg32 r) const { return *reinterpret_cast<uint32_t volatile *>(ecam_addr (sbdf, std::to_underlying (r))); }

                // Configuration Write  (always 1 DWORD == 32-bit with byte enables)
                void write (Cfg::Reg8  r, uint8_t  v) const { *reinterpret_cast<uint8_t  volatile *>(ecam_addr (sbdf, std::to_underlying (r))) = v; }
                void write (Cfg::Reg16 r, uint16_t v) const { *reinterpret_cast<uint16_t volatile *>(ecam_addr (sbdf, std::to_underlying (r))) = v; }
                void write (Cfg::Reg32 r, uint32_t v) const { *reinterpret_cast<uint32_t volatile *>(ecam_addr (sbdf, std::to_underlying (r))) = v; }
