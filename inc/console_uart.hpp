...
class Console_uart : protected Console
{
    private:
        ...
        bool using_regs (Acpi_gas::Asid asid, uint64 addr) const override final
        {
    ...
    protected:
        ...
        bool setup (Regs const r)
        {
            ...
            if (EXPECT_TRUE (r.phys)) {
                mmap = mmap_base.fetch_add (PAGE_SIZE) | (r.phys & OFFS_MASK);
                Hptp::master_map (mmap & ~OFFS_MASK, r.phys & ~OFFS_MASK, 0, Paging::Permissions (Paging::G | Paging::W | Paging::R), Memattr::dev());
            }
        ...
        bool setup_regs (Acpi_gas::Asid asid, uint64 addr) override final
        {
            return setup (Regs { .phys = (asid == Acpi_gas::Asid::MMIO) * addr, .port = static_cast<uint16>((asid == Acpi_gas::Asid::PIO) * addr) });
        }
