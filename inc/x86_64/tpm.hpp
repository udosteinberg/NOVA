    ...
    private:
        ...
        template<typename T> class Interface
        {
            protected:
                ...
                template<typename I = T> static auto read  (Locality l, typename I::Reg8  r)      { return *reinterpret_cast<uint8_t  volatile *>(MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)); }
                template<typename I = T> static auto read  (Locality l, typename I::Reg32 r)      { return *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)); }
                template<typename I = T> static void write (Locality l, typename I::Reg8  r, uint8_t  v) { *reinterpret_cast<uint8_t  volatile *>(MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)) = v; }
                template<typename I = T> static void write (Locality l, typename I::Reg32 r, uint32_t v) { *reinterpret_cast<uint32_t volatile *>(MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)) = v; }
        };
        ...
        class Fifo final : private Interface<Fifo>
        {
            ...
            private:
                ...
                [[nodiscard]] static bool wait_done (Locality l, uint32_t m)
                {
                    ...
                    // Wait for stsValid=1 and subsequently check that Expect/dataAvail is 0
                    ...
                    for (unsigned i { 0 }, b { 0 }; i < s; b--) {
                        ...
                        *p++ = read (l, Reg8::DATA);
                        ...
                        if (++i == sizeof (Res)) [[unlikely]]
                            s = min (sizeof (buffer), reinterpret_cast<Res const *>(buffer)->size());
                    }
