...
class Tpm final : private Tcg
{
    ...
    private:
        ...
        template<typename T> class Interface
        {
            protected:
                ...
                template<typename X = T> static auto read  (Locality l, typename X::Reg8  r)      { return *std::start_lifetime_as<uint8_t  volatile> (MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)); }
                template<typename X = T> static auto read  (Locality l, typename X::Reg32 r)      { return *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)); }
                template<typename X = T> static auto read  (Locality l, typename X::Reg64 r)      { return *std::start_lifetime_as<uint64_t volatile> (MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)); }
                template<typename X = T> static void write (Locality l, typename X::Reg8  r, uint8_t  v) { *std::start_lifetime_as<uint8_t  volatile> (MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)) = v; }
                template<typename X = T> static void write (Locality l, typename X::Reg32 r, uint32_t v) { *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)) = v; }
                template<typename X = T> static void write (Locality l, typename X::Reg64 r, uint64_t v) { *std::start_lifetime_as<uint64_t volatile> (MMAP_GLB_TPM2 + std::to_underlying (l) + std::to_underlying (r)) = v; }
        };
                    ...
                    for (unsigned i { 0 }, b { 0 }; i < s; b--) {
                        ...
                        *p++ = read (l, Reg8::DATA);
                        ...
                        if (++i == sizeof (Res)) [[unlikely]] {
                            ...
                            s = std::start_lifetime_as<Res const> (buffer)->size();
        ...
        [[nodiscard]] static bool pcr_extend (Locality l, Hmac_session &s, unsigned pcr, auto const &h) { return invoke (l, Tpm2_pcr_extend<Hmac_session::Type> { pcr, h }, &s); }
    ...
    public:
        ...
        [[nodiscard]] static bool extend (Locality l, unsigned pcr, Hash_sha1_160 const &sha1_160, Hash_sha2_256 const &sha2_256, Hash_sha2_384 const &sha2_384, Hash_sha2_512 const &sha2_512)
        {
            ...
            ret &= (!hash.supported (Hash_bmp::Type::SHA1_160) || pcr_extend (l, s, pcr, sha1_160));
            ret &= (!hash.supported (Hash_bmp::Type::SHA2_256) || pcr_extend (l, s, pcr, sha2_256));
            ret &= (!hash.supported (Hash_bmp::Type::SHA2_384) || pcr_extend (l, s, pcr, sha2_384));
            ret &= (!hash.supported (Hash_bmp::Type::SHA2_512) || pcr_extend (l, s, pcr, sha2_512));
            ret &= flush_context (l, s.handle);

            // Release locality
            if (!release (l)) [[unlikely]]
                return false;
