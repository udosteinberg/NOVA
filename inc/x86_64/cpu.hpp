...
class Cpu final
{
    private:
        ...
        static uint64_t phys_local CPULOCAL;
        ...
        static void enumerate_topology (uint32_t, uint32_t &, uint32_t &, uint32_t (&)[4]);
    ...
    public:
        ...
        {
            // AMD requires SS.RPL = STAR[49:48] = 3 for SYSRET and SS.RPL = STAR[33:32] = 0 for SYSCALL
            .star  = static_cast<uint64_t>(SEL_KERN_DATA | 3) << 48 | static_cast<uint64_t>(SEL_KERN_CODE | 0) << 32,
        ...
        static uint32_t     socket          CPULOCAL;
        ...
        {
            return reinterpret_cast<uintptr_t>(p) - MMAP_CPU_DATA + phys_local;
        }
