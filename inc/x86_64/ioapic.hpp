...
class Ioapic final : public List<Ioapic>
{
    private:
        ...
        // Direct Registers
        enum class Dir_Register8 : unsigned
        {
            IND     = 0x0,                  // Index Register
        };

        // Direct Registers
        enum class Dir_Register32 : unsigned
        {
        ...
        // Indirect Registers
        enum class Ind_Register32 : uint8_t
        {
            ID      = 0x0,                  // ID Register
        ...
        auto read  (Dir_Register32 r) const      { return *reinterpret_cast<uint32_t volatile *>(reg_base + std::to_underlying (r)); }
        void write (Dir_Register32 r, uint32_t v) const { *reinterpret_cast<uint32_t volatile *>(reg_base + std::to_underlying (r)) = v; }
        void write (Dir_Register8  r, uint8_t  v) const { *reinterpret_cast<uint8_t  volatile *>(reg_base + std::to_underlying (r)) = v; }

        void index (Ind_Register32 r) const
        {
            write (Dir_Register8::IND, std::to_underlying (r));
        }

        uint32_t read (Ind_Register32 r)
        {
        ...
        void write (Ind_Register32 r, uint32_t v)
        {
            Lock_guard <Spinlock> guard { lock };
            index (r);
    ...
    public:
        ...
        auto mre() { return read (Ind_Register32::VER) >> 16 & BIT_RANGE (7, 0); }
        auto ver() { return read (Ind_Register32::VER)       & BIT_RANGE (7, 0); }

        ALWAYS_INLINE
        void set_dst (unsigned gsi, uint32_t v)
        {
            auto const rte { gsi - gsi_base };
            write (Ind_Register32 (std::to_underlying (Ind_Register32::RTE) + 2 * rte + 1), v);
        }
        ...
        {
            auto const rte { gsi - gsi_base };
            write (Ind_Register32 (std::to_underlying (Ind_Register32::RTE) + 2 * rte), msk << 16 | trg << 15 | pol << 13 | ((VEC_GSI + gsi) & BIT_RANGE (7, 0)));
        }
