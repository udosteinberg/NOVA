...
class Ioapic final : public List<Ioapic>, private Mmio
{
    ...
    public:
        ...
        explicit Ioapic (uint64_t p, uint8_t i, unsigned g) : List { list }, Mmio { p, PAGE_SIZE (0) }, gsi_base { g }, gsi_last { g + mre() }, id { i } {}
        ...
        /*
         * Determine IOAPIC to which a GSI is connected
         *
         * @param gsi   Global System Interrupt
         * @return      Pointer to IOAPIC (PIN) or nullptr (MSI)
         */
        static Ioapic *lookup (gsi_t gsi)
        {
            for (auto l { list }; l; l = l->next)
                if (gsi >= l->gsi_base && gsi <= l->gsi_last)
                    return l;
