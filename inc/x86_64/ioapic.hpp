...
class Ioapic final : public List<Ioapic>
{
    private:
        ...
        // Indirect Registers
        enum class Ind_Register32 : uint8
        {
            ID      = 0x0,                  // ID Register
        ...
        inline auto read  (Dir_Register32 r) const    { return *reinterpret_cast<uint32 volatile *>(reg_base + std::to_underlying (r)); }
        inline void write (Dir_Register32 r, uint32 v) const { *reinterpret_cast<uint32 volatile *>(reg_base + std::to_underlying (r)) = v; }
        inline void write (Dir_Register8  r, uint8  v) const { *reinterpret_cast<uint8  volatile *>(reg_base + std::to_underlying (r)) = v; }
