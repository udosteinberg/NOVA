...
class Ptab
{
    public:
        ...
        class Entry
        {
            ...
            public:
                ...
                static constexpr unsigned bpl { bit_scan_reverse (PAGE_SIZE / sizeof (OAddr)) };
                ...
                static constexpr auto addr_mask() { return BIT64_RANGE (T::obits - 1, PAGE_BITS); }
        ...
        Status update (IAddr, OAddr, unsigned, Paging::Permissions, Memattr::Cacheability, Memattr::Shareability);
        ...
        // Default order limit is an L2 table fully populated with superpages
        static inline unsigned lim { T::lev_bit (2) + T::lev_bit (1) + T::lev_bit (0) };

        // Maximum leaf page size: 4 (512GB), 3 (1GB), 2 (2MB), 1 (4KB)
        static inline void set_leaf_max (unsigned l) { lim = min (lim, l * T::bpl); }
    ...
    private:
        ...
        {
            return Buddy::alloc (static_cast<uint8>(o));
        }
