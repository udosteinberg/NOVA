...
template<typename T, typename I, typename O> class Ptab
{
    ...
    protected:
        ...
        void deallocate_root()
        {
            replace (T::lev(), root, T { 0 });
        }
    ...
    private:
        ...
        static void deallocate (PTE *ptr, unsigned l)
        {
            if (l)
            ...
            operator delete (ptr, Cpu::online);
        }

        static void replace (unsigned l, PTE &ptr, T pte)
        {
            T old;
