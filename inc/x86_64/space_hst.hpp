...
class Space_hst : public Space_mem<Space_hst>
{
    public:
        ...
        static inline auto max_order() { return Hptp::lim; }

        [[nodiscard]] inline auto get_ptab (unsigned cpu) { return loc[cpu].root_init(); }

        inline auto lookup (uint64 v, uint64 &p, unsigned &o, Memattr &ma) const { return hptp.lookup (v, p, o, ma); }

        inline auto update (uint64 v, uint64 p, unsigned o, Paging::Permissions pm, Memattr ma) { return hptp.update (v, p, o, pm, ma); }
        ...
        static void user_access (uint64 addr, size_t size, bool a) { Space_mem::user_access (nova, addr, size, a, Memattr::dev()); }
};
