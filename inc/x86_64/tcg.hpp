...
class Tcg
{
    ...
    protected:
        ...
        struct Tpm2b final : public Size<uint16_t>                                  // Size is flexible
        {
            ...
            auto next() const { return reinterpret_cast<Tpm2b const *>(data() + size()); }
        };
        ...
        template<typename H> struct Session final
        {
            ...
            void compute_auth (Cmd *c, unsigned n)
            {
                ...
                auto const as { uint32_t { *reinterpret_cast<Unaligned_be<uint32_t> const *>(aa - sizeof (uint32_t)) } };
                auto const pa { aa + as };
                ...
                auto const auth { reinterpret_cast<Tpms_auth_command<H> *>(aa) };
                auth->handle = handle;
                auth->attr   = attr;
            ...
            [[nodiscard]] bool verify_auth (Res const *r, unsigned n, Tpm_cc cc)
            {
                ...
                auto const ps { uint32_t { *reinterpret_cast<Unaligned_be<uint32_t> const *>(pa - sizeof (uint32_t)) } };
                ...
                auto const auth { reinterpret_cast<Tpms_auth_response<H> const *>(pa + ps) };
                attr = auth->attr;
