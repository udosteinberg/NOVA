...
class Console_uart_ns16550 : protected Console_uart
{
    private:
        ...
        {
            if (mmap) [[likely]]
                return *std::start_lifetime_as<uint8_t volatile> (mmap + (std::to_underlying (r) << regs.shl));
            else
        ...
        {
            if (mmap) [[likely]]
                *std::start_lifetime_as<uint8_t volatile> (mmap + (std::to_underlying (r) << regs.shl)) = v;
            else
