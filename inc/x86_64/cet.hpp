...
class Cet
{
    public:
         ...
         */
        ALWAYS_INLINE
        static inline void ss_deactivate()
        {
#if defined(__CET__) && (__CET__ & 2)
            #define ASM_CET_1 clrssbsy %0;
            asm volatile (EXPAND (PATCH (ASM_CET_1,,PATCH_CET_SS)) : : "m" (SSTK_TOP));
#endif
        }
         ...
         */
        ALWAYS_INLINE
        static inline void ss_unwind()
        {
#if defined(__CET__) && (__CET__ & 2)
            uintptr_t ssp;
            #define ASM_CET_2 rdsspq %0; sub %0, %1; shr $3, %1; incsspq %1;
            asm volatile (EXPAND (PATCH (ASM_CET_2,,PATCH_CET_SS)) : "=&r" (ssp) : "r" (&SSTK_TOP));
#endif
        }
};
