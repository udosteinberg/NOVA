...
struct Sys_create_sm final : private Sys_abi
{
    ...
    uint32_t iid() const { return p2() & BIT_RANGE (31, 0); }
};
...
struct Sys_assign_dev final : private Sys_abi
{
    ...
    auto smmu() const { return p0() & ~OFFS_MASK (0); }

    unsigned long dma_old() const { return p1(); }

    unsigned long dma_new() const { return p2(); }

    auto dad() const { return p3(); }

    auto &sbw() const { return p1(); }
};
...
struct Sys_assign_int final : private Sys_abi
{
    ...
    auto vec() const { return static_cast<uint8_t>(p2() >> 8); }

    auto cpu() const { return static_cast<uint16_t>(p2() >> 16); }
