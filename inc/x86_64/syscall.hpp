...
{
    public:
        auto smmu() const { return ARG_1 & ~OFFS_MASK (0); }

        unsigned long dma_old() const { return ARG_2; }

        unsigned long dma_new() const { return ARG_3; }

        auto dad() const { return ARG_4; }

        auto &sbw() { return ARG_2; }
};
...
class Sys_assign_int final : public Sys_regs
{
    public:
        ...
        auto idx() const { return static_cast<uint16_t>(ARG_2 >> 16); }

        auto src() const { return static_cast<pci_t>(ARG_2 >> 32); }

        auto &msi_addr() { return ARG_2; }

        auto &msi_data() { return ARG_3; }
};
