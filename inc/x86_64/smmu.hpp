...
class Smmu final : public List<Smmu>, private Mmio
{
    private:
        ...
        void init_ctx()
        {
            ...
            write (Reg64::RTADDR, Kmem::ptr_to_phys (ctx));
    ...
    public:
        ...
        static void all_invalidate_iec (gsi_t gsi)
        {
            for (auto l { list }; l; l = l->next)
        ...
        [[nodiscard]] static bool gsi_lookup (gsi_t gsi, uint8_t &vec)
        {
            // Check allocation
            assert (irt);
            ...
            return irt[gsi].lookup (vec);
        }

        [[nodiscard]] static Status assign_int (gsi_t, cpu_t, uint8_t, pci_t, uint8_t, uintptr_t &, uintptr_t &);
