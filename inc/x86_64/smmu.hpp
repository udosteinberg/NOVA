...
class Smmu : public List<Smmu>, protected Mmio
{
    public:
        ...
        static constexpr uint32_t ise_msi (pci_t src, uint16_t idx) { return BIT (31) | uint32_t { idx } << 16 | Pci::bdf (src); }

        [[nodiscard]] virtual Status irte_get (Atomic<Entry> *&, Intid, pci_t, uint16_t, bool) = 0;
        [[nodiscard]] virtual Status irte_set (Atomic<Entry> *, Atomic<uintptr_t> &, bool, Intid, pci_t, apic_t, uint8_t, bool, uint32_t) = 0;
