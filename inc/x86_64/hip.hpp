...
class Hip final
{
    private:
        ...
        uint64_t        tmr_frq;                                                        // 0x50
        uint8_t         sbw_obj, sbw_hst, sbw_gst, sbw_dma, sbw_pio, sbw_msr, sbw_r[2]; // 0x58
        uint8_t         mco_obj, mco_hst, mco_gst, mco_dma, mco_pio, mco_msr, mco_r[2]; // 0x60
        uint16_t        cpu_bsp, cpu_max, kid_max, reserved;                            // 0x68
        uint16_t        sel_hst_arch, sel_hst_nova, sel_gst_arch, sel_gst_nova;         // 0x70
...
static_assert (__is_standard_layout (Hip) && sizeof (Hip) == sizeof (Hip_arch) + 0x80);
