    ...
    private:
        ...
        struct Element
        {
            ...
            enum class Type : uint32_t
            {
                ...
                ACPI_MADT   = 6,    // Validated ACPI MADT
                LOG20_TXT   = 7,    // TPM 2.0 Event Log (TXT)
                LOG20_TCG   = 8,    // TPM 2.0 Event Log (TCG)
                ACPI_MCFG   = 9,    // Validated ACPI MCFG
                TPR         = 13,   // TPR Request
                ACPI_DTPR   = 14,   // Validated ACPI DTPR
                ACPI_CEDT   = 15,   // Validated ACPI CEDT
            };
            ...
            explicit constexpr Element (Type t, uint32_t s) : type { std::to_underlying (t) }, size { s } {}
        };
        ...
        struct Element_log20_txt
        {
            ...
            explicit constexpr Element_log20_txt (uint64_t p, uint32_t s) : phys { p }, size { s } {}
        };
        ...
        struct Element_log20_tcg
        {
            ...
            explicit constexpr Element_log20_tcg (uint64_t p, uint32_t s) : phys { p }, size { s } {}
        };
        ...
        struct Data_efi_pre
        {
            Unaligned_le<uint64_t>    const size;
        ...
        static_assert (__is_standard_layout (Data_efi_pre) && alignof (Data_efi_pre) == 1 && sizeof (Data_efi_pre) == sizeof (uint64_t) + 36);
        ...
        struct Data_pre_mle
        {
            ...
            auto mtrr()       { return std::start_lifetime_as<Mtrr_backup      > (this + 1); }
            auto mtrr() const { return std::start_lifetime_as<Mtrr_backup const> (this + 1); }
        };

        static_assert (__is_standard_layout (Data_pre_mle) && alignof (Data_pre_mle) == 1 && sizeof (Data_pre_mle) == sizeof (uint64_t) + 24);
        ...
        static_assert (__is_standard_layout (Data_pre_acm) && alignof (Data_pre_acm) == 1 && sizeof (Data_pre_acm) == sizeof (uint64_t) + 92);
        ...
        static_assert (__is_standard_layout (Data_acm_mle) && alignof (Data_acm_mle) == 1 && sizeof (Data_acm_mle) == sizeof (uint64_t) + 148);
        ...
        static auto read  (Space s, Reg8  r)      { return *std::start_lifetime_as<uint8_t  volatile> (MMAP_GLB_TXTC + std::to_underlying (s) + std::to_underlying (r)); }
        static auto read  (Space s, Reg32 r)      { return *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_TXTC + std::to_underlying (s) + std::to_underlying (r)); }
        static auto read  (Space s, Reg64 r)      { return *std::start_lifetime_as<uint64_t volatile> (MMAP_GLB_TXTC + std::to_underlying (s) + std::to_underlying (r)); }

        static void write (Space s, Reg8  r, uint8_t  v) { *std::start_lifetime_as<uint8_t  volatile> (MMAP_GLB_TXTC + std::to_underlying (s) + std::to_underlying (r)) = v; }
        static void write (Space s, Reg32 r, uint32_t v) { *std::start_lifetime_as<uint32_t volatile> (MMAP_GLB_TXTC + std::to_underlying (s) + std::to_underlying (r)) = v; }
        static void write (Space s, Reg64 r, uint64_t v) { *std::start_lifetime_as<uint64_t volatile> (MMAP_GLB_TXTC + std::to_underlying (s) + std::to_underlying (r)) = v; }
        ...
        [[nodiscard]] static bool reserve (uintptr_t &ptr, size_t &avl, size_t size)
        {
            if (size > avl) [[unlikely]]
                return false;
        ...
        template<typename T> [[nodiscard]] static T const *consume (uintptr_t &ptr, size_t &avl, size_t x = 0)
        {
            // The remaining heap must cover T before its size field can be read
            if (sizeof (T) + x > avl) [[unlikely]]
                return nullptr;

            auto const t { std::start_lifetime_as<T const> (ptr) };

            // T must declare a size that is sane and fits
            size_t const size { t->size };
            if (sizeof (T) + x > size || !reserve (ptr, avl, size)) [[unlikely]]
                return nullptr;

            return t;
        }

        ALWAYS_INLINE static inline bool check_acm (Mle_header *, uintptr_t, size_t, uint32_t, uint32_t &, uint32_t &, uint32_t &);
        ALWAYS_INLINE static inline bool init_heap (Mle_header *, uintptr_t, size_t, uint32_t, uint32_t, unsigned);
        ALWAYS_INLINE static inline bool init_mtrr (uint64_t, uint64_t, unsigned, unsigned);

        static void parse_elem (uintptr_t, size_t, uintptr_t);
