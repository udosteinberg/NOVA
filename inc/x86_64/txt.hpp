...
#pragma pack(1)
        ...
        static_assert (__is_standard_layout (Data_efi_pre) && sizeof (Data_efi_pre) == sizeof (Data) + 36);

        // Section C.3: PRE to MLE Data
        struct Data_pre_mle : Heap_alloc
        {
            Data     const  data;
            ...
            struct Mtrr : Heap_alloc
            {
                ...
                explicit Mtrr (uint64_t b, uint64_t m) : base { b }, mask { m } {}
            };

            explicit Data_pre_mle (unsigned c) : data { .size = sizeof (Data_pre_mle) + c * sizeof (Mtrr) } {}

            auto mtrr() const { return reinterpret_cast<Mtrr const *>(reinterpret_cast<uintptr_t>(this + 1)); }
        };

        static_assert (__is_standard_layout (Data_pre_mle) && sizeof (Data_pre_mle) == sizeof (Data) + 32);

        // Section C.4: PRE to ACM Data
        struct Data_pre_acm : Heap_alloc
        {
            Data            data            { sizeof (Data_pre_acm) };
            uint32_t        version         { ver_pre_acm };            // 0    v1+
            uint32_t        flags;                                      // 4    v7+
        ...
        static_assert (__is_standard_layout (Data_pre_acm) && sizeof (Data_pre_acm) == sizeof (Data) + 92);
        ...
        static_assert (__is_standard_layout (Data_acm_mle) && sizeof (Data_acm_mle) == sizeof (Data) + 148);
