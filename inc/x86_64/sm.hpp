...
class Sm final : public Kobject, private Queue<Ec>
{
    ...
    public:
        [[nodiscard]] static Sm *create (Status &s, uintptr_t v, bool i)
        {
            auto const sm { new (cache) Sm (v, i) };
        ...
        gsi_t get_gsi() const
        {
            // Only valid for interrupt semaphores
            assert (subtype == Kobject::Subtype::SM_INT);
