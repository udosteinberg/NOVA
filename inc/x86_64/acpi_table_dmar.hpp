...
class Acpi_table_dmar final
{
    private:
        ...
        struct Scope : private Unaligned_le<uint8_t>            // 0
        {
            ...
            Unaligned_le<uint8_t>   flags;                      // 2
            Unaligned_le<uint8_t>   reserved;                   // 3
            Unaligned_le<uint8_t>   id;                         // 4
            Unaligned_le<uint8_t>   bus;                        // 5
            ...
            struct Path
            {
                Unaligned_le<uint8_t> d, f;
            };

            static_assert (alignof (Path) == 1 && sizeof (Path) == 2);

            [[nodiscard]] auto type() const { return Type { uint8_t { *this } }; }

            [[nodiscard]] bool parse (uint16_t, pci_t &) const;
        };

        static_assert (alignof (Scope) == 1 && sizeof (Scope) == 6);

        /*
         * 8.2: Remapping Structure
         */
        struct Remapping : private Unaligned_le<uint16_t>       // 0
        {
            ...
            [[nodiscard]] auto type() const { return Type { uint16_t { *this } }; }
        };
        ...
        struct Remapping_drhd : public Remapping                // 0
        {
            ...
            Unaligned_le<uint8_t>   ord;                        // 5
            Unaligned_le<uint16_t>  seg;                        // 6
