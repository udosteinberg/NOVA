    ...
    private:
        ...
        class Inttable final
        {
            private:
                ...
                [[nodiscard]] static auto update (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool u, Intid iid, uint32_t ise, Irte n, auto const &live)
                {
                    Irte o { e->load() };
            ...
            public:
                ...
                [[nodiscard]] static auto set (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool pin, Intid iid, apic_t dst, uint8_t vec, bool trg, uint32_t ise)
                {
                    return update (e, i, !pin, iid, ise, Irte { dst, vec, trg, static_cast<uint16_t>(ise) }, [] (Irte const &) { return true; });
                }

                [[nodiscard]] static auto clr (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool pin, Intid iid, apic_t dst, uint8_t vec)
                {
                    return update (e, i, true, iid, 0x0, Irte { pin }, [dst, vec] (Irte const &o) { return o.dst() == dst && o.vec() == vec; });
                }
        ...
        class Devtable final
        {
            ...
            public:
                ...
                [[nodiscard]] Atomic<Entry> *cte (pci_t src)
                {
                    ...
                    if (!e.present()) [[unlikely]] {
                        ...
                        if (ptr->compare_exchange (e, tmp, Dpt_itl::noncoherent)) [[likely]]
                    ...
                    return &e.table()->slot[Pci::ari (src)];
                }
        ...
        [[nodiscard]] Status irte_get (Atomic<Entry> *&e, Intid iid, pci_t, uint16_t, bool) override final
        {
            e = itbl->entry (iid);
        ...
        [[nodiscard]] Status irte_set (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool pin, Intid iid, pci_t, apic_t dst, uint8_t vec, bool trg, uint32_t ise) override final
        {
            return Inttable::set (e, i, pin, iid, dst, vec, trg, ise);
        }

        [[nodiscard]] Status irte_clr (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool pin, Intid iid, pci_t, apic_t dst, uint8_t vec) override final
        {
            return Inttable::clr (e, i, pin, iid, dst, vec);
        }
