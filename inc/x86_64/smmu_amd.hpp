    ...
    private:
        ...
        class Inttable final
        {
            private:
                ...
                [[nodiscard]] static auto update (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool u, pci_t src, uint32_t ise, Irte n, auto const &live)
                {
                    Irte o { e->load() };
                    ...
                    if (live (o)) [[likely]] {
                        ...
                        // Update interrupt source encoding if the live sink changed
                        if (u)
                            i = ise;
            ...
            public:
                ...
                [[nodiscard]] static auto set (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool pin, pci_t src, apic_t dst, uint8_t vec, bool trg, uint32_t ise)
                {
                    return update (e, i, !pin, src, ise, Irte { dst, vec, trg }, [] (Irte const &) { return true; });
                }

                [[nodiscard]] static auto clr (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool pin, pci_t src, apic_t dst, uint8_t vec)
                {
                    return update (e, i, true, src, 0x0, Irte { pin }, [dst, vec] (Irte const &o) { return o.dst() == dst && o.vec() == vec; });
                }
        ...
        class Devtable final
        {
            ...
            public:
                ...
                struct Cte final : public Entry
                {
                    ...
                    explicit constexpr Cte (Entry e) : Entry { e } {}
                ...
                [[nodiscard]] Inttable *table (pci_t src, bool &alloc)
                {
                    ...
                    if (!e.present()) [[unlikely]] {
                        ...
                        if (ptr->compare_exchange (e, tmp, false)) [[likely]]
                            return tbl;
                ...
                [[nodiscard]] Atomic<Entry> *cte (pci_t src) { size_t const i { Pci::bdf (src) }; return i < num_dev ? &slot[i].cte : nullptr; }
                [[nodiscard]] Atomic<Entry> *ite (pci_t src) { size_t const i { Pci::bdf (src) }; return i < num_dev ? &slot[i].ite : nullptr; }
        ...
        {
            explicit constexpr Cmd_invalidate_iommu_pages (uint16_t dom) : Cmd { Type::INVALIDATE_IOMMU_PAGES, uint64_t { dom } << 32, BITN_RANGE (62, 12) | BIT_RANGE (1, 0) } {}
        };
        ...
        [[nodiscard]] Status irte_set (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool pin, Intid, pci_t src, apic_t dst, uint8_t vec, bool trg, uint32_t ise) override final
        {
            return Inttable::set (e, i, pin, src, dst, vec, trg, ise);
        }

        [[nodiscard]] Status irte_clr (Atomic<Entry> *e, Atomic<uintptr_t> &i, bool pin, Intid, pci_t src, apic_t dst, uint8_t vec) override final
        {
            return Inttable::clr (e, i, pin, src, dst, vec);
        }
        ...
        [[nodiscard]] bool nova_assigned (pci_t) override final;
