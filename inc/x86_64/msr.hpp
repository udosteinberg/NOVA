...
class Msr final
{
    public:
        ...
        enum class Register : unsigned
        {
            ...
            FSB_FREQ                        = 0xcd,
            PLATFORM_INFO                   = 0xce,
            IA32_CORE_CAPABILITIES          = 0xcf,         // IA32_CORE_CAPABILITIES
            IA32_MPERF                      = 0xe7,
            IA32_APERF                      = 0xe8,
            IA32_MTRR_CAP                   = 0xfe,         // MTRR
            IA32_ARCH_CAPABILITIES          = 0x10a,        // IA32_ARCH_CAPABILITIES
        ...
        static void write (Register msr, uint64_t val)
        {
            asm volatile ("wrmsr" : : "d" (static_cast<uint32_t>(val >> 32)), "a" (static_cast<uint32_t>(val)), "c" (msr));
        }

        static auto read  (Array r, unsigned b, unsigned n)       { return read (static_cast<Register>(std::to_underlying (r) + b * n)); }
        static void write (Array r, unsigned b, unsigned n, uint64_t v) { write (static_cast<Register>(std::to_underlying (r) + b * n), v); }
};
