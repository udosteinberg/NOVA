...
class Acpi_fixed final
{
        ...
        static unsigned read (Acpi_gas &g)
        {
            if (g.asid == Acpi_gas::Asid::PIO) {
                switch (g.bits) {
                    ...
                    case  8: return Io::in<uint8_t> (static_cast<unsigned>(g.addr));
                    case 16: return Io::in<uint16_t>(static_cast<unsigned>(g.addr));
                    case 32: return Io::in<uint32_t>(static_cast<unsigned>(g.addr));
                }
            }
        ...
        static void write (Acpi_gas &g, unsigned v)
        {
            if (g.asid == Acpi_gas::Asid::PIO) {
                switch (g.bits) {
                    ...
                    case  8: return Io::out<uint8_t> (static_cast<unsigned>(g.addr), static_cast<uint8_t> (v));
                    case 16: return Io::out<uint16_t>(static_cast<unsigned>(g.addr), static_cast<uint16_t>(v));
                    case 32: return Io::out<uint32_t>(static_cast<unsigned>(g.addr), static_cast<uint32_t>(v));
                }
            }
        }
        ...
        static void write_block (Acpi_gas &g, uint8_t v)
        {
            ...
            if (g.asid == Acpi_gas::Asid::PIO)
                for (unsigned i = 0; i < g.bits / 8; i++)
                    Io::out<uint8_t>(static_cast<unsigned>(g.addr) + i, v);
        }
    ...
    public:
         ...
         * Enable ACPI mode
         */
        static void enable (unsigned port, uint8_t e, uint8_t c, uint8_t p)
        {
            assert (port);
