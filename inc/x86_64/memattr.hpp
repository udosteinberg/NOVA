...
class Memattr final
{
    ...
    public:
        // Physical address bits min/max if CPUID reports nothing else
        static constexpr unsigned pbits_min { 36 }, pbits_max { 52 };

        static inline constinit unsigned obits { pbits_min };
        ...
        explicit constexpr Memattr (Cache c, Keyid k = 0) : val { k << 3 | std::to_underlying (c) } {}

        constexpr bool operator== (Memattr const &) const = default;

        static constexpr auto ram() { return Memattr { Cache::MEM_WB }; }
        static constexpr auto gfx() { return Memattr { Cache::MEM_WC }; }
        static constexpr auto dev() { return Memattr { Cache::MEM_UC }; }

        constexpr auto keyid() const { return BIT_RANGE (14, 0) & val >> 3; }

        constexpr auto cache_s1() const { return BIT_RANGE (2, 0) & val; }
        constexpr auto cache_s2() const { return BIT_RANGE (2, 0) & pat >> 8 * cache_s1(); }

        auto key_encode() const { return uint64_t { keyid() } << obits; }

        static auto key_decode (uint64_t val) { return Keyid (val >> obits & (BIT (kbits) - 1)); }

        static constexpr auto ept_to_ca (unsigned e) { return Cache (BIT_RANGE (2, 0) & rev >> 8 * e); }
