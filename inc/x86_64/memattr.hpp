...
class Memattr final
{
    ...
    public:
        enum class Cacheability : unsigned
        {
            MEM_WB  = CA_MEM_WB,
            MEM_WT  = CA_MEM_WT,
            MEM_WC  = CA_MEM_WC,
        ...
        enum class Shareability : unsigned
        {
            NONE    = 0,
            OUTER   = 0,
            INNER   = 0,
        ...
        static unsigned ca_to_ept (Cacheability ca) { return pat >> 8 * std::to_underlying (ca) & BIT_RANGE (2, 0); }

        static Cacheability ept_to_ca (unsigned e) { return Cacheability (rev >> 8 * e & BIT_RANGE (2, 0)); }
};
