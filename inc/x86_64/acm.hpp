...
class Acm final
{
    public:
        ...
        struct Header
        {
            ...
            auto pch (Info const *i) const { return i->version < 2 ? nullptr : std::start_lifetime_as<List const> (reinterpret_cast<uintptr_t>(this) + i->pch_list); }
            auto cpu (Info const *i) const { return i->version < 4 ? nullptr : std::start_lifetime_as<List const> (reinterpret_cast<uintptr_t>(this) + i->cpu_list); }
            auto tpm (Info const *i) const { return i->version < 5 ? 0 :  *std::start_lifetime_as<uint32_t const> (reinterpret_cast<uintptr_t>(this) + i->tpm_list); }
        };
