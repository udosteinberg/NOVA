...
class Vmcs final
{
    ...
    public:
        ...
        // Automatically derive VMCS access width from component encoding
        template<Encoding E> using vmcs_t = std::conditional_t<((std::to_underlying (E) >> 13) & 3) == 0, uint16_t,
                                            std::conditional_t<((std::to_underlying (E) >> 13) & 3) == 1, uint64_t,
                                            std::conditional_t<((std::to_underlying (E) >> 13) & 3) == 2, uint32_t, uintptr_t>>>;
        ...
        template<Encoding E> static inline auto read()
        {
            uintptr_t v;
            asm volatile ("vmread %1, %0" : "=rm" (v) : "r" (std::to_underlying (E)) : "cc");
            return static_cast<vmcs_t<E>>(v);
        }

        template<Encoding E> static inline void write (vmcs_t<E> v)
        {
