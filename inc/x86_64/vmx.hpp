...
{
    private:
        uint32 const    rev         { static_cast<uint32>(basic) };
        uint32 const    abort       { 0 };

        static Vmcs *   root        CPULOCAL;
        static uint64   basic       CPULOCAL;
        static uint64   ept_vpid    CPULOCAL;
        static uint32   pin         CPULOCAL;
        static uint32   ent         CPULOCAL;
        static uint32   exi_pri     CPULOCAL;
        static uint64   exi_sec     CPULOCAL;

        static inline void vmxon()
        {
            uint64 phys = Kmem::ptr_to_phys (root);
    ...
    public:
        ...
        static uint32       cpu_pri_clr CPULOCAL;
        static uint32       cpu_pri_set CPULOCAL;
        static uint32       cpu_sec_clr CPULOCAL;
        static uint32       cpu_sec_set CPULOCAL;
        static uint64       cpu_ter_clr CPULOCAL;
        static uint64       cpu_ter_set CPULOCAL;
        ...
        enum Exi_pri
        {
            ...
            EXI_LOAD_HOST_CET       = BIT (28),
            EXI_LOAD_HOST_PKRS      = BIT (29),
        ...
        void init (Bitmap_pio *, Bitmap_msr *, uintptr_t, uintptr_t, uint64, uint16);
        ...
        {
            uintptr_t v;
            asm volatile ("vmread %1, %0" : "=rm" (v) : "r" (static_cast<uintptr_t>(e)) : "cc");
        ...
        {
            asm volatile ("vmwrite %0, %1" : : "rm" (static_cast<uintptr_t>(v)), "r" (static_cast<uintptr_t>(e)) : "cc");
        }

        ALWAYS_INLINE
        static inline uint16 vpid()
        {
            return has_vpid() ? read<uint16> (Encoding::VPID) : 0;
        }
