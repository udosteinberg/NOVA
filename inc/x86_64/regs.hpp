...
template <> inline mword  Exc_regs::get_g_cs_dl<Vmcs>()        const { return Vmcs::read<uint32> (Vmcs::Encoding::GUEST_AR_CS) >> 13 & 0x3; }
template <> inline mword  Exc_regs::get_g_flags<Vmcb>()        const { return static_cast<mword>(vmcb->rflags); }
template <> inline mword  Exc_regs::get_g_flags<Vmcs>()        const { return Vmcs::read<mword>  (Vmcs::Encoding::GUEST_RFLAGS); }
