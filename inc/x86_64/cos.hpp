 ...
 * Cache Allocation Technology (CAT) + Code/Data Prioritization (CDP)
...
class Cos
{
    ...
    private:
        ...
        static uint8  config    CPULOCAL;       // Current QOS configuration
        static uint8  supcfg    CPULOCAL;       // Supported QOS configuration
        static uint8  cos_l3    CPULOCAL;       // Supported L3 COS MSRs
        static uint8  cos_l2    CPULOCAL;       // Supported L2 COS MSRs
        static uint8  cos_mb    CPULOCAL;       // Supported MB COS MSRs
        static uint8  hcb_l3    CPULOCAL;       // Supported L3 highest capacity bit
        static uint8  hcb_l2    CPULOCAL;       // Supported L2 highest capacity bit
        static uint16 del_mb    CPULOCAL;       // Supported MB maximum delay value
        static uint16 current   CPULOCAL;       // Currently active COS

        static inline void set_cos (uint16 cos) { Msr::write (Msr::Register::IA32_PQR_ASSOC, static_cast<uint64>(cos) << 32); }

        static inline void set_l3_mask (uint16 n, uint32 val) { Msr::write (Msr::Array::IA32_L3_MASK, 1, n, val); }
        static inline void set_l2_mask (uint16 n, uint32 val) { Msr::write (Msr::Array::IA32_L2_MASK, 1, n, val); }
        static inline void set_mb_thrt (uint16 n, uint16 val) { Msr::write (Msr::Array::IA32_MB_THRT, 1, n, val); }

        static inline void set_qos_cfg (uint8 cfg)
        {
         ...
         */
        static inline bool valid_cbm (uint32 cbm, uint8 hcb)
        {
            if (!cbm || cbm > BIT_RANGE (hcb, 0))
                return false;

            auto c = cbm >> bit_scan_forward (cbm);
    ...
    public:
         ...
         * @return      True if valid, false otherwise
         */
        static inline bool valid_cos (uint16 cos)
        {
            uint16 cos_num { 1 };
            ...
            if (config) {
                ...
                uint8 l3 = cos_l3 >> !!(config & Config::CDP_L3);
                if (cos_num < l3)
                    cos_num = l3;

                // If CDP is enabled, then the valid COS range is half the number of MSRs
                uint8 l2 = cos_l2 >> !!(config & Config::CDP_L2);
                if (cos_num < l2)
                    cos_num = l2;
         ...
         */
        static inline void make_current (uint16 cos)
        {
         ...
         */
        static inline Status cfg_qos (uint8 cfg)
        {
            if (config)
         ...
         */
        static inline Status cfg_l3_mask (uint16 n, uint32 cbm)
        {
            if (!config)
                return Status::ABORTED;
         ...
         */
        static inline Status cfg_l2_mask (uint16 n, uint32 cbm)
        {
            if (!config)
                return Status::ABORTED;
         ...
         */
        static inline Status cfg_mb_thrt (uint16 n, uint16 del)
        {
            if (!config)
                return Status::ABORTED;
