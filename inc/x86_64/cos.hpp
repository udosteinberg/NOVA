    ...
    public:
        ...
        static inline bool valid_cos (uint16 cos)
        {
            ...
            if (config) {
                ...
                uint8 const l3 = cos_l3 >> !!(config & Config::CDP_L3);
                if (cos_num < l3)
                    cos_num = l3;

                // If CDP is enabled, then the valid COS range is half the number of MSRs
                uint8 const l2 = cos_l2 >> !!(config & Config::CDP_L2);
                if (cos_num < l2)
                    cos_num = l2;
         ...
         */
        static inline auto cfg_qos (uint8 cfg)
        {
            if (config)
         ...
         */
        static inline auto cfg_l3_mask (uint16 n, uint32 cbm)
        {
            if (!config)
                return Status::ABORTED;
         ...
         */
        static inline auto cfg_l2_mask (uint16 n, uint32 cbm)
        {
            if (!config)
                return Status::ABORTED;
         ...
         */
        static inline auto cfg_mb_thrt (uint16 n, uint16 del)
        {
            if (!config)
                return Status::ABORTED;
