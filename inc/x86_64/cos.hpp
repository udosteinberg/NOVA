...
class Cos final
{
    ...
    private:
        ...
        static uint16_t current CPULOCAL;       // Currently active COS

        static inline void set_cos (uint16_t cos) { Msr::write (Msr::Register::IA32_PQR_ASSOC, static_cast<uint64_t>(cos) << 32); }
    ...
    public:
         ...
         * @return      True if valid, false otherwise
         */
        static inline bool valid_cos (uint16_t cos)
        {
            uint16_t cos_num { 1 };
         ...
         */
        static inline void make_current (uint16_t cos)
        {
