...
class Acpi_arch
{
    private:
        ...
        static uintptr_t rsdp_probe (uintptr_t map, uintptr_t off, size_t len)
        {
            for (auto ptr { map + off }; ptr < map + off + len; ptr += 16)
                if (std::start_lifetime_as<Acpi_table_rsdp const> (ptr)->valid())
                    return ptr - map;
    ...
    protected:
        ...
        static uintptr_t rsdp_find()
        {
            ...
            if ((addr = rsdp_probe (map, *std::start_lifetime_as<uint16_t const> (map + 0x40e) << 4, 0x400)) ||
        ...
        static void parse_tables (bool &ret)
        {
            Hptp::master_map (sipi, sipi, 0, Paging::Permissions (Paging::XS | Paging::R), Memattr::ram());

            void const *ptr;

            if ((ptr = Acpi_table::map (dmar, 0))) [[likely]]
                ret &= std::start_lifetime_as<Acpi_table_dmar const> (ptr)->parse();
            if ((ptr = Acpi_table::map (ivrs, 0))) [[likely]]
                ret &= std::start_lifetime_as<Acpi_table_ivrs const> (ptr)->parse();
            if ((ptr = Acpi_table::map (lpit, 0))) [[likely]]
                ret &= std::start_lifetime_as<Acpi_table_lpit const> (ptr)->parse();
        }
        ...
        static void wake_prepare()
        {
            ...
            if (facs)
                std::start_lifetime_as<Acpi_table_facs> (Hptp::map (MMAP_GLB_MAP0, facs, Paging::W))->set_wake (sipi + static_cast<uint32_t>(&__wake_vec - &__init_aps));
        }

        static inline constinit uint64_t dbg2 {}, dmar {}, facs {}, fadt {}, hest {}, ivrs {}, lpit {}, madt {}, mcfg {}, spcr {}, srat {}, tpm2 {}, xsdt {};
