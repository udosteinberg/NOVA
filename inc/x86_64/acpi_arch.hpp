...
class Acpi_arch
{
    ...
    protected:
        ...
        {
            uintptr_t addr, map { reinterpret_cast<uintptr_t>(Hptp::map (Hptp::Remap::MAP0, 0)) };
        ...
        {
            if (hpet)
                static_cast<Acpi_table_hpet *>(Hptp::map (Hptp::Remap::MAP0, hpet))->parse();
            if (mcfg)
                static_cast<Acpi_table_mcfg *>(Hptp::map (Hptp::Remap::MAP0, mcfg))->parse();
            if (dmar)
                static_cast<Acpi_table_dmar *>(Hptp::map (Hptp::Remap::MAP0, dmar))->parse();
            if (lpit)
                static_cast<Acpi_table_lpit *>(Hptp::map (Hptp::Remap::MAP0, lpit))->parse();
        }
        ...
        static void wake_prepare()
        {
            ...
            auto const ptr { Hptp::map (Hptp::Remap::MAP0, sipi, true) };
            memcpy (sra, ptr, sizeof (sra));
            memcpy (ptr, reinterpret_cast<void *>(Kmem::sym_to_virt (&__init_aps)), len);

            if (facs)
                static_cast<Acpi_table_facs *>(Hptp::map (Hptp::Remap::MAP0, facs, true))->set_wake (sipi + static_cast<uint32_t>(&__wake_vec - &__init_aps));
        }

        static inline uint64_t dbg2 { 0 }, dmar { 0 }, facs { 0 }, fadt { 0 }, hpet { 0 }, lpit { 0 }, madt { 0 }, mcfg { 0 }, spcr { 0 };

    public:
        static constexpr auto sipi { PAGE_SIZE };

        // SIPI vector must be a PAGE-aligned address < 1MB
        static_assert (sipi < BIT (20) && !(sipi & OFFS_MASK));
        ...
        {
            memcpy (Hptp::map (Hptp::Remap::MAP0, sipi, true), sra, sizeof (sra));
        }
};
