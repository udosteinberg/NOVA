 ...
 */
template<typename T, int L, int S, int M> requires (Valid_MO<L, S, M> && std::integral<T>) class Atomic<T, L, S, M> final
{
    private:
        T val;
    ...
    public:
        ...
        ALWAYS_INLINE inline T load (int o = L) const
        {
            return __atomic_load_n (&val, o);
        }

        ALWAYS_INLINE inline void store (T v, int o = S)
        {
