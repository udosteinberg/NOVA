...
template<typename T, int L, int S> class Atomic<T, L, S, std::enable_if_t<std::is_integral<T>::value>> final
{
    ...
    public:
        ...
        ALWAYS_INLINE inline void exchange           (T &o, T const &n) {        __atomic_exchange           (&val, &n, &o,                          __ATOMIC_SEQ_CST); }
        ALWAYS_INLINE inline T    exchange_n         (      T const  n) { return __atomic_exchange_n         (&val,  n,                              __ATOMIC_SEQ_CST); }
        ALWAYS_INLINE inline bool compare_exchange   (T &o, T const &n) { return __atomic_compare_exchange   (&val, &o, &n, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST); }
        ALWAYS_INLINE inline bool compare_exchange_n (T &o, T const  n) { return __atomic_compare_exchange_n (&val, &o,  n, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST); }
