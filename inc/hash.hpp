...
class Sha
{
    ...
    protected:
        ...
        template<typename T> void preprocess (T *h, uint8_t const *m, size_t s, bool more, void (*hash)(T *, uint8_t const *))
        {
            ...
            *std::start_lifetime_as<Unaligned_be<L>> (b + res_size) = L { len } * __CHAR_BIT__;
        ...
        template<typename T> static void sha1 (T *h, uint8_t const *m)
        {
            ...
            auto const p { std::start_lifetime_as_array<Unaligned_be<T> const> (m, 16) };
...
template<uint16_t H, unsigned D, typename T, T... I> class Hash : private Sha
{
    ...
    public:
        ...
        {
            constexpr auto n { dig_size / sizeof (T) };
            auto const d { std::start_lifetime_as_array<Unaligned_be<T>> (p, n) };
