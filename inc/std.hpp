...
namespace std
{
    ...
    template<typename T> struct remove_pointer             { using type = T; };     // non-pointer
    template<typename T> struct remove_pointer<T*>         { using type = T; };     // pointer
    ...
    template<>           struct __helper_int<__int128_t>            : true_type  {};
    template<>           struct __helper_int<__uint128_t>           : true_type  {};
    ...
    template<typename T> concept signed_integral   = integral<T> && (T(0) > T(-1));
    template<typename T> concept unsigned_integral = integral<T> && (T(0) < T(-1));
    ...
    // See https://en.cppreference.com/cpp/types/is_standard_layout
    template<typename T> concept standard_layout = __is_standard_layout (T);

    // See https://en.cppreference.com/cpp/types/is_constructible
    template<typename T> concept trivially_constructible = __is_trivially_constructible (T);

    // See https://en.cppreference.com/cpp/types/is_destructible
#if __has_builtin(__is_trivially_destructible)                      // since gcc-16
    template<typename T> concept trivially_destructible = __is_trivially_destructible (T);
#else                                                               // up to gcc-15
    template<typename T> concept trivially_destructible = __has_trivial_destructor (T);
#endif

    // See https://en.cppreference.com/cpp/types/is_trivially_copyable
    template<typename T> concept trivially_copyable = __is_trivially_copyable (T);

#if __has_builtin(__builtin_is_implicit_lifetime)                   // since gcc-16
    template<typename T> concept implicitly_creatable = __builtin_is_implicit_lifetime (T);
#else                                                               // up to gcc-15
    template<typename T> concept implicitly_creatable = trivially_copyable<T>;
#endif

    // See https://en.cppreference.com/cpp/utility/launder
    template<typename T> [[nodiscard]] __attribute__((nonnull)) constexpr T* launder (T *t) noexcept { return __builtin_launder (t); }

    // See https://en.cppreference.com/cpp/memory/start_lifetime_as
    template<implicitly_creatable T> [[nodiscard]] __attribute__((nonnull)) inline auto start_lifetime_as (void *p) noexcept
    {
        return launder (static_cast<T *>(__builtin_memmove (p, p, sizeof (T))));
    }

    template<implicitly_creatable T> [[nodiscard]] inline T const          *start_lifetime_as (void const          *p) noexcept { return start_lifetime_as<T> (const_cast<void *>(p)); }
    template<implicitly_creatable T> [[nodiscard]] inline T       volatile *start_lifetime_as (void       volatile *p) noexcept { return start_lifetime_as<T> (const_cast<void *>(p)); }
    template<implicitly_creatable T> [[nodiscard]] inline T const volatile *start_lifetime_as (void const volatile *p) noexcept { return start_lifetime_as<T> (const_cast<void *>(p)); }
    template<implicitly_creatable T> [[nodiscard]] inline T                *start_lifetime_as (uintptr_t p)            noexcept { return start_lifetime_as<T> (reinterpret_cast<void *>(p)); }

    // See https://en.cppreference.com/cpp/memory/start_lifetime_as
    template<implicitly_creatable T> [[nodiscard]] inline auto start_lifetime_as_array (void *p, size_t n) noexcept
    {
        // C++23 (P2679R2): If n == 0, the function has no effects and returns p
        if (!n) [[unlikely]]
            return static_cast<T *>(p);

        // If n > 0, passing a nullptr is undefined behavior
        if (!p) [[unlikely]]
            __builtin_unreachable();

        return launder (static_cast<T *>(__builtin_memmove (p, p, sizeof (T) * n)));
    }

    template<implicitly_creatable T> [[nodiscard]] inline T const          *start_lifetime_as_array (void const          *p, size_t n) noexcept { return start_lifetime_as_array<T> (const_cast<void *>(p), n); }
    template<implicitly_creatable T> [[nodiscard]] inline T       volatile *start_lifetime_as_array (void       volatile *p, size_t n) noexcept { return start_lifetime_as_array<T> (const_cast<void *>(p), n); }
    template<implicitly_creatable T> [[nodiscard]] inline T const volatile *start_lifetime_as_array (void const volatile *p, size_t n) noexcept { return start_lifetime_as_array<T> (const_cast<void *>(p), n); }
    template<implicitly_creatable T> [[nodiscard]] inline T                *start_lifetime_as_array (uintptr_t p,            size_t n) noexcept { return start_lifetime_as_array<T> (reinterpret_cast<void *>(p), n); }
}
